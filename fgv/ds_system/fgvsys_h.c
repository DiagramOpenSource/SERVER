...
static char *letrasm = NULL;
static char *textom[2] = {NULL,NULL};
...
static int dame_cuadro_lineas(int flin,int *pi,int *pf)
{
   int limitel = -((v_mpfirel(flin)->lineas%160)/6);
   int campini,campfin;
   int c;
   int p;
   int visual = 0,lineal;
   ...
   {
      numcam = (v_mpfi(flin)->f_campos[c-1].c_mante.c_actual % 1000) + 1;
   ...
   if (fi_modo_lineas[flin])
   {
       *pi = minlin*100+colini;
       *pf = maxlin*100+colini+ limitel*fi_modo_lineas[flin] - 1;
       return(1);
   }
...
int flin,faclin,modo,r;
{
   ...
   if (!modo)
   {
      if (!cambio_color_fijo && color_factor && campo_color > 0 && file_color == flin && faclin == 100 && color_factor_inicio)
	  {
      ...
      else if (color_factor && campo_color > 0 && file_color == flin)
         {		 
         ...
         /*
         #ifdef FGVWIN
         if (color_factor && campo_color > 0 && (rentra_sal_atrib == 'N' || rentra_sal_atrib == ' '))
         {		 
         rentra_sal_cc = 5;
         rentra_sal_cf = 0;		 
         }
      ...
      if (v_mpfi(flin)->f_campos[numcam-1].c_mante.c_visual == 0)
         if (!nopinta && (lineal != 0 || modo > 1)) 
         {
            if (lineal != 0) 
            {
               ...
               if (fi_modo_lineas[flin])
               {
                   p = p + ((faclin - 100)/100)*fi_modo_lineas[flin];
                   visual = faclin;
...
int *color_trobato;
{
   ...
   *faclin += IncrementoLineas;
   if (*faclin != fac)  {    /* si hay lineas intento borrar el resto de pagina */
      fac = *faclin;
...
static int entra_linea(f,linea,faclin,ultima,alta,final,lleno,lim1,lim2,baja,saleret,noprim,ipfi,desorden,primera,ant_qj)
int f;
char **linea;
int *faclin;
char **ultima;
...
int *ant_qj;
{
   ...
   letrasm = __Texto[486];
   textom[0] = __Texto[484];
   textom[1] = __Texto[485];
   ...
   {      
      ...
      do {  
         ...
         *ant_qj = -1;
         ...
         qj = pregun_clave(&h,f,*faclin,altai);
         ...
         */
         ...
         if (saleret == 1 && (!qj || qj == 1 || qj == 7)) {
            if (!qj && *final) qj = 7;
            if (memoplin > 4) sprintf(v_memo(5004)->alfabetica,"%d",((!qj) ? 0 : -1));
            if (!qj)
               qj = 7;
         }
      } while (qj != 1 && qj != 7 && (clave_lin_trata(f,tmp,7,lim2) || clave_lin_trata(f,tmp,8,lim1) ||
         (*final == 1 && ((qj == 3 && *desorden != 2) || (qj == 5 && *desorden != 2)))));
      ...
      if (*final == 0) 
      {
         ...
         if (leida == 0 && *lleno != -2)
         {
            w = w_ini(2401,2480);
            w_salva(w);
            switch(menusg(textom,letrasm,0,"",1)) 
            {
            case -1:
            case 0:
               libera_ind(v_mpfi(f));
      ...
      else
      {
         ...
         if (trata_reg(f,2,ISLOCK,0,1,__Texto[483]) < 0)
         {
           if (ya_puesto)
           {
              aviso(0,"XX1");
      ...
      if (leida == 0
         ...
         if ((qj = entra_datos(altai,f,*faclin)) < 0 || qj == 7)
         {
            if (*final != 0)
            {
               if (ipfi < 0)
...
static int determina_salto(int flin)
{
   ...
   for (c=campini; c<=campfin;c++)
   {
      numcam = (v_mpfi(flin)->f_campos[c-1].c_mante.c_actual % 1000);
...
int factor;
{
   ...
   limitel = -((v_mpfirel(flin)->lineas%160)/6);

   if (!l_separador || t_ini == -1 || !poner)
   {   
      if (poner || fi_modo_lineas[flin])
           t_ini = -1;      
      campini = v_mpfi(flin)->f_pantalla[0].p_campoini;
      campfin = v_mpfi(flin)->f_pantalla[0].p_campofin;
      for (c=campini; c<=campfin;c++)
      {
               ...
               v_mpfi(flin)->f_campos[numcam].c_mante.c_visual == 0)
           {
               ...
               if (fi_modo_lineas[flin]) 
               {
                   ini = v_mpfi(flin)->f_campos[numcam].c_mante.c_qp + (factor/100)*fi_modo_lineas[flin];
               }
               else
               {
               ...
               else
               {
                   ...
                   {
                       v_scroll(ini,fin,ll*(IncrementoLineas/100));
                   }          
                   else
                   {
                       if (t_ini == -1)
   ...
   if (fi_modo_lineas[flin]) 
   {
       if (t_ini < 0)
       {
           ...
           t_fin = maxlin*100+mincol+( limitel*fi_modo_lineas[flin] )-1;
       }
       v_scroll(t_ini,t_fin,(ll*(IncrementoLineas/100)*fi_modo_lineas[flin])*100);
   }
   else
   {
       if (l_separador && poner)
       {
          v_scroll(t_ini,t_fin,ll*(IncrementoLineas/100));   
       }
   }
...
int sin_barra;
{
   ...
   if (ipfi == -2 || (ipfi >= 0 && (-((v_mpfirel(f)->lineas % 160) % 6)) != pag_activa[ipfi])) 
   {
      if (modo == DIFERIDO) 
        {
         rr = 0;
      ...
      if (modo != DIFERIDO && modo != VISUALIZA) 
        {
         ...
         activa_pag(f,0); /* aunque no se pinta es como si lo estuviese */
        }
      ...
      if (!strcmp(v_memo(5004)->alfabetica,"SINBARRA"))
      {
          sin_barra = 1;
      }
      else if (!strcmp(v_memo(5004)->alfabetica,"CONBARRA"))
      {
          sin_barra = 0;
      }
      ...
      if (modo && modo != DIFERIDO) {
         ...
         if (!sin_barra && DespachadorExterno)
         {
            int pi,pf;
            long pn;
            if (dame_cuadro_lineas(f,&pi,&pf))
            {
#ifdef RWSERVER
               if (fi_modo_lineas[f])
               {
                   llama_a_despacho_en_lineas(pi,pf,100);
               }
               else
               {
                   llama_a_despacho_en_lineas(pi,pf,0);
               }
#else
#ifdef FGVWIN
               RECT r;
               ...
               if (fi_modo_lineas[f])
               {
                   (*DespachadorExterno)((int)DESPACHO_EN_LINEAS,100,(void *)&r);
               }
               else
               {
         ...
         do {                           /* donde esta a clave ultima de lin*/
            ...
            hacer = entra_linea(f,&linea,&faclin,&ultima,modo,&final,&lleno,lim1,lim2,&baja,saleret,noprim,ipfi,&desorden,&primera,&ant_qj);
            ...
            case 4 :
               ...
               if (!final || faclin > 100)
               {
                  if (primeralinea(f,lim2,0,lim1,primera)) {pito(132);break;}
                  if (anteriorlinea(f,lim2,0,0,lim1))
                  {
               ...
               {
                  if (anteriorlinea(f,lim2,final,0,lim1)) {pito(133);break;}
               }
               i = total_l - 1;
               r = 1;
               while(-1)
         ...
         if (m == 3) {
            ...
            if (m == 2) {
               ...
               if (abre1_rel(f) == 0) {
                  ...
                  ol = fi_clave_lineas[f];
                  n = ( (-(v_mpfirel(f)->lineas))/160 - 2 ); /* campo linea!! */
                  if (!v_mpfi(f)->f_campos[n].c_key) {
                     rw_error(79);
                  }
                  fi_clave_lineas[f] = n * 100 + v_mpfi(f)->f_campos[n].c_key - 1;
...
int faclin;
{
         ...
         int k = prog[p].indice/100 - 1,         
            ...
            oll = fi_clave_lineas[f],
            olc = fi_modo_lineas[f];
         ...
         fi_clave_lineas[f] = campo * 100 + k;
         if (encolumnas)
         {
             fi_modo_lineas[f] = ancho_columna(f,0);
         }
         else
         {
             fi_modo_lineas[f] = 0;
         ...
         p_ejecuta(par[4],tip,faclin,18); /* posicion */
         
         clave_lin_trata(f,(char *)&posicion,4,"");
         strcpy(v_memo(5004)->alfabetica,"");
         p_ejecuta(par[4],tip,faclin,19); /* limite inferior */
         ...
         p_ejecuta(par[5],tip,faclin,20); /* limite superior */
         ...
         fi_clave_lineas[f] = oll;
		 
         fi_modo_lineas[f] = olc; 
