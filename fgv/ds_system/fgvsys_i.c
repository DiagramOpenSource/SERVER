...
int faclin;
{
         ...
         /* printf("NOOP");getchar();*/
         sprintf(tmp,__Texto[62],programa_activo,recur_punto,p,tip,faclin,prog[p].i,prog[p].indice);
         aviso(0,tmp);
         /* rw_error(42); */
         return(-1);
}
   ...
   if (r) {
      ...
      if (f >= 0) {
         ...
         if (!pp)
         {
			...
			{
				nn = 0;
				sprintf(tmp,"%15ld%15ld%15d%15d%15d%15ld%15d",0L,
				   tam_btrv(_pfi),
				   0L,0L,0L,
				   0L,nn);
			}
...
int faclin;
{
   ...
   if (pasa_a_int(v_memo(5007)->numerica) > 0) faclin = pasa_a_int(v_memo(5007)->numerica);
   f = entra_campo(i,o+1,b,ALTA,faclin,0,
      ((v_mpfi(i)->f_campos[o].c_key == 1) ? 1 : 0),0,-1,ALTA);
...
int nfac = pasa_a_int(v_memo(5007)->numerica);
   ...
   if (nfac >= 0)
       ...
       faclin = 0;
   if (entra_datos(prog[p].indice/100,
      v_prog(prog[p].indice%100),faclin) < 0)
...
int faclin;
{
   ...
   {
		if (v_memo(m)->alfabetica[0] == 'E') 
		{
			euro = 1;
			sexo = 1;
			numero = rwatof(v_memo(m)->alfabetica+1);
			decis = decimales(v_memo(m)->alfabetica+1);
		} else if (v_memo(m)->alfabetica[0] == 'M') 
		{
            sexo = 1;
            numero = rwatof(v_memo(m)->alfabetica+1);
			decis = decimales(v_memo(m)->alfabetica+1);
        ...
        else
		{
			...
			decis = decimales(v_memo(m)->alfabetica);
		}
	    free(v_memo(m)->alfabetica);
		resultado[0] = 0;
	    letras(resultado,numero,sexo,euro,decis);
...
int controla_paqueteria(int solo_sys)
{
   ...
   if (puede)
   {
      ...
      for (i = 0;i < nPaq;i++)
      {
         ...
         {
            sprintf(tmp,__Texto[287],Paq[i].nompaq);
            aviso(0,tmp);
            no_instala[0] = 0;
            continue;
         }
		 ...
		 if (!la_marca)
		 {
			 ...
			 {
				la_marca = 2000000000; /* debe obligar a instalar */
			 }
		 }
         info = NULL;
         if (la_marca)
         {
            if (		    
					la_marca == 2000000000 || (la_marca > (Paq[i].control_utime+62) || la_marca < (Paq[i].control_utime-62))
                                             )
            {
                                                            ...
                                                            if (!comprueba_ins_sistema(Paq[i].i.sistema,Paq[i].i.dependesubsistema) && puede == 1)
                                                            {
                                                               ...
                                                               {		
                                                                  ...
                                                                  {                    
                                                                     ...
                                                                     sprintf(tmpxx,__Texto[288],Paq[i].i.descrip,Paq[i].nompaq);
...
static int chinos_esvariables(char *i)
{
   while(*i && *i != '.') i++;
   if (!*i) return(0);   
   i++;
...
static int chinos_bienstring(char *qo,char *qi,int lo)
{   
   int li=strlen(qi);
   int i;
...
int MirameLosChinos(int reg,unsigned char *_databuf,int l_reg,FgvStdTabla *itabla,int reparar,FILE *file_log,FI_DEF *pfi)
{
	int i,len,j,kk;
	int chinos = 0;
	int encampo = 0;
	char tmp[51];
	char buffer[512];
	unsigned char *databuf = _databuf;
	double minimo = (double)-99999999999999999999.9999999;
	double maximo = (double)99999999999999999999.9999999;
	double valor;
	...
	for (i = 0;i < itabla->ncampos;i++)
	{
		...
		if (itabla->Campo[i].tipo == CHARTIPO+48)
		{
			...
			if (pfi && !encampo)
			{
				if (chinos_bienstring(databuf,pfi->f_campos[i].c_minimo,len))
				{
					chinos++;
					encampo = 1;
				}
			...
			*/
			if (encampo)
			{
				if (file_log)
				{
					memcpy(buffer,databuf,len);
					buffer[len] = 0;
					fprintf(file_log,"TIPOS ERRONEOS ALFA: %s, Reg:%d , Campo:%d ,[%s]\r\n",itabla->ident,reg,i,buffer);
				}
			    if (reparar == 7)
				{
					memset(databuf,' ',len);
		...
		{
			encampo = 0;
			if (reparar != 4 || (i != 50 && i != 51) || strcmp(quita_path(itabla->PathDef),"dsgasm13"))
			{
				for (j = 0;j < 8;j++)
				{
			...
			if (encampo)				
			{
				if (file_log)
				{
					memcpy(buffer,databuf,8);
					buffer[8] = 0;
					fprintf(file_log,"TIPOS ERRONEOS FECHA: %s, Reg:%d , Campo:%d ,[%s]\r\n",itabla->ident,reg,i,buffer);
				}
				if (reparar == 7)
				{
		...
		else
		{
			...
			if (pfi)
			{
				if (!chinos_esvariables(pfi->f_campos[i].c_minimo) && !chinos_esvariables(pfi->f_campos[i].c_maximo))
				{
					maximo = rwatof(pfi->f_campos[i].c_maximo);
					minimo = rwatof(pfi->f_campos[i].c_minimo);
				}
				else
				{
					minimo = (double)-99999999999999999999.9999999;
					maximo = (double)99999999999999999999.9999999;
				}
				/*
				maximo = 0.0;
				for (kk = 0;kk <= (itabla->Campo[i].longitud+itabla->Campo[i].decimales+1);kk++)
				{
					maximo *= 10.0;
				}
				minimo = -maximo;
			...
			switch(itabla->Campo[i].tipo)
			{
			case 49:
				valor = (double)ldint(databuf);
				break;
			case 50:
				valor = (double)ldlong(databuf);
				break;
			case 51:
				valor = (double)lddbl(databuf);
				break;
			case 52:
				valor = (double)ldfloat(databuf);
				break;
			}			
			...
			if (valor < minimo || valor > maximo)
			{
				...
				if (file_log)
				{
					fprintf(file_log,"TIPOS ERRONEOS NUMERICOS: %s, Reg:%d , Campo:%d\r\n",itabla->ident,reg,i);
				}
			}
		}
	...
	if (chinos && file_log)
	{
		databuf = _databuf;
		/* volcado */		
		fprintf(file_log,"Datos ficha:");
		for (j = 0;j < itabla->ncampos;j++)
		{
			len = itabla->Campo[j].len;
			memcpy(buffer,databuf,len);
			buffer[len] = 0;
			if (itabla->Campo[j].tipo != 48 && itabla->Campo[j].tipo != 53)
			{
				sprintf(tmp,"%%%d.%dlf",itabla->Campo[j].longitud+itabla->Campo[j].decimales,itabla->Campo[j].decimales);
			}
			if (j)
				fprintf(file_log,",");
			fprintf(file_log,"[%d]'",j);
			switch(itabla->Campo[j].tipo)
			{
			case 48:
				fprintf(file_log,"%s",buffer);
				break;
			case 49:
				fprintf(file_log,tmp,(double)ldint(buffer));
				break;
			case 50:
				fprintf(file_log,tmp,(double)ldlong(buffer));
				break;
			case 51:
				fprintf(file_log,tmp,(double)lddbl(buffer));
				break;
			case 52:
				fprintf(file_log,tmp,(double)ldfloat(buffer));
				break;
			case 53:
				fprintf(file_log,"%s",buffer);
				break;
        ...
        if (fd_paq >= 0)
        {
            ...
            {				
				...
				if (registro[0] && tamactual >= _maximo_dsk_)
				{
					if (file_log)
					{
						fprintf(file_log,"NO PROCESADO POR LIMITACION DE TAMAÑO\r\n");
					}
					continue;
				}
				if (registro[0] && filtrar[0])
				{
					if (strbusca(registro,filtrar) < 0)
					{
						if (file_log)
						{
							fprintf(file_log,"NO PROCESADO POR NO PERTENECER AL FILTRO\r\n");
						}
						continue;
					}
				}
				...
				if (!registro[0] || losprocesados[i] == 1) 
				{
					...
					if (file_log)
					{
						fprintf(file_log,"YA HABIA SIDO PROCESADO\r\n");
					}
					continue;
				}
				...
				else
				{
					...
					{						
						...
						if (fd_diu < 0)
						{
							...
							if (file_log)
							{
								fprintf(file_log,"NO HAY REGISTROS A PROCESAR\r\n");
							}
							continue;
						}
					}					
                ...
                if (lee_defn(registro+52,&fi))
				{
					...
					if (file_log)
					{
						fprintf(file_log,"DEF ILEGIBLE, DESCARTADO\r\n");
					}
					if (version_ciu > 0)
					{
				...
				else
                {
                    ...
                    if (fd < 0)
					{
						...
						if (file_log)
						{
							fprintf(file_log,"NO EXISTE FICHERO DATOS\r\n");
						}
					}
					else
                    {
							...
							if (escopia || rw_base_read_(fd_diu,registros,tam) == tam)
							{
								...
								else
								{
									...
									if (escopia)
									{
										...
										{
											if ((ierr = ixx_lee_datos(fi->f_fd,fi->f_buf,nreg+1)) == 1)
											{
												c = 0;
											...
											if (dtabla)
											{
												if (MirameLosChinos(nreg,(unsigned char *)fi->f_buf,fi->f_lfich,dtabla,0,file_log,NULL))
												{
													hay_errores++;
													hay_errores_file++;
												}
									...
									{									
										...
										while(dato < (registros+tam))
										{
											...
											{                                
												...
												if (dtabla)
												{
													if (MirameLosChinos(reg,(unsigned char *)fi->f_buf,fi->f_lfich,dtabla,0,file_log,NULL))
													{
														hay_errores++;
														hay_errores_file++;
													}
						...
						if (file_log)
						{
							fprintf(file_log,"ANTES:NONE,DESPUES:%ld,",tam_btrv(fi));
							fprintf(file_log,"Incidencias:%d,",nerrores);
							for (j = 0;j < 16;j++)
							{
								fprintf(file_log,"%02x",0);
							}
							fprintf(file_log,",");
							for (j = 0;j < 16;j++)
							{
							...
							fprintf(file_log,",");
							if (hay_errores_file || nerrores > 0)
							{
				...
				for (i = 0;i < ficheros && rr == 0;i++)
				{
					...
					*/
					...
					if (version_ciu > 0)
					{
						...
						if (pos_seek > rw_lseek_(fd_paq,0L,SEEK_CUR))
						{
							...
							else
							{
								...
								if (escopia)
								{
									for (j = 0;j <nreg;j++)
									{
										...
										{
											if (!enlistanegra && MirameLosChinos(0,(unsigned char *)databuf,l_reg,&itabla,0,NULL,NULL))
											{
												if (file_log)
												{
