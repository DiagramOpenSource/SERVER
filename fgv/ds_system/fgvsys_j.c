...
static char *letrascon = NULL;
static char *textocon[2] = {NULL,NULL};

static char tmp_s[2048];
...
static int sql_select_ejecuta(FI_DEF *pfi,FI_DEF *padrepfis[],int nfiltros[],struct grid_filtro *filtros[],struct _sql_origen *origen,struct _sql_relacion *relacion,char *buffers[],struct gridpars *par,int el_punto,int (*out_fun)(),void *param,int nf);
  ...
  memcpy(buf,pfi->f_buf,pfi->f_lfich);
    ...
    if (limi2)
    {
		...
		if (p && !par->padre)
		{
			...
			while(*p1 && *p1 != '}') p1++;
			n = (int)(p1 - p);
			if (n >= sizeof(par->autofiltro))
			{
				n = sizeof(par->autofiltro) - 1;
			}
			memcpy(par->autofiltro,p1,n);
			par->autofiltro[n] = 0;
		}		
		*/
    }
...
static int libera_sqltoscreen(struct _sql_toscreen *sqltoscreen)
{
	...
	if (sqltoscreen->relacion)
	{
		...
		{
			for (j = 0;j < sqltoscreen->relacion[i].nrel;j++)
			{
				if (sqltoscreen->relacion[i].mapa_ordenacion[j])
				{
					free((char *)sqltoscreen->relacion[i].mapa_ordenacion[j]);
				}
			}
			
			if (sqltoscreen->relacion[i].ofd == 1)
			{
				cierra_ind(sqltoscreen->padrepfis[i]);
			}			
		}
...
static int LanZaHiJo(struct gridpars *par,char *titulo,int nfiltros,struct grid_filtro *filtros,int ncolucam,int *colucam,int noborra,int eventohijo,int sql)
{
	...
	else
	{
		...
		/*
		if (par->pfi->f_PathDef[0] == 7 && par->pfi->f_PathDef[1] == 7 && par->pfi->f_PathDef[2] == 0)
		{			
			copia_defn(&pfi,par->pfi);
		}
		else
		{
			flag_leedefn = 0;
    ...
    {		
		...
		else
		{
			int hay_orden = 0;
			/*
			  Se tirara siempre como "complex" 
			int agrupacion = 0;
			int totales = 0;
			*/
			...
			if (hay_orden)
			{
				...
				*/
				...
				for (i = laclave;i < pfi->f_nclaves;i++) 
				{
					free((char *)pfi->f_clave[i].k_campo);
					free((char *)pfi->f_clave[i].k_orden);
				}
				if (pfi->f_nclaves < (laclave+1))
				{
					char *tmpx;
					tmpx = reserva_malloc(sizeof(KEY_DEF) * (laclave+1),3);
				...
				for (i = laclave;i < pfi->f_nclaves;i++)
				{
					pfi->f_clave[i].k_ncampos = hay_orden;
					pfi->f_clave[i].k_fflag = 0;
					pfi->f_clave[i].k_campo = (short *)malloc(sizeof(short) * (hay_orden+1));
					pfi->f_clave[i].k_orden = (short *)malloc(sizeof(short) * (hay_orden+1));
				}				
				/*hay_orden = 1;*/
				for (i = 0;i < nfiltros;i++)
				{
					if ((filtros[i].tipo & 0x10))
					{
						pfi->f_clave[laclave].k_campo[i] = filtros[i].campo+1;
						pfi->f_clave[laclave].k_campo[i+1] = 0;
						if ((filtros[laclave].tipo & 0x20))
						{
							pfi->f_clave[laclave].k_orden[i] = -1;
							pfi->f_clave[laclave].k_orden[i+1] = 0;
						}
						else
						{
							pfi->f_clave[laclave].k_orden[i] = 1;
	...
	if (skipped)
	{
		...
		*/
		...
		{	
			...
			{
				long npos;				
				npos = (posicion-par->offset)+1;				
				while(npos < par->registros)
				{
				   if (grid_lee_por_posicion(par,key,npos+par->offset,dat_reg,modo,flags,_from,_to) < 0)
				   {
			   		   if (ixxstatus == IXXECORRUP)
							return -1;
					   break;
				...
				lee_por_posicion(par->pfi,key,posicion,dat_reg,modo,flags);
			}
			*/
		}
		else
	...
	if (*s == 26)
	{
		...
		*essql = 1;
		...
		for (ii = 0;ii < nf;ii++)
		{
			...
			{
				j = -1;/*ind_rel(quita_path(files[ii]));*/
				if (j >= 0)
				{
					padrepfis[ii] = v_mpfi(j);					
				}
				else
				{
		...
		{
			int kk,xx;
			/* la clave primaria al fichero raiz la incluiremos siempre */

			for (kk = 0;kk < padrepfis[0]->f_clave[0].k_ncampos;kk++)
			{
			   campo = padrepfis[0]->f_clave[0].k_campo[kk] - 1;
			   for (xx = 0;xx < n;xx++)
			   {
				   if (origen[xx].ipadre == 0 && origen[xx].icampo == campo)
					   break;
			   }
			   if (xx >= n)
			   {
				   inserta_campo(pfi,n,padrepfis[0]->f_campos[campo].c_lista,					
...
double vcf;
								...
								{	
									...
									{									
										...
										else if (modo_completo)
										{
											...
											/*
											{ // OJOCESC 
												sprintf(par->cache[j].buf,"X[%d][%d]",par->offset+from+i,ixxstatus);													
											}
											*/
    ...
    else if (operacion == 1)
    {
		...
		if (par->padre && par->padre->id_grid == -1)
		{
            int ofs;
            int lll;
			...
			/*
				...
				for (_campo = 0;_campo < par->padre->ncampos;_campo++)
				{
					campo = par->padre->colcampos[_campo];
					if (campo < 0) break;
					i = dInfoCampo(par->padre->InfoCampos,campo ,0)->c_maxqc;
					memcpy(tmpxx,bloque,i);
					tmpxx[i] = 0;
					st_campo(dInfoCampo(par->padre->InfoCampos,campo ,0)->Id,par->padre->pfi,tmpxx);
					bloque += i;
				}
			...
			*/
			...
			if (par->sqltoscreen && par->sqltoscreen->origen)
			{
				...
				{
					if (origen[i].ipadre == 0)
					{
						cc = origen[i].icampo;
						if (cc >= 0 && cc < par->padre->pfi->f_ncampos && par->padre->pfi->f_campos[cc].c_long == par->pfi->f_campos[i].c_long)
						{
							memcpy(par->padre->pfi->f_buf+par->padre->pfi->f_campos[cc].c_offset,par->pfi->f_buf+par->pfi->f_campos[i].c_offset,par->pfi->f_campos[i].c_long);
						}
					}
				}
			}
    ...
    {       
		...
		if (par->mapa_de_ordenacion)
		{
			...
			for (i = 0;i < par->nmapa_de_ordenacion;i++)
			{
				...
				{
					par->reg = RegSegunMapaAsignacion(par,i - par->offset)+1;					
					par->regsel = registro;
					isrecnum = registro;
					par->regcur = par->reg;
    ...
    else if (operacion == 5) /* rellenar hijo */
    {
		...
		if (par->sqltoscreen)
		{
			...
			sql_select_ejecuta(par->pfi,par->sqltoscreen->padrepfis,par->sqltoscreen->nfiltros,par->sqltoscreen->filtros,par->sqltoscreen->origen,par->sqltoscreen->relacion,buffers,par,el_punto,NULL,NULL,nf);
...
sal_rellena_hijo:
				...
				*/
				...
				else
				{
					...
					if (lee_defn(bloque,&padrepfis[i]) >= 0)
					{
						...
						if (j >= 0)
						{
							strcpy(padrepfis[i]->f_fichero,v_mpfi(j)->f_fichero);
							j = -1;
						}
						else
						{
    ...
    else if (operacion == 14) /* Grabar Filtro */
	{
		char tmpx[512];
		...
		if (!strcmp(bloque,"_INDICE_"))
		{
			...
			{			
				    ...
				    rw_lseek_(file,0L,SEEK_SET);
					bloque3 = malloc(l+2+strlen(bloque));
    ...
    if (operacion == 18) /* aviso de perdida de foco */
	{
        if (par->hook)
        {            
	...
	if (operacion == 20)
	{
		...
		*pndef = 0;
		...
		if (1 /*par->ipfi != 48*/)
		{
			...
			{
				prel = -1;/*ind_rel(quita_path(bloque));*/
				if (prel >= 0)
				{
				...
				else
				{
					...
					{						
						if (prel >= 0)
						{
							strcpy(padre->f_fichero,v_mpfi(prel)->f_fichero);
							prel = -1;
						}
						else
						{
			...
			if (padre)
			{
				for (i = 0;i < padre->f_nrelaci;i++)
				{
					...
					cargado = 0;
					rel = -1;/*ind_rel(quita_path(padre->f_relaci[i].r_fichero));*/
					if (rel < 0)
					{
						/* cargarlo --- lee_defn */				
						...
						if (lee_defn(tmp,&fi) >= 0)
						{
							...
							{
								strcpy(fi->f_fichero,v_mpfi(rel)->f_fichero);
								rel = -1;
							}
							cargado = 1;
						}
					...
					if (fi)
					{
						...
						else
						{
							...
							{
								lineas = 2; /* relacion erronea, error en la estructura defs */
							}
						}
						if (lineas < 2)
						{
	...
	{
		FI_DEF *fi = NULL;
		int rel = -1; /*ind_rel(quita_path(bloque));*/
		if (rel < 0)
		{
			rel = ind_rel(quita_path(bloque));
			lee_defn(bloque,&fi);
		}
		else
		{
...
static int RellenaInfoClave(struct Info_Entra_Clave  *clave,FI_DEF *pfi,int c,int max_campos)
{
    ...
    for (i = 0;i < pfi->f_clave[c].k_ncampos;i++)
    {
        ...
        {
            if ( (pfi->f_campos[j].c_mante.c_actual%1000) == k )
            {
...
{    
	...
	if (Pseudo10)
	{
		if (pfi->f_campos[c].c_mante.c_visual || !hay_dat_permiso(pfi,10000+c))
		{
			campo->c_flags |= 1;
		}	
		...
		if (isslowconection && !pfi->f_campos[c].c_key)
		{
			campo->c_flags |= 1;
		}
	}		
...
int RellenaInfoDef(struct Info_Entra_Campos *info,FI_DEF *pfi,int flag,int key,int *len_reg,int *micampo)
{
    struct Info_Entra_Campo  *campo;
    ...
    for (fase = 0;fase < 2;fase++)
    {
        ...
        {			
            ...
            if (!(flag & 1))
            {
                ...
                pfi->f_campogids = (short *)malloc(pfi->f_ncampos * sizeof(short));
                memset((char *)pfi->f_campogids,0xff,pfi->f_ncampos * sizeof(short));
				pfi->f_cactual = (int *)malloc(pfi->f_ncampos * sizeof(int));
				for (i = 0;i < pfi->f_ncampos;i++)
				{
					pfi->f_cactual[i] = pfi->f_campos[i].c_mante.c_actual;
				}
            }			
        }
        ...
        if (!flag) /* orden de "pantalla"*/
        {
           for (npan = 0;npan < pfi->f_npanta;npan++)
           {
               ...
               {
                    i = pfi->f_campos[numcam-1].c_mante.c_actual % 1000;
                    if (i < 0 || i >= pfi->f_ncampos) 
						continue;
                    if (!Pseudo10)
					{
					   if (pfi->f_campos[i].c_mante.c_visual) 
						   continue;
					   if (isslowconection && !pfi->f_campos[i].c_key)
					   {
							continue;
					   }
					}

					if (conta_campos > pfi->f_ncampos)
					{
						/*aviso(0,"PX 0");*/
						continue;
					}
			...
			if (Pseudo10)
			{
				for (i = 0;i < pfi->f_ncampos;i++)
				{
					...
					{
						if ( (pfi->f_campos[numcam].c_mante.c_actual%1000) == i )
						{
							break;
						}
					}
        ...
        {         /* orden de "natural"*/
            for (i = 0;i < pfi->f_ncampos;i++)
            {
                ...
                if (!Pseudo10)
				{				   
				   if (isslowconection && (!pfi->f_campos[i].c_key || pfi->f_campos[i].c_key > 2))
				   {
						continue;
				   }
				}
			...
			for (i = 0;i < pl[4];i++,buffer += 31)
			{
				if (*buffer == '|')
				{
				  for (j = 0;j < np_ex;j++)
				  {
					 if (!strcmp(buffer+1,p_ex[j].nombre)) 
					 {
						p_ejecuta(p_ex[j].indice,O_NOR,0,29);
						strcpy(pbuf+pl[5+i],m_ex[4].mv.valc);
						break;
					 }
				  }
				...
				{	
					...
					if (((int)RetroPar[el_punto]) == 0xfffffff)
					{
						/* auto rpt!! */
						auto_rpt = 1;
					}
					if (!strcmp(buffer+1,"dirfic"))
					{
						if (auto_rpt)
						{
							strcpy(pbuf+pl[5+i],mi_remoto_dirbass());
							strcat(pbuf+pl[5+i],"crystal/");
						}
						else
						{
							strcpy(pbuf+pl[5+i],dirfic);						
					...
					{						
						if (auto_rpt)
						{
							strcpy(pbuf+pl[5+i],mi_remoto_dirbass());
							strcat(pbuf+pl[5+i],"crystal/");
							strcat(pbuf+pl[5+i],quita_path(nom_inf));
							strcat(pbuf+pl[5+i],".rpt");
						}
						else
						{
							strcpy(pbuf+pl[5+i],nom_inf);
					...
					{
#ifdef RWSERVER
						if (!auto_rpt && en_conexion_externa)
							strcpy(pbuf+pl[5+i],"SI");						
						else
#endif
							strcpy(pbuf+pl[5+i],"NO");
...
int CargaVInterface(char *nome,int visual)
{
	...
	if (!strncmp(nome,"DSITCP:",7))
	{
		...
		DSTCP_VInterface(atoi(nome+7),visual);
...
{	
  ...
  */
  ...
  gmodo |= 0x1000000; /* indicar que somos un 9 */
  ...
  tam = RellenaInfoDef(NULL,pfi,(gmodo & 64),encl,&lreg,NULL);  
  ...
  tam += RellenaInfoDef(InfoCampos,pfi,(gmodo & 64),encl,NULL,NULL);
...
int BorraHijoGrid(int mi_punto,int alpadre)
{
	...
	if (par->sqltoscreen)
	{
		libera_sqltoscreen(par->sqltoscreen);
	}
	par->sqltoscreen = NULL;
...
struct Info_Entra_Campo  *campo;
  ...
  if (pfi->f_cactual)
  {
	  for (i = 0;i < pfi->f_ncampos;i++)
	  {
  		  pfi->f_campos[i].c_mante.c_actual = pfi->f_cactual[i];
	  }
      free((char *)pfi->f_cactual);
      pfi->f_cactual = NULL;
  }
...
static char **elfiltro = NULL;
  ...
  if (!esremota && remotear && DespachadorExterno)
  {
	  char tmp[512];	  
	  int pi = 0;
	  int pf = 0;
	  ...
	  {  
		 int nl;
		 pi     = v_mpfi(pipfi)->f_campos[campo].c_mante.c_qp + 100;     
		 pf     = v_mpfi(pipfi)->f_pantalla[numpan].p_posref + (v_mpfi(pipfi)->f_pantalla[numpan].p_posfin-101);
		 nl     = (pf%100) - (pi%100);
		 if (nl < 35)
		 {
			 if ((pi%100) < (36-nl))
	  ...
	  for (errc = 0; errc < 2; errc++)
	  {
		  sprintf(tmp,"%-51s %-51s %05d %05d %05d %ld",pfi->f_PathDef+strlen(rw_directorio("")),pfi->f_fichero+strlen(rw_directorio("")),encl,pi,pf,updatecount);
		  /*consulta_local(tmp);*/
		  (*DespachadorExterno)((int)DESPACHO_CONSULTA_REMOTA,(int)0,tmp);
		  if (tmp[0] == 1 || tmp[0] == 2)
		  {
#ifdef RWSERVER
			if (en_conexion_externa)
			{
			  int sal = 0;      
			  strcpy(tmp,"dch/");
			  strcat(tmp,pfi->f_PathDef+strlen(rw_directorio("")));
			  strcat(tmp,".mas");
			  sal = rw_envia_fichero(tmp,5);

			  strcpy(tmp,"dch/");
			  strcat(tmp,pfi->f_fichero+strlen(rw_directorio("")));
			  strcat(tmp,".ddx");
			  sal = rw_envia_fichero(tmp,5);
  ...
  if (ipfi > -1)
  {
	  ...
	  if (par.hook < 1)
	  {
		  ...
		  for (i = 0;i < np_ex;i++)
		  {
			 if (strcmp("EVENTOGRID",p_ex[i].nombre) == 0) 
			 {
				par.hook = p_ex[i].indice;
				break;
			 }
		  }		  
	  }
  ...
  */
  ...
  {  
	 ...
	 par.gmodo = 4+8+16/*+64*/;
	 par._sid  = 0;/*default_sid;/* pipfi+1+(numpan * 1000);*/
...
/*#else*/
int consul_clave_old(ipfi,encl,numpan,limi1,limi2,filtro)
int ipfi,encl,numpan,limi1,limi2,filtro;
{
   int oc = ind_clave_activa(v_mpfi(ipfi)) - 1;
   VENT *w;
   int len,tecla;
   int nc,campo,nk,repinta,offqp;
   int j,i,siotrocampo,gi,gj;
   int pi,pf,extra = 0;
   int pos,mientras,posa;
   int salida = 0;
   int tmp_pos = 0;
   char tmp[300];
   char *buf_prime = malloc(v_mpfi(ipfi)->f_lfich + 1);
   ...
   encl--;
   otra_clave = encl;
   if (!otra_clave && v_mpfi(ipfi)->f_nclaves > 1)
      otra_clave = 1;
   ...
   if (limi2)
   {
      limite_2 = malloc(v_mpfi(ipfi)->f_lfich + 1);
      p_ejecuta(limi2,O_NOR,0,30);
      memcpy(limite_2,v_mpfi(ipfi)->f_buf,v_mpfi(ipfi)->f_lfich);
   }
   
   
   if (limi1)
   {
      limite_1 = malloc(v_mpfi(ipfi)->f_lfich + 1);
      p_ejecuta(limi1,O_NOR,0,31);
      memcpy(limite_1,v_mpfi(ipfi)->f_buf,v_mpfi(ipfi)->f_lfich);
   }
   
   
   if (leer_reg(ipfi,ISGTEQ,0,0))
    {
      if (leer_reg(ipfi,ISLAST,0,0))
						{
         aviso(0,__Texto[63]);
         salida = -1;
         goto salir;
  ...
  if (!hay_dat_permiso(v_mpfi(ipfi),-2))
  {
   ...
   if (v_mpfi(ipfi)->f_nclaves > 1)
      nk = otra_clave + 1;
   else
      nk = 1;
   
   if (nk == 1 && v_mpfi(ipfi)->f_ncampos > 1 && v_mpfi(ipfi)->f_campos[1].c_key != 1)
      siotrocampo = 1;
   else
      siotrocampo = 0;
   ...
   for (j = 0;j < nk;j++) {
      ...
      if (j /*(numpan == -1 || j)*/ && j != otra_clave) continue;
      
      nc = v_mpfi(ipfi)->f_clave[j].k_ncampos;
      offqp = len;
      for (i = 0;i < nc;i++) 
						{
         campo = v_mpfi(ipfi)->f_clave[j].k_campo[i];
         gi = v_mpfi(ipfi)->f_campos[campo-1].c_mante.c_qpl;
         gj = long_campo(v_mpfi(ipfi),(campo - 1));
         if ((v_mpfi(ipfi)->f_campos[campo-1].c_tipo != CHARTIPO && v_mpfi(ipfi)->f_campos[campo-1].c_tipo != FECHATIPO) || gi < 1 || gi > gj)
            gi = gj;
         len += (gi+1);
						}
   }
   ...
   if (len > 78) {
      /* se deberia reducir a una sola clave */
      /* ... */
   }
   
   len++;
   sprintf(tmp,__Texto[289],v_mpfi(ipfi)->f_titulo);
   if (strlen(tmp) >= (len-3))
      len = strlen(tmp) + 4;
   
   
   pi = 700 + (80-len)/2;
   pf = 2200 + (80-len)/2 + len - 1;
   w = w_ini(pi-301,pf+101);
   w_salva(w);
   ...
   Nor();
   zonablan(pi-99,pf-101);
   Ai();
   bi_cuadro(pi-200,pf,1);
   bi_cuadro(pi+1,(pi/100*100)+(pf%100)-1,1);
   Bi();
   
   Ar();
   zonablan(pi-99,(pi/100*100)+(pf%100)-101);
   pprint(pi-99+(len-strlen(tmp))/2,tmp);
   Cr();
   mensaje_en_cabecera("%s",__Texto[198],2);
   
   
   if (DespachadorExterno)
   { 
#ifdef RWSERVER
      llama_a_despacho_en_lineas(pi,pf);
#else
#ifdef FGVWIN
						{
                     RECT r;
                     ConvierteTxtARect(pi,pf,&r);
                     (*DespachadorExterno)((int)DESPACHO_EN_LINEAS,0,(void *)&r);
                  }
#endif
#endif
   ...
   while(mientras) 
   {
      pos = pi + 101;
      tmp_pos = pos;
      if (leer_reg(ipfi,ISCURR,0,0)) {
         /*printf("\n\r error de registro corriente en consul_clave 0\n\r");*/
         rw_error(1020);
         /* getchar(); */
						}
      memcpy(buf_prime,v_mpfi(ipfi)->f_buf,v_mpfi(ipfi)->f_lfich);
      anula_mapa_raton(-1);
      total_en_pantalla = 0;
      while(pos < (pf-100)) {
         
         /*#ifdef FGVIXX*/
         tabla_reg[total_en_pantalla] = isrecnum;
         total_en_pantalla++;
         /*#endif*/
         cursor(pos);
         for (j = 0;j < nk;j++)
         {
            ...
            for (i = 0;i < nc;i++)
            {
               ...
               if ((v_mpfi(ipfi)->f_campos[campo-1].c_tipo != CHARTIPO && v_mpfi(ipfi)->f_campos[campo-1].c_tipo != FECHATIPO) || gi < 1 || gi > gj)
                  gi = gj;
               pon_mapa_raton(pos,pos+gi-1,pos,2);
               if (j < (nk-1) || i < (nc-1))
                  v_print("³");
            }
         }
         if (siotrocampo) { /* de  momento a pelo segundo campo */
            v_print("³");
            printcam(v_mpfi(ipfi),2);
         }
         tmp_pos = pos;
         pos += 100;
         if (pos < (pf-100))
         {
            if (consulta_lee_next(ipfi)) 
            {
               consulta_lee_last(ipfi);
               break;
            }
            if (compara_la_clave(encl+1,ipfi,limite_2) > 0)
            {
               consulta_lee_prev(ipfi);
               break;
            }
         }
      }
      ...
      while(pos < (pf-100)) {
         ...
         /* zonablan(pos,pos+len-3); */
         pos += 100;
						}
      
      /* buscar registro corriente por rebobinado */
      while(tmp_pos != posa)
      {
         kr = posa/100 - resta;
         pon_registro_actual(v_mpfi(ipfi),tabla_reg[kr]);
      ...
      while(!repinta) 
      {
         ...
         if (siotrocampo) 
         { /* de  momento a pelo segundo campo */
            v_print("³");
            printcam(v_mpfi(ipfi),2);
         }
         Cr();
         ...
         case CONSULTAR:
         case LISTADO:
            memcpy(v_mpfi(ipfi)->f_buf,buf_prime,v_mpfi(ipfi)->f_lfich);
            /* pedir que entre una nueva clave inicial */
            do {
               haycontenido = 0;
               
               fgv_fin_contenido(v_mpfi(ipfi));
               
               nc = v_mpfi(ipfi)->f_clave[encl].k_ncampos;
               if (encl)
                  j = offqp;
               else
                  j = 0;
               ...
               for (i = 0;i < nc;i++) 
               {
                  campo = v_mpfi(ipfi)->f_clave[encl].k_campo[i] - 1;
                  ld_campo(campo,v_mpfi(ipfi),tmp);  /* valor defecto guardado en el buffer */
                  /* tmp[0] = 0; */
                  
                  if (v_mpfi(ipfi)->f_campos[campo].c_qc == -1)
                  {
                     desarregla_fecha(tmp,v_mpfi(ipfi)->f_campos[campo].c_mante.c_qpl);
                  }
                  entra(pi+101 /* posa */ +j,v_mpfi(ipfi)->f_campos[campo].c_qc,
                     v_mpfi(ipfi)->f_campos[campo].c_minimo,
                     v_mpfi(ipfi)->f_campos[campo].c_maximo,
                     tmp,v_mpfi(ipfi)->f_campos[campo].c_mante.c_qpl*10,tmp);
                  if (v_mpfi(ipfi)->f_campos[campo].c_qc == -1)
                  {
                     arregla_fecha(tmp,v_mpfi(ipfi)->f_campos[campo].c_mante.c_qpl);
                  }		
                  /* prueba a pelo */
                  quitab(tmp);
                  
                  if (v_mpfi(ipfi)->f_campos[campo].c_qc > 19 && strlen(tmp) > 2)
                  {
                     pp = tmp;
                     conte = NULL;
                     while(*pp)
                  ...
                  st_campo(campo,v_mpfi(ipfi),tmp);  /* pone dato en el registro (buffer) */
                  
                  gi = v_mpfi(ipfi)->f_campos[campo].c_mante.c_qpl;
                  gj = long_campo(v_mpfi(ipfi),campo);
                  if ((v_mpfi(ipfi)->f_campos[campo].c_tipo != CHARTIPO && v_mpfi(ipfi)->f_campos[campo].c_tipo != FECHATIPO) || gi < 1 || gi > gj)
                     gi = gj;
                  j += (gi+1);		
               }
#ifdef FGVIXX
            ...
            if (!haycontenido)
            {
               if (leer_reg(ipfi,ISGTEQ,0,0)) 
			   {
                  leer_reg(ipfi,ISLAST,0,0);
               }
		       if (!hay_dat_permiso(v_mpfi(ipfi),-2))
			   {
				  consulta_lee_next(ipfi);
			   }
            }
         ...
         case SOLICITA_INICIO:
            if (limite_1)
            {
               memcpy(v_mpfi(ipfi)->f_buf,limite_1,v_mpfi(ipfi)->f_lfich);
               leer_reg(ipfi,ISGTEQ,0,0);
		       if (!hay_dat_permiso(v_mpfi(ipfi),-2))
			   {
				  consulta_lee_next(ipfi);
			   }
         ...
         case SOLICITA_FINAL:
            if (limite_2)
            {
               memcpy(v_mpfi(ipfi)->f_buf,limite_2,v_mpfi(ipfi)->f_lfich);
               leer_reg(ipfi,ISLOWEQ,0,0);
		       if (!hay_dat_permiso(v_mpfi(ipfi),-2))
			   {
				  consulta_lee_prev(ipfi);
			   }
               ...
               while(posa < (pf-200))
               {
                  if (consulta_lee_prev(ipfi)) break;
                  posa += 100;
               }          
               repinta = 1;
               break;
         ...
         case SOLICITA_SCROLLUP:
            ...
            if (repinta)
            {
               ...
               {
                  if (compara_la_clave(encl+1,ipfi,limite_1) < 0)
                  {
                     pon_registro_actual(v_mpfi(ipfi),tabla_reg[kr]);
                     apunta_ind(v_mpfi(ipfi),0,ISEQUAL,0);
                     apunta_ind(v_mpfi(ipfi),encl+1,ISCURR,0);
         ...
         case SOLICITA_RETROCESO:
            ...
            pon_registro_actual(v_mpfi(ipfi),tabla_reg[0]);
            apunta_ind(v_mpfi(ipfi),0,ISEQUAL,0);
            apunta_ind(v_mpfi(ipfi),encl+1,ISCURR,0);
            for (tmp_pos = pi + 101;tmp_pos < (pf-100);tmp_pos += 100)
            {
               if (consulta_lee_prev(ipfi) || compara_la_clave(encl+1,ipfi,limite_1) < 0)
               {
                  if (limite_1)
                  {
                     memcpy(v_mpfi(ipfi)->f_buf,limite_1,v_mpfi(ipfi)->f_lfich);
         ...
         case SOLICITA_AVANCE:
            pon_registro_actual(v_mpfi(ipfi),tabla_reg[total_en_pantalla-1]);
            apunta_ind(v_mpfi(ipfi),0,ISEQUAL,0);
            apunta_ind(v_mpfi(ipfi),encl+1,ISCURR,0);
            repinta = 1;
            if (consulta_lee_next(ipfi) || compara_la_clave(encl+1,ipfi,limite_2) > 0)
            {
               kr = posa/100 - resta;
               pon_registro_actual(v_mpfi(ipfi),tabla_reg[kr]);
               apunta_ind(v_mpfi(ipfi),0,ISEQUAL,0);
  ...
  if (DespachadorExterno)
  {
     (*DespachadorExterno)((int)DESPACHO_EN_LINEAS,1,NULL);
  }
...
char *ffiltro;
{
    ...
    if (usar_grid == 777 && !consulta_vieja)
    {
        return(consul_clave_grid(NULL,ipfi,encl,numpan,limi1,limi2,filtro,pipfi,campo,ffiltro));
    }
    else	
    {
        return(consul_clave_old(ipfi,encl,numpan,limi1,limi2,filtro));
    }
}
...
char *o;
{
   ...
   if (en_conexion_externa && remoto_es_sistema_grafico)
   {
      Ai();
   }
   else
   ...
   for (i = 9;i < 16;i++)
      pprint(i*100+19,buf);
   pprint(921,__Texto[66]);
   pprint(933,v_mpfi(f)->f_titulo);
   pprint(1021,__Texto[67]);
   pprint(1033,o);
   pprint(1321,__Texto[68]);
   if (iserrno == ELOCKED)
      sprintf(buf,__Texto[69]);
   else
      if (iserrno == ENOREC)
         sprintf(buf,__Texto[70]);
      else
         if (iserrno == EKEXISTS)
            sprintf(buf,__Texto[71]);
         else
            if (iserrno == EENDFILE)
               sprintf(buf,__Texto[72]);	
            else
            {
               if (fecha_protegida)
               {
                  if (iserrno == 999)
					  sprintf(buf,__Texto[73]);
                  else
                     sprintf(buf,__Texto[74],iserrno);
               }
               else
               {
                  if (iserrno == 999)
                     sprintf(buf,__Texto[75]);		   
                  else
         ...
         if (abre1_rel(ipfi) == 0) {
            ...
            if (leida == 0) {
               if (m == 0) {
                  ...
                  operaerror = 0;
                  p_ejecuta(m%100,O_NOR,faclin,32);
...
int m;
{
   ...
   while (-1) 
   {
	   if (Pseudo10 && DespachadorExterno)
	   {
      ...
      {         
         ...
         if (!(info->build_numero))
		 {
			...
			else
			{
				...
				Paq[i].flag = -1; /* instalado pero borrado el tgz */
			}
		 }
         else
            Paq[i].control_utime = info->build_numero;         
...
int BrowseData(int ipfi,int k,int flags,int pini,int pfin,int limi1,int limi2,int evento,char *ffiltro)
{
    struct gridpars par;
    int punto;
    int tecla;
      ...
      {          
		  if ((par.gmodo&1))
		  {
			  if (!par.regcur || par.regcur > par.registros)
			  {
				  ...
				  if (grabar_reg(ipfi,3,20) < 0) {pito(138);continue;}
				  if (par.regcur > par.registros)
					  alta = 2;
				  else
					  alta = 1;
  ...
  if (rr == 0)
  {      
      pon_registro_actual(par.pfi ,resultado);
      apunta_ind(par.pfi ,0,ISEQUAL,0);
      lee_reg(par.pfi ,ISCURR);
      apunta_ind(par.pfi ,k+1,ISCURR,0);
  }
...
int fgvsimplelineal(int p,int tip,int faclin)
{
    ...
    int flags = (int)v_memo(prog[p++].indice)->numerica;
       ...
       evento = 0;
   {
    ...
    sprintf(v_memo(5004)->alfabetica,"%d",BrowseData(ipfi,k,flags,pini,pfin,limi1,limi2,evento,ffiltro));
    n_np = 7;
    return(0);
}
...
static char *sql_recoge_sentencia(char *comando)
{
	...
	if (*comando == '@') /* file */
	{
		int f = rw_open_(comando+1,O_RDONLY | O_BINARY);
		if (f >= 0)
		{
...
static int acumula_filtro(struct _sql_output_ *salida)
{
	...
	if (salida->agrupacion || salida->totales)
	{
		memcpy(buf,pfi->f_buf,pfi->f_lfich);
	}
...
static void sql_procesa_select(char *salida,char *comando,FI_DEF **_fires,int (*out_fun)(),void *param)
{
	...
	char *p1,*p2,*p3,cc;
	char tmp[512];
	struct _sql_campos campos;
	struct _sql_from from;
	struct _sql_into into;
	struct _sql_wheres wheres;
	struct _sql_orderbys orderbys;
	struct _sql_origen *origen;
	struct _sql_relacion *relacion;
	int condicion;
	int sin_salida = 0;
	...
	if (!memcmp(sentencia,"ORDER BY ",9))
	{
		int nc,no;
		sentencia += 9;
			...
			if (pfi)
			{
				...
				if (sin_salida || crea_ind(pfi) >= 0)
				{
...

					{
						...
						sprintf(salida,"0 Seleccionados:%d",sql_select_ejecuta(pfi,padrepfis,nfiltros,filtros,origen,relacion,buffers,NULL,0,out_fun,param,from.nfrom));
					...
					for (i = 0;i < from.nfrom;i++)
					{
						for (j = 0;j < relacion[i].nrel;j++)
						{
							if (relacion[i].mapa_ordenacion[j])
							{
								free((char *)relacion[i].mapa_ordenacion[j]);
							}
						}
						if (relacion[i].ofd == 1)
						{
...
static int abre_chn(char *fdatos,int file_chinos)
{
	char tmp[512];
	...
	if (file_chinos < 0)
	{
		strcpy(tmp,fdatos);
		strcat(tmp,".chn");
		file_chinos = rw_open_(tmp,O_RDWR | O_BINARY | O_CREAT | O_TRUNC,S_IREAD | S_IWRITE);
	}
	return file_chinos;
}

int repara_fichero(char *def,char *fdatos,char *modalidad)
{
	FI_DEF *fi = NULL;
	FI_DEF *pfi = NULL;
	FI_DEF *ppfi = NULL;
	...
	if (!strcmp(modalidad,"CHINOS"))
	{
		modo = 1;
		modochinos = 0;
	}
	else if (!strcmp(modalidad,"?CHINOS"))
	{
		modo = 2;
		modochinos = 0;
	}
	else if (!strcmp(modalidad,"REVISACHINOS"))
	{
		modo = 3;
		modochinos = 0;
	}	
	else if (!strcmp(modalidad,"BORRACHINOS"))
	{
		modo = 4;
		modochinos = 4;
	}
	...
	if (modo != 3)
	{
		strcpy(tmp,fdatos);
		strcat(tmp,".chn");
		file_chinos = rw_open_(tmp,O_RDONLY | O_BINARY);
		if (file_chinos >= 0)
		{
			rw_close_(file_chinos);
			file_chinos  = -1;
			if (confirmar(0,"Ya existe un fichero .chn, Continuar? ","N") < 0)
			{		
				return -2;
			}
		}
    ...
    zonablan(610,1970);
    cuadro(610,1970);
    sprintf(tmp,"Def..: %-48.48s",def);			
    pprint(812,tmp);
    sprintf(tmp,"Datos: %-48.48s",fdatos);
    pprint(912,tmp);
	...
	if (!lee_defn(def,&fi))
	{
		...
		strcpy(fi->f_fichero,fdatos);
		fd = abre_directo(fi,fdatos);
		if (fd >= 0)
		{
			fi->f_fd = fd;
			...
			if (dtabla)
			{
				...
				while(-1)
				{
					...
					if ((kki = lee_directo(fd,nreg,fi,fi->f_buf)) < 0)
					{						
						break;
					}
					if (!kki && MirameLosChinos(nreg,(unsigned char *)fi->f_buf,fi->f_lfich,dtabla,modochinos,file_log,ppfi))
					{
						if (file_chinos < 0 && (modo == 1 || modo == 4))
						{
							file_chinos = abre_chn(fdatos,file_chinos);
						}
						if (file_chinos >= 0)
						{
							rw_write_(file_chinos,fi->f_buf,fi->f_lfich);
							tmp[0] = 10;
							rw_write_(file_chinos,tmp,1);
						}

						if (!hay_errores && modo == 2)
						{
							pfi = NULL;
							if (lee_defn(def,&pfi))
							{
							...
							{
								free(pfi->f_titulo);
								pfi->f_titulo = strmalloc(200);
								strcpy(pfi->f_titulo,"Reparacion de: ");
								strcat(pfi->f_titulo,fi->f_titulo);								
								for (i = 0;i < pfi->f_ncampos;i++)
								{
									pfi->f_campos[i].c_key = 0;
								}
								k = pfi->f_lfich;								
                                inserta_campo(pfi,pfi->f_ncampos,"I_Reg","I_Reg",0,"0","999999999","0");
								pdouble = pfi->f_buf + k;
								
								while(pfi->f_nclaves > 1)
								{
									pfi->f_nclaves--;
									free((char *)pfi->f_clave[pfi->f_nclaves].k_campo);
									free((char *)pfi->f_clave[pfi->f_nclaves].k_orden);
								}				
								
								pfi->f_nclaves = 1;
								pfi->f_clave[0].k_ncampos = 1;
								pfi->f_clave[0].k_campo[0] = pfi->f_ncampos;
								pfi->f_clave[0].k_orden[0] = 1;
								pfi->f_campos[pfi->f_ncampos-1].c_key = 1;
						...
						if (pfi)
						{
							memcpy(pfi->f_buf,fi->f_buf,fi->f_lfich);
							stdbl((double)nreg,pdouble);
							graba_nuevo(pfi);
						}
						if (modo == 1)
						{							
							MirameLosChinos(0,(unsigned char *)fi->f_buf,fi->f_lfich,dtabla,7,NULL,ppfi);
							graba_directo(fd,nreg,fi,fi->f_buf);
							sprintf(tmp,"Reparados: %6d",hay_errores);
						}
						else if (modo == 4)
						{
							borra_directo(fd,nreg,fi);
							sprintf(tmp,"Borrados: %6d",hay_errores);
						}
						else
						{							
							sprintf(tmp,"Localizados: %6d",hay_errores);
						}						
						pprint(1312,tmp);
					}					
				}				
			...
			if (hay_errores > 0)
			{
				...
				if (pfi)
				{
					while(-1)
					{
						i = consul_clave_grid(pfi,0,1,0,0,0,0,-1,-1,NULL);
						if (i < 0)
						{
						    if (confirmar(0,"Confirmar la REPARACION (S/N) ","N") < 0)
						       break;


							file_chinos = abre_chn(fdatos,file_chinos);
							i = lee_reg(pfi,ISFIRST);
							while(i >= 0)
							{
								nreg = (long)lddbl(pdouble);
								lee_directo(fd,nreg,fi,fi->f_buf);
								if (file_chinos >= 0)
								{
									rw_write_(file_chinos,fi->f_buf,fi->f_lfich);
									tmp[0] = 10;
								...
								MirameLosChinos(0,(unsigned char *)fi->f_buf,fi->f_lfich,dtabla,7,NULL,ppfi);
								graba_directo(fd,nreg,fi,fi->f_buf);
								sprintf(tmp,"Reparado Registro: %6ld",nreg);
								pprint(1312,tmp);
								i = lee_reg(pfi,ISNEXT);
							}							
							break;
						}
						if (confirmar(0,"Desea BORRARLO? (S/N) ","N") >= 0)
						{
							file_chinos = abre_chn(fdatos,file_chinos);
							nreg = (long)lddbl(pdouble);							
							if (file_chinos >= 0)
						...
						if (confirmar(0,"Desea REPARARLO? (S/N) ","N") >= 0)
						{							
							file_chinos = abre_chn(fdatos,file_chinos);
							nreg = (long)lddbl(pdouble);
							lee_directo(fd,nreg,fi,fi->f_buf);							
						...
						if (confirmar(0,"Desea BORRAR TODOS (S/N) ","N") >= 0)
						{
						   if (confirmar(0,"SEGURO QUE DESEA BORRARLOS TODOS? (S/N) ","N") < 0)
							   continue;
						   if (confirmar(0,"HAY ALGUN REGISTRO VALIDO?","S") >= 0)
							   continue;
							...
							{	
								...
								sprintf(tmp,"Borrando Registro: %6ld",nreg);
								pprint(1312,tmp);
								borra_directo(fd,nreg,fi);
								i = lee_reg(pfi,ISNEXT);
							}
...
int itcp_varios(SOCKET client,int operacion,char *linea)
{
	...
	if (operacion == 0)
	{
	  for (j = 0;j < np_ex;j++)
	  {
		 ...
		 {
			p_ejecuta(p_ex[j].indice,O_NOR,0,33);
			sprintf(tmp,":OK: %s\r\n",m_ex[4].mv.valc);
			rwi_senddata(client,tmp,-1);
			ok = 1;
			break;
...
#ifdef nosirve
FI_DEF *crea_def_de_infocampo(struct Info_Entra_Campos *info)
{
	FI_DEF *pfi;
	int i;
	...
	pfi = (FI_DEF *)malloc(sizeof(FI_DEF));
	memset((char *)pfi,0,sizeof(FI_DEF));
	pfi->f_titulo = strmalloc(strlen(info->NomFic)+1);
	strcpy(pfi->f_titulo,info->NomFic);
	strcpy(pfi->f_fichero,info->PathDef);
	strcpy(pfi->f_PathDef,info->PathDef);

	for (i = 0;i < info->n_campos;i++)
	{
		inserta_campo(pfi,i,(char *)dInfoCampo(info,i ,1),
					(char *)dInfoCampo(info,i ,1),
					dInfoCampo(info,i,0)->c_qc,
					(char *)dInfoCampo(info,i,2),
					(char *)dInfoCampo(info,i,3),
					"");
	}
