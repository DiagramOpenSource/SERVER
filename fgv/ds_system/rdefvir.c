...
extern int recur_nfi[MAXRECUR] = {0,0,0,0,0,0,};
extern void *recur_mpfi[MAXRECUR] = {NULL,NULL,NULL,NULL,NULL,};
...
/*
static void verificamelo(FI_DEF *fi)	{
		int npan,numcam,campini,campfin;
		char tmp2[512];
		static int conta = 0;
		
		if (strcmp(quita_path(fi->f_fichero),"agifa024"))
			return;
		conta++;
		if (conta == 2)
		{
		...
		numcam = 0;
        for (npan = 0;npan < fi->f_npanta;npan++)
        {
               campini = fi->f_pantalla[npan].p_campoini;
			   if (npan && campini < campfin)
			   {
					sprintf(tmp2,"Anomalia detectada [%ld] de: %s",conta,fi->f_fichero);
					aviso(0,tmp2);
			   }
               campfin = fi->f_pantalla[npan].p_campofin;			   
               for (; campini<=campfin && campini <= fi->f_ncampos ;campini++) 
               {
                   numcam++;
               }
        }
		if (numcam > fi->f_ncampos)
		{
			sprintf(tmp2,"Anomalia detectada [%ld] de: %s",conta,fi->f_fichero);
			aviso(0,tmp2);
		}
