...
/*
double DsEuroConvierte = (double)0.006010121043837822893753080187035;
int DsDeciModoOtro = 2;
int DsDeciModoNormal = 0;
*/
...
int conexion_lenta = 0;

int pass_cambio_autorizado = 1;
...
char *_xtitu_tecla[20] = {NULL,NULL,NULL,NULL,NULL,
...
char *desc_funs[200] = {NULL,};
static int posi_funs[200] = {0,};
...
} agi_init[] =
{
   ...
   {"PFuncion21",&(_xtitu_tecla[0]),0,0},
   {"PFuncion22",&(_xtitu_tecla[1]),0,0},
   {"PFuncion23",&(_xtitu_tecla[2]),0,0},
   {"PFuncion24",&(_xtitu_tecla[3]),0,0},
   {"PFuncion25",&(_xtitu_tecla[4]),0,0},
   {"PFuncion26",&(_xtitu_tecla[5]),0,0},
   {"PFuncion27",&(_xtitu_tecla[6]),0,0},
   {"PFuncion28",&(_xtitu_tecla[7]),0,0},
   {"PFuncion29",&(_xtitu_tecla[8]),0,0},
   {"PFuncion30",&(_xtitu_tecla[9]),0,0},
   {"PFuncion31",&(_xtitu_tecla[10]),0,0},
   {"PFuncion32",&(_xtitu_tecla[11]),0,0},
   {"PFuncion33",&(_xtitu_tecla[12]),0,0},
   {"PFuncion34",&(_xtitu_tecla[13]),0,0},
   {"PFuncion35",&(_xtitu_tecla[14]),0,0},
   {"PFuncion36",&(_xtitu_tecla[15]),0,0},
   {"PFuncion37",&(_xtitu_tecla[16]),0,0},
   {"PFuncion38",&(_xtitu_tecla[17]),0,0},
   {"PFuncion39",&(_xtitu_tecla[18]),0,0},
   {"PFuncion40",&(_xtitu_tecla[19]),0,0},
   ...
   {"IntentaUsarCodigoMalo",&key_codigocapullo,0,0},
   ...
   {"SOS",&sos_activado,1,0},
   {"CONEXION_LENTA",&conexion_lenta,1,0},   
   ...
   {NULL,NULL,0,0},   
	...
	int salva;
    {
       ...
       case 1:
		  if (salva && _salva_ini)
		  {
			  _salva_ini[j] = malloc(sizeof(int));
			  *((int *)_salva_ini[j]) = *((int *)agi_init[j].dato);
		  }
       ...
       case 2:
		  if (salva && _salva_ini)
		  {
			  _salva_ini[j] = malloc(sizeof(long));
			  *((long *)_salva_ini[j]) = *((long *)agi_init[j].dato);
		  }
       ...
       case 3:
		  if (salva && _salva_ini)
		  {
			  _salva_ini[j] = malloc(sizeof(double));
			  *((double *)_salva_ini[j]) = *((double *)agi_init[j].dato);
		  }
	   ...
	   case 4:
		  if (salva && _salva_ini)
		  {
			  _salva_ini[j] = malloc(strlen(*agi_init[j].dato)+1);
			  strcpy(_salva_ini[j],*agi_init[j].dato);
		  }
		  strcpy(*agi_init[j].dato,tmp);
		  break;
       default:
          break;
       }
    ...
    int inicio_fgv()
	   ...
	   _salva_ini = (char **)malloc(sizeof(char *) * (j + 1));
	   memset((char *)_salva_ini,0,sizeof(char *) * (j + 1));
       ...
       for (i = 0;i < 200;i++) {desc_funs[i] = NULL;posi_funs[i]=0;}
         ...
         for (i = 0;AgiRegistro[i].menu[0];i++)
         {
            if (AgiRegistro[i].menu[0] == 1)
            {
               ...
               if ( (AgiRegistro[i].flags & 1) )
               {
                  ...
                  {
                     fprintf(f,__Texto[304],AgiRegistro[i].orden,AgiRegistro[i].corchetes,AgiRegistro[i].directorio,AgiRegistro[i].acceso,AgiRegistro[i].menu);
                     fprintf(f,__Texto[305],AgiRegistro[i].orden,AgiRegistro[i].corchetes,AgiRegistro[i].directorio,AgiRegistro[i].empresa);
                  }
                  else
                  {				
                     fprintf(f,"{{%s0 %s fich NULL %s}}\n",AgiRegistro[i].corchetes,AgiRegistro[i].directorio,AgiRegistro[i].menu);
                  }			 
               }
         }
         fclose(f);
      ...
      else
      {
         ...
         if ((AgiRegistro[i].flags & 2))
            ...
            if ((AgiRegistro[i].flags & 4))
               c = 'X';
            fprintf(f,"%s%s,%s,%s,%c,%d,\"%s\"",AgiRegistro[i].corchetes,AgiRegistro[i].menu,AgiRegistro[i].directorio,AgiRegistro[i].instalable,c,AgiRegistro[i].orden,AgiRegistro[i].descripcion);
            if (AgiRegistro[i].acceso[0])
            {
