...
int numero;
{
   ...
   unsigned long numero_generacion,nng;
   ...
   rw_read_(file1,(char *)&numero_generacion,sizeof(long));
   moto_long(&numero_generacion);

   i = n_p_tabla - 2;
   if (p_tabla[i] == -1 && p_tabla[i] == -1)
   {
	   memcpy((char *)(p_tabla+(n_p_tabla-2)),(char *)&numero_generacion,sizeof(long));
   }
   if (memcmp((char *)(p_tabla+(n_p_tabla-2)),(char *)&numero_generacion,sizeof(long)))
   {
	   /*rw_error(103);*/	   
	   mensaje_en_cabecera("%s","AVISO 103",0);
	   pito(148);
	   pito(149);
   ...
   for (i = 0;i < nficin;i++)
   {
      ...
      else
      {
         ...
         {
            if (j == 48) continue;
            if (!strcmp(quita_path(s_d[recur+j].nombre_fichero),nombre))
            {
         ...
         if (j == nfi)
         {
			 ...
			 {
				if (j == 48) continue;
				if (!strcmp(quita_path(s_d[recur+j].nombre_fichero),quita_path(nombre)))
				{
				   en_pr[numero].tabla_ficheros[n] = j;
				   break;
				}
         ...
         if (j == nfi)
         {
            ...
            {
               if (j == 48) continue;
               if (!strcmp(s_d[recur+j].enlaza_pfi,nombre))
               {
                  en_pr[numero].tabla_ficheros[n] = j;
                  break;
               }
            ...
            if (j == nfi)
            {       
               printf(__Texto[309],nombre);getchar();
            }        
         }
      }
   }
   ...
   for (i = 0;i < ntabex;i++)
   {
      ...
      if (j == nm_ex)
      {
         printf(__Texto[310],nombre);getchar();
      }
      else
      {
  ...
  {
	   char bbb[350];
	   sprintf(bbb,"tamano=%d PROG=%d MEMO=%d CONDIC=%d BUCLE=%d VAL1=%d",tamano,sizeof(PROGRAM),sizeof(T_MEMORIA),
		                               sizeof(CONDIC),sizeof(BUCLE),sizeof(union _m_val1_));
	   aviso(0,bbb);
   }
   */  
   ...
   if (nmemo > 0)
   {
      if (leememo)
      {
         ...
         {
            rw_read_(file1,(char *)&(memo[_m_i].mv),sizeof(union _m_val1_));
			rw_read_(file1,(char *)&(memo[_m_i].m_tipo),sizeof(short));
			rw_read_(file1,(char *)&(memo[_m_i].m_fflag),sizeof(short));
            moto_short(&(memo[_m_i].m_tipo));
			moto_short(&(memo[_m_i].m_fflag));
      ...
      {
#ifdef ESMOTOROLA
		 rw_lseek_(file1,(long)nmemo *12,SEEK_CUR);
#else
...
int prime;
{
   ...
   moto_short(&rel);
   if (rel == ID_ANT_TAB && !getenv("_O_OPS__"))
   {
      ant_formato = 1; 
      aviso(0,__Texto[84]);
      rw_close_(file1);
   ...
   if (rel == ID_TAB || (rel == ID_ANT_TAB && getenv("_O_OPS__")) )
   {	
      rw_read_(file1,(char *)&rel,sizeof(short));
      moto_short(&rel);
   }
   if (rel < 0) /* es aplicacion estandar */
   {
	  ...
	  {		  
		  char tmpx[512];
		     sprintf(tmpx,"local_term[50]=%d local_term[51]=%d",local_term[50],local_term_[51]);
		     aviso(0,tmpx);
	  }
	  */
  ...
  if (n_p_tabla)
  {	 
     p_tabla = malloc(sizeof(short)*(n_p_tabla+2));
     if (!p_tabla)
     {
        salida = -2;
        goto lee_salida;
     }
     proc_tabla = malloc((n_p_tabla+2));
     if (!proc_tabla)
     {
        salida = -2;
        goto lee_salida;
     }
     tipo_tabla = malloc((n_p_tabla+2));
     if (!tipo_tabla)
     {
        salida = -2;
        goto lee_salida;
...
#ifdef ESMOTOROLA
  for (_m_i = 0;_m_i < (n_p_tabla-2);_m_i++)
  {
  ...
  if (np_ex > 0) {
     ...
     for (_m_i = 0; _m_i < np_ex;_m_i++)
     {
        ...
        /*moto_short(&(p_ex[_m_i].exportable));*/
     }
#endif
     for (i = 0;i < np_ex;i++) {
        rw_read_(file1,&c,1);
  ...
  for (jj = 0;jj < i;jj++)
  {
     ...
     else
     {
        ...
        if (jj < memoplin)
           m_ex[jj].mv.valc = malloc(MAXDEFECTO+1);
        else
  ...
  if (nm_ex > i) 
  {
     ...
     {
        moto_short(&(m_ex[_m_i].m_tipo));
        moto_short(&(m_ex[_m_i].indice_global));
        if (m_ex[_m_i].m_tipo == DOUBLETIPO)
           moto_double(&(m_ex[_m_i].mv.vald));
     }
#endif
     for (;i < nm_ex;i++)
     {
  ...
  if (nen_pr)
  {
     en_pr = (struct enlaza_prc *)malloc(sizeof(struct enlaza_prc) * (nen_pr+1));
     if (!en_pr)
     {
        salida = -2;
        goto lee_salida;
...
int prime;
{
   ...
   if (prime == 0) {
      for (qj=0;qj<nfi;qj++) {
         ...
         {
            if (s_d[qj+recur].buf)  /* caso inicializado */
			{
               free(s_d[qj+recur].buf);
			   s_d[qj+recur].tam_buf = 0;
			}
         }
         if (qj != 48)
         {
