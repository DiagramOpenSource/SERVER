...
int inicio_agisys()
{
   ...
   if (fildes < 0)
   {	   
      fildes = rw_open_(buf,O_RDWR | O_BINARY | O_CREAT, S_IREAD | S_IWRITE );
   }
   if (fildes < 0)
   {
      char tmpx[256];
      sprintf(tmpx,__Texto[329],rw_directorio(""));
      aviso(0,tmpx);
      fin_sig();
      Terfinal();
      exit(2);
...
int cuadro_activacion(char *num1,char *num2,char *res)
{
   ...
   {
      VENT *w = w_ini(1020,2063);
      w_salva(w);
      zonablan(1020,2063);
      cuadro(1020,2063);
      Ar();
      pprint(1021,__Texto[330]);
      Cr();
      pprint(1222,__Texto[331]);
      pprint(1422,__Texto[332]);
      pprint(1622,__Texto[333]);
      pprint(1822,__Texto[334]);
      Ai();
      pprint(1238,num1);
      pprint(1438,num2);
      pprint(1931,__Texto[335]);
      Bi();
      res[0] = 0;
...
static int llama_activacion(long nume_serie,int posicion)
{
   ...
   if (nume_serie == 123456L && posicion == 789)
   {
      ...
      if (tiempo > 1)
      {
         ...
         {
            sprintf(num1,__Texto[336],tiempo);
            aviso(0,num1);
         }
      ...
      if (!buen_oro_(nume_serie,posicion,res))
      {
         ...
         {
            if (tiempo > 1)
               sprintf(num1,__Texto[337],tiempo);
            else
               sprintf(num1,__Texto[338]);
			 if (!abandonarelbarco)
			 {
                aviso(0,num1);
			 }
...
int fin()
{
   ...
   if (!abandonarelbarco)
   {
	   sprintf(tmp,__Texto[339],orip_i);
	   aviso(0,tmp);
   }
   Terfinal();
   exit(0);
