...
int *ss;
{
   ...
   if (_pon_incidencia)
   {
      sprintf(buf,"signal %d<<%d>>",s,_especial_control_);
   ...
   pprint(1009,__Texto[341]);
   v_printd("<<%d>>",_especial_control_);

   if (s != SIGHUP && s != SIGILL && s != SIGKILL && s != SIGSEGV && s != SIGTERM)
   {
      v_printd(" %2d",s);
      pprint(1228,__Texto[342]);
      pprint(1420,__Texto[343]);
      entra(1246,1,__Texto[449],"",__Texto[447],0,buf);
   }
   else
   {
      buf[0] = __Texto[447][0];
      pito(164);pito(164);pito(164);pito(164);pito(164);pito(164);pito(164);pito(164);
      sleep(10);
   }
#ifndef FGVWIN
   Cr();
#endif
   if (buf[0] == __Texto[448][0]) { /* RECUPERAR EL ERROR */
      if (_pon_incidencia)
      {
         sprintf(buf,__Texto[344],s);
         (*_pon_incidencia)(0,buf);
      }
      w_pinta(salva);
      w_fin(salva);
