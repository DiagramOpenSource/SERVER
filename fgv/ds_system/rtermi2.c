...
void Terinici()
{
   ...
   */
      ...
      while (fgets(linea,82,file1) != NULL) {
         ...
         if (!i)
         {
            ...
            if (extra_terinici > 0)
            {
			   ...
			   for (j = 0;j < _NUMTECLA;j++)
			   {
				  tecla[j] = (short *)malloc(sizeof(short) * (NUMCARTE+1));
				  memcpy((char *)tecla[j],(char *)_tecla[j],sizeof(short) * (NUMCARTE+1));				  
			   }
   ...
   if (extra_terinici == 0)
   {
	   /* extra de teclas de funcion solo accesibles desde windows */
	           extra_terinici = 20;
               NUMTECLA = _NUMTECLA + extra_terinici;
               FUNCIONES = _FUNCIONES + extra_terinici;
               titu_tecla = (struct tit_tecla *)malloc(sizeof(struct tit_tecla) * (NUMTECLA-NUMESPEC));
               ...
               for (j = 0;j < extra_terinici;j++)
               {
                  titu_tecla[_NUMTECLA-NUMESPEC+j].r = NULL;
                  titu_tecla[_NUMTECLA-NUMESPEC+j].t = "Extra funcion";
                  titu_tecla[_NUMTECLA-NUMESPEC+j].d = _xtitu_tecla[j];
				  _xtitu_tecla[j] = NULL;
               }
               tecla = (short **)malloc(sizeof(short *) * NUMTECLA);
			   for (j = 0;j < _NUMTECLA;j++)
...
int ayuda_teclas()
{
   ...
   Ar();
   zonablan(602,2279);
   pprint(611,__Texto[348]);
   Cr();
   Ai();
   pprint(702,__Texto[349]);
   pprint(721,__Texto[350]);
   pprint(732,__Texto[351]);
   Bi();
   Ar();
