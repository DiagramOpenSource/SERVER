...
int l_entra(qp,maxqc,qi,qs,qd,qh,qo)
int qp,
maxqc,
qh;
char *qi,*qs,*qd,*qo;
   ...
   while( -1 ) {
      ...
      do {
         ...
         if (!modo && !rentra_insert && *texto) {
            ...
            quitab(tmp);
            j = strlen(tmp);
            if (j > tcol) {j = tcol;tmp[j] = 0;pito(169);}
            free(*texto);
...
int qpi_rt;
{
   ...
   while( (tmp = *(edi_text + i)) != NULL && i < topelin) 
   {
      if ((n = strlen(tmp)) != 0) 
	  {
...
int *oc;
{
   ...
   *c_v %= 100;
   ...
   if (modo_externo && DespachadorExterno)
   {
	  ...
	  {		 
		 ppp -= 2;
		 while (ppp >= (xxtmp + sizeof(int) + 2))
		 {
			 if (*ppp == 13 && *(ppp+1) == 10 && *(ppp-2) == 13 && *(ppp-1) == 10)
			 {
				 *ppp = 0;
				 ppp -= 2;
			 }
      ...
      if (l_inibloque >= ntlin) {
         ...
         if (v) {
            ...
            do {
			   ...
			   {/* para el caso cr se puede usar un control para la entra() */
