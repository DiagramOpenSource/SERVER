...
static instalacion_lista_grabar = 0;
...
static unsigned char p_versionletra = 0;
...
char *path2;
{
   ...
   if ((ifd = rw_base_open_ (path1, O_RDONLY | O_BINARY)) < 0)
      return -1;
   if ((ofd = rw_base_open_ (path2, O_RDWR | O_BINARY | O_CREAT | O_TRUNC,S_IREAD | S_IWRITE)) < 0)
      return -1;  
   while ((nrbytes = rw_base_read_ (ifd, buf, sizeof (buf))) > 0)
			{
      if ((nwbytes = rw_base_write_ (ofd, buf, nrbytes)) != nrbytes)
      {
         nrbytes = -1;
         break;
      }
			}
   if ((nrbytes < 0) | (0 != rw_base_close_ (ifd)) | (0 != rw_base_close_ (ofd)))
			{
      rw_base_unlink (path2);
      return -1;
			}
   return 0;
}
...
void fl_read ()
{
   ...
   if (estoy_en_gzip)
   {
                  ...
                  if (es_instalacion || pinta_progreso)
                     presenta_progreso(blocksize);
   }
   else
   {
						err = rw_base_read_(archive, ar_block->charptr, (int) blocksize);
                  if (err == blocksize)
                  {
                     if (es_instalacion || pinta_progreso)
                        presenta_progreso(blocksize);
...
void fl_write ()
{
   ...
   err = rw_base_write_(archive, ar_block->charptr, (int) blocksize);
   if (err != blocksize)
   {
						/* multi volumen */
   }
...
int reading;
{
   ...
   if (!estoy_en_gzip)
   {
      if (reading == 2)
      {
         archive = rw_base_open_(ar_files[0], O_RDWR | O_CREAT | O_BINARY, 0666);
      }
      else if (reading)
      {
         archive = rw_base_open_(ar_files[0], O_RDONLY | O_BINARY, 0666);
      }
      else
      {
         archive = rw_base_open_(ar_files[0],  O_RDWR | O_BINARY | O_CREAT | O_TRUNC, 0666);
      }
      if (archive < 0)
      {
...
int toplevel;		/* Whether we are a toplevel call */
{
   ...
   */
      ...
      )
   {
      ...
      {
         f = rw_base_open_ (p, O_RDONLY | O_BINARY);
         if (f < 0)
            goto badperror;
      }
      else
      ...
      while (sizeleft > 0)
      {
         ...
         count = rw_base_read_ (f, start->charptr, bufsize);
...
register long size;
{
   ...
   while (size > 0)
      ...
      if (es_instalacion == 1)
      {
		 if (file_rwi > -1)
		 {
			  if (rellena_size >= blocksize)
				  nn = blocksize;
			  else
				  nn = rellena_size;
			  rw_base_write_(file_rwi,x->charptr,nn);
		 } 
...
char *fname;
{
   ...
   if (otro_destino)
   {
      ...
      file = rw_base_open_(tmp_c,O_BINARY | O_RDONLY);
      if (file >= 0)
      {
         rw_base_close_(file);
         return(0);
      } 
      fileo = rw_base_open_(fname,O_BINARY | O_RDONLY);
      if (fileo < 0)
      {
         return(0);
      }
      do 
      {
         file = rw_base_open_(tmp_c,O_BINARY | O_RDWR | O_CREAT | O_TRUNC,S_IREAD | S_IWRITE);
         if (file < 0)
         {
      ...
      while ((nrbytes = rw_base_read_ (fileo, tmp_b, sizeof (tmp_b))) > 0)
      {
         if ((nwbytes = rw_base_write_ (file, tmp_b, nrbytes)) != nrbytes)
         {
            nrbytes = -1;
            break;
         }
      ...
      rw_base_close_(file);
      rw_base_close_(fileo);     
      chmod(tmp_c,st1.st_mode);
      if (strbusca(tmp_c,"ibm80.bak") > -1)
      {        
         f1 = rw_base_open_(tmp_c,O_RDWR | O_BINARY);
         if (f1 >= 0)
         {   
            ...
            rw_base_lseek_(f1,0L,SEEK_SET);
            rw_base_write_(f1,&signature,sizeof(long));
            t1 = numero ^ midata;
            rw_base_write_(f1,&t1,sizeof(long));
            t1 = data2 ^ midata;
            rw_base_write_(f1,&t1,sizeof(long));
            t1 = data ^ midata;
            rw_base_write_(f1,&t1,sizeof(long));
...
int insta_aviso(int p,char *mensa)
{
   ...
   if (dlg)
   {
      ...
      MessageBox(NULL,mensa,__Texto[101],MB_OK | MB_ICONHAND);   
...
/*
   ...
   {	   	
	   ...
	   {
           (*DespachadorExterno)((int)DESPACHO_MULTIPLE,15,(void *)&hInstacia);
	   } 
   }
#endif
*/
...
int presenta_progreso(progreso)
int progreso;
{
   int i,por;
   char tmp[81];
   ...
   if (instalacion_en_grafico || pinta_progreso)
   {
#ifdef FGVDOS
    MSG msg;
      ...
      if (pinta_progreso && dlg)
           SetDlgItemText(dlg,IDC_VER1,current_file_name);
         ...
         if (ori[i+1] == '!')
         {
            ...
            if (ori[i+1] == '.')
            {
               ...
               if (ori[i+1] == '%')
               {
                  ...
                  if (ori[i+1] == 'V')
                  {
                     sprintf(tmp2,"%d.%02d.%03d",infp.version1,infp.version2,(int)infp.versionletra);
                     strcpy(tmp+j,tmp2);
                     j += strlen(tmp2);
                  }
                  else
...
int rellena_instalacion(bloque)
char *bloque;
{
...
static int pregunta_instalacion()
{
   ...
   {
      sprintf(tmp,__Texto[206],infp.descrip);
   ...
   if (infp.rdependemodo)
   {
      ...
      {	
         ...
         {
            sprintf(tmp,__Texto[382],infp.rdepende1,infp.rdepende2,(int)infp.rdependeletra);
            insta_aviso(0,tmp);
            if (infp.rdependemodo == 1)
            {
               if (insta_confirmar(2440,__Texto[383],__Texto[448]))
               {
                  return(-1);
               }		 
            }
   ...
   if (infp.pdependemodo)
   {
      ...
      {	
         if (!instalacion_silenciosa)
         {
            sprintf(tmp,"Esta Instalacion requiere Version Previa %d.%02d.%03d o superior",infp.pdepende1,infp.pdepende2,(int)infp.pdependeletra);
            insta_aviso(0,tmp);
            if (infp.pdependemodo == 1)
            {
               if (insta_confirmar(2440,__Texto[438],__Texto[448]))
               {
                  return(-1);
               }		 
            }
   ...
   if (registro_instalacion) /* comprobar menu para version instalada */
   {
	  AGIREGISTRO item;
	  FILE *f;
	  int laversion;
	  ...
	  queversion = infp.version1 * 100000 + infp.version2 * 1000 + infp.versionletra;

      for (i = 0;i < nregistro_instalacion;i++)
      {
		  strcpy(tmp,registro_instalacion[i]);
		  parser_item_registro(tmp,&item);
		  if (item.menu[0] && item.directorio[0])
		  {
			  sprintf(tmp,"%s/%s.men",rw_directorio(item.directorio),item.menu);
			  f = fopen(tmp,"r");
			  laversion = 0;
			  if (f)
			  {					
					...
					while(fgets(buff,2044,f))
					{
						...
						if (buff[0] == '#')
						{
							...
							nn = strbusca(buff+off,":");
							if (nn > -1)
							{
								off += (nn+1);								
								if (buff[off] == '(' && buff[off+4] == ')')
								{
									memcpy(tmp,buff+off+1,3);
									tmp[3] = 0;
									ver3 = atoi(tmp);
									laversion = atoi(tmp);
									off = strlen(buff+off) - 5;
									if (off >= 0)
									{										
										if (buff[off+2] == '.')
										{
											memcpy(tmp,buff+off+3,2);
											tmp[2] = 0;
											ver2 = atoi(tmp);
											laversion += atoi(tmp)*1000;
											memcpy(tmp,buff+off,2);
											tmp[2] = 0;
											ver1 = atoi(tmp);
											laversion += atoi(tmp)*100000;
											break;
										}
									}
								}
								else
								{
									...
									if (off >= 0)
									{
										if (buff[off+2] == '.' && buff[off+5] == '.')
										{
											memcpy(tmp,buff+off+6,3);
											tmp[3] =0;
											ver3 = atoi(tmp);
											laversion = atoi(tmp);
											memcpy(tmp,buff+off+3,2);
			  ...
			  if (laversion > 0 && laversion > queversion)
			  {
				  sprintf(tmp,"La version actual de %s (en %s.men) es mayor! [%02d.%02d.%03d > %02d.%02d.%03d]",item.directorio,item.menu,ver1,ver2,ver3,infp.version1,infp.version2,(int)infp.versionletra);
				  aviso(0,tmp);
				  instalacion_ok = 4;
				  return -1;
			  }
   ...
   else
   {
      ...
      while (pp < nvariables_instalacion)
      {
         if (pagina)
         {
            ...
            if (instalacion_silenciosa < 2)
            {
               v_scroll(906+inip*100,1574,ppi);	
            }
         }
         numero = variables_instalacion[pp][0] - 48;
         modo   = variables_instalacion[pp][1] - 48; /* el modo vamos ignorarlo de momento */
         ...
         do {
            ...
            if (qj == 1 || qj == 7)
            {
               if (insta_confirmar(2440,__Texto[439],__Texto[448]))
               {
                  return(-1);
               }
               pprint(2440,"                                 ");
      ...
      if (!instalacion_silenciosa && insta_confirmar(2440,__Texto[440]," "))
      {
         return(-1);
      }
 } /* instalacion en grafico */
...
again_file:
      ...
      */
      ...
      if (f_unlink && !f_keep) {
         if (rw_base_unlink(skipcrud + current_file_name) == -1)
            if (errno != ENOENT)
      ...
      */
      if (head->header.linkflag == LF_CONTIG)
         fd = rw_base_open_ ((longname ? longname : head->header.name)
         + skipcrud,
         openflag | O_CTG,
      ...
      * With 3-arg open(), we can do this up right.
         */
         fd = rw_base_open_ (skipcrud + current_file_name,
      ...
      if (fd < 0)
      {
         ...
         if (es_instalacion)
         {
            if (reintento < 20)
            {			   
			   if (!reintento && yarenombrado)
			   {
				   strcpy(tmp_y,skipcrud + current_file_name);
				   strcat(tmp_y,"._ai");
				   rw_base_unlink(tmp_y);
				   rw_base_rename(skipcrud + current_file_name,tmp_y);
				   /* intentael el renombrado */
				   yarenombrado = 1;				   
				   goto again_file;
			   }
               ...
               {
                  sprintf(tmp_c,__Texto[207],current_file_name); 
            ...
            if (instalacion_silenciosa < 2)
            {
               sprintf(tmp_c,__Texto[208],current_file_name);
               if (!instalacion_silenciosa)
               {			   
                  insta_aviso(0,tmp_c);
               }
               ...
               {
                  insta_mensaje(0,tmp_c);
                  pito(173);pito(173);pito(173);pito(173);
               }
            }
            return;
         }
      ...
      /*			long	offset,
         ...
         */
         ...
         if (written > size)
            written = size;
         errno = 0;
         check = rw_base_write_ (fd, data, written);
         /*
...
unsigned tam;
{
   ...
   return(rw_base_read_(*pfd,buf,tam));
}
...
unsigned tam;
{
         ...
         if (gzip_skip)
         {
            if (es_instalacion == 1)
            {
               while(gzip_skip && tam)
               {
                  ...
                  gzip_skip -= blocksize;
				  if (es_sacarwi && file_rwi > -1)
				  {
					  if (rellena_size >= blocksize)
						  nn = blocksize;
					  else
						  nn = rellena_size;
					  rw_base_write_(file_rwi,gzip_parcial,nn);
				  } 
         ...
         if (gzip_continuacion)
         {
            if (gzip_continuacion <= tam)
            {
               if (gzip_realsize)
               {
                  rw_base_write_(gzip_continuafd, buf,gzip_realsize);
                  gzip_realsize = 0L;
               }
            ...
            else
            {
               ...
               {
                  rw_base_write_(gzip_continuafd, buf,tam);	  
                  gzip_realsize -= tam;
               }
               else
               {	
            ...
            if (!gzip_continuacion)
            {
               rw_base_close_(gzip_continuafd);    
               if (we_are_root || f_do_chown)
               {
...
int limpia_instalacion(char *destino,char *fichero)
{
   int i;
   if (es_instalacion)
   {	 	 
...
{ 
   ...
   if (strlen(fichero) > strlen(destino) && !memcmp(destino,fichero,strlen(destino))) {
	   argc = _separa(fichero+strlen(destino),argv,1,249);
	   /*argv[0] = fichero;*/
   }
   else
   ...
   extr_init ();
   i = strlen(fichero) - 4;
   if (i > 0 && (!strcmp(fichero+i,".tgz") || (!strcmp(fichero+i,".dsa")) || !strcmp(fichero+i,".taz") || 
      !strcmp(fichero+i+2,".Z") || !strcmp(fichero+i+2,".z") ||
				  !strcmp(fichero+i,".tgZ") || !strcmp(fichero+i,".taZ") ))
   {	  
      i = rw_base_open_(fichero,O_BINARY | O_RDONLY);
      if (i >= 0)
      {
#ifdef RWSERVER
   ...
   limpia_instalacion(destino,fichero);
...
char *continua;
{
   ...
   if (entorno_antivirus)	   
   {
	   ...
	   file = rw_base_open_(tmp,O_RDONLY | O_BINARY);
   }
   if (file > -1)
   {
	   char bloque[4096];
	   ...
	   size = rw_base_lseek_(file,0L,SEEK_END);
	   if (size > 4096)
		   size = 4096;
	   rw_base_lseek_(file,0L,SEEK_SET);
	   rw_base_read_(file,bloque,size);
	   rw_base_close_(file);
	   ...
	   abl = blocksize;
	   blocksize = 4096;
	   rellena_instalacion(bloque);
	   blocksize = abl;
	   hay_ins_continuacion = 0;	  
	   instalacion_ok = 1;
	   limpia_instalacion("",tmp);
   }
   else   
   {	   
	   if (entorno_antivirus)	   
	   {
		   ...
		   if (file < 0)
		   {
			   file_rwi = rw_base_open_(tmp,O_RDWR | O_BINARY | O_CREAT | O_TRUNC,S_IREAD|S_IWRITE);
		   }
		   else
		   {
			   rw_base_close_(file);
...
char *continua;
{
   ...
   if (!puede_grabar)
   {
      return datos_paquete(paquete,NULL);
   }
...
int debe;
{
         ...
         if (tam1)
         {
            rw_base_lseek_(fd,inip,SEEK_SET);
            r = rw_base_locking(fd,2,tam1);
            rw_base_lseek_(fd,inip,SEEK_SET);
            rw_base_locking(fd,0,tam1);
            if (r) return(r);
         }
         if (!tam2) return(r);
         rw_base_lseek_(fd,inip+tam1+1L,SEEK_SET);
         r = rw_base_locking(fd,2,tam2);
         rw_base_lseek_(fd,inip+tam1+1L,SEEK_SET);
         rw_base_locking(fd,0,tam2);
         return(r);
}
...
int se_esta_instalando()
{
   char tmp[512];
   int nopuede = -1;
   int fd = agi_control_fildes;
   int i;
   if (fd == -1)
   {
	  strcpy(tmp,rw_directorio("informa.pan"));
      fd = rw_base_open_(tmp,O_BINARY | O_RDWR);
   }
   if (fd >= 0) 
   {  
      rw_base_lseek_(fd,50001L,SEEK_SET);
      if (rw_base_locking(fd,2,1L) == -1)
      {
         insta_mensaje(0,__Texto[209]);
         for (i = 0;i < 100;i++)
         {
			pulsatecla();
            rw_base_lseek_(fd,50001L,SEEK_SET);
            if (!rw_base_locking(fd,1,1L) != -1) break;
         }
         Bl(4);
         if (i >= 100)
         {
            rw_base_lseek_(fd,50001L,SEEK_SET);
            rw_base_locking(fd,0,1L);
      ...
      rw_base_lseek_(fd,50000L,SEEK_SET);
      if (rw_base_locking(fd,2,1L) == -1)
	  {
		  nopuede = 1;
	  }
...
int debe_grabar;
{
...
#ifdef FGVX
   /* OJO, sin instalacion ... de momento */
   return 0;
#endif
   ...
   {  
                  ...
                  if ((ri = bloquea_region_sys(fd,nume,debe_grabar)) != -1)
                  {
                     ...
                     {
                        puede_grabar = 1 + ri;
						instalacion_lista_grabar = puede_grabar;
                     }
                  }
                  else
                  {
   ...
   if (file) 
   {
                  ...
                  {		
                     ...
                     else
					 {
						 ...
						 {
							Paq[nPaq].i.versionletra = atoi(tmp+j);
						 }
					 }
					 ...
					 {
						 Paq[nPaq].i.rdependeletra = atoi(tmp+j);
					 }
					 ...
					 {
						 Paq[nPaq].i.pdependeletra = atoi(tmp+j);
					 }
					...
					if (!entorno_antivirus)
					{
                     ...
                     else
                     {
                        ...
                        if (strcmp(tmp2,".") && strcmp(tmp2,"..") && strcmp(tmp2,"agilista.rwp") && 
                           memcmp(tmp2,"instalar",8) && (!coincide_(tmp2,"*.rwi") || !entorno_antivirus))
                        {
                           i = strlen(tmp);		   
                           strcat(tmp,_buf->d_name);
                           rw_base_unlink(tmp);
      ...
      {	       
         ...
         {  
            ...
            { 
               ...
               {
                  rw_base_lseek_(fd,50001L,SEEK_SET);
                  if (!rw_base_locking(fd,1,1L)) break;
               }
            }
         }
         file = fopen(tmp,"wt");
         if (file) 
         {
            for (i = 0;i < nPaq;i++)
            {
               ...
               fprintf(file,"%s,%c,%s,%d,%02d,%03d,%d,%s,%s,%d,%d,%d,%02d,%03d,%d,%d,%02d,%03d,%d,%d,%s,%ld\n",
                  Paq[i].nompaq,
                  Paq[i].i.tipo,
                  Paq[i].i.sistema,
                  Paq[i].i.version1,
                  Paq[i].i.version2,
                  (int)Paq[i].i.versionletra,
                  Paq[i].i.autoversion,
                  Paq[i].i.fecha,
                  Paq[i].i.descrip,
                  Paq[i].i.ver_interna, 
                  ...
                  Paq[i].i.rdepende1,
                  Paq[i].i.rdepende2,
                  (int)Paq[i].i.rdependeletra,					  
                  Paq[i].i.rdependemodo,
                  Paq[i].i.pdepende1,
                  Paq[i].i.pdepende2,
                  (int)Paq[i].i.pdependeletra,					  
...
HBITMAP pantalla_prueba(LPRECT lpRect)
{
...
WORD APIENTRY prueba_save(HDIB hDib, LPSTR lpFileName)
{
