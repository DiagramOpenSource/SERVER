...
COLORREF color_1 = RGB(0,0,128);
COLORREF color_2 = RGB(0,128,0);
COLORREF color_3 = RGB(0,128,128);
COLORREF color_4 = RGB(128,0,0);
COLORREF color_5 = RGB(128,0,128);  
COLORREF color_6 = RGB(128,128,0);  
COLORREF color_7 = RGB(128,128,128);  


COLORREF color_i1 = RGB(0,0,248);
COLORREF color_i2 = RGB(0,252,0);
COLORREF color_i3 = RGB(0,252,248);
COLORREF color_i4 = RGB(248,0,0);
COLORREF color_i5 = RGB(248,0,248);  
COLORREF color_i6 = RGB(248,252,0);  
COLORREF color_i7 = RGB(255,255,255);  


COLORREF color_fondo_1 = RGB(0,0,128);
COLORREF color_fondo_2 = RGB(64,128,128);
COLORREF color_fondo_3 = RGB(0,252,248); 
COLORREF color_fondo_4 = RGB(248,0,0);
COLORREF color_fondo_5 = RGB(248,0,248);
COLORREF color_fondo_6 = RGB(248,252,0);
COLORREF color_fondo_7 = RGB(255,255,255);

COLORREF color_fondo_campo = RGB(255,255,255);
COLORREF color_fondo       = RGB(192,192,192);
COLORREF color_texto_campo = RGB(0,0,0);
COLORREF color_texto = RGB(0,0,0);
...
#define POINT2MPOINT(pt, mpt)   ((mpt).x = (SHORT)(pt).x, (mpt).y = (SHORT)(pt).y)
    ...
    static void DS_FillRect(HDC hdc,RECT *rect,HBRUSH brush)
	{	
		HBITMAP hbmp = NULL;
    
	     /*
		   if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,(int)&espacio,(void *)w->vhdc);
         */
		/*brush != agi_brush_gris && GetBkColor(hdc) != color_fondo*/
		if (brush == agi_brush_gris)
			hbmp = agitapiz;
		else if (brush == agi_brush || brush == NULL)
			hbmp = ds_bmp_fondo;		
		...
		else
		{
	       ...
	       BitBlt(hdc,rect->left,rect->top,rect->right-rect->left+1,rect->bottom-rect->top+1,hMemoryDC,rect->left,rect->top,SRCCOPY);
    ...
    int tiempo;
          ...
          char *str;
          {
             ...
             HFONT oldfont;
             RECT espacio;
             COLORREF hcolor = color_1;
             SIZE ss;
             int cx,cy;
             char tmp[128];
             int pp,pp2;
			 int ancho = principal_rect.right-principal_rect.left;
             ...
             BitBlt(stdpan->vhdc,0,espacio_arriba+scaracter.cy*3,ancho,scaracter.cy,hMemoryDC,0,espacio_arriba+scaracter.cy*3,SRCCOPY);
             SelectObject(hMemoryDC,hOldBitmap);
             hOldBitmap = NULL;
             ...
             cx = 2+( (principal_rect.right-principal_rect.left) - ss.cx) / 2;
             ...
             SetRect(&espacio,2,espacio_arriba+scaracter.cy*3,ancho-2,espacio_arriba+scaracter.cy*4-1);
             ExtTextOut(stdpan->vhdc,cx,cy,ETO_CLIPPED,&espacio,str,ntext,NULL);
             SelectObject(stdpan->vhdc,oldfont);
             if (!NO_PINTAR_AHORA)
             {
                hdc = GetDC(principal_w);
                BitBlt(hdc,principal_rect.left,principal_rect.top+espacio_arriba+scaracter.cy*3,ancho,scaracter.cy,stdpan->vhdc,0,espacio_arriba+scaracter.cy*3,SRCCOPY);
         ...
         int boton;
		 {
							   ...
							   (*DespachadorExterno)((int)DESPACHO_FIN_CONTROL,boton,NULL);
								/*
        ...
        int boton_arriba(int nada)
		{
			return nada;
		}

		int boton_flag(int idboton,int flag)
		{
			/*
			if ((flag&1))
			{
				if (DespachadorExterno)
				{
					(*DespachadorExterno)((int)DESPACHO_FOCOCONTROL,idboton-1,NULL);  
				}
			}
			*/
			return 0;
		}
		int boton_abajo(int nada)
		{
			return nada;
		}		

		 						
int crea_boton_menusg(linea,columna,texto,nl,id,flag)
   int linea,columna;
char *texto;
int nl;
short id;
...
int pregunta_impresion(_fispo,__ejspool,_rw_user,especial_servidor,__fispool,otrospool)
char *_fispo;
char *__ejspool;
char *_rw_user;
...
int correcto()
{
   ...
   tmp[0] = 'N';
   strcpy(tmp+1,"¿Es correcto?");
  
   
   // qj = DialogBoxParam(hInst, "AGICORRECTO", principal_w, agilpDlgProcx,(LPARAM)NULL);
   if (defectoesc)
	    qj = DialogBoxParam(hInst,"WAGICORRECTON", principal_w /*GetParent(principal_w)*/,agilpDlgProcx,(LPARAM)tmp);
   else
   ...
   */
   ...
   if (!menusg_virtual || menusg_virtual == CRETURN || menusg_virtual == -1)
   {
      ...
      if (varlin == -1)
      {
					if (lineas)      /* hay varias lineas  */
                           {
                              b_opcion[numero] = crea_boton_menusg(col[numero]/100,col[numero]%100,">",1,PGDN,0);
                           }
                           for (i=0;i<numero;i++)
                           {
                              b_opcion[i] = crea_boton_menusg(col[i]/100,col[i]%100,opcion[i]+d,strlen(opcion[i]+d),MID_MENUSG+i,(((i+1) == elegida) ? 1 : 0));
                           }
      }
      else
      {
							if (lineas)      /* hay varias lineas  */
                           {
                              b_opcion[numero] = crea_boton_menusg(col[0]/100,col[0]%100+numero*macol,">",1,PGDN,0);
                              b_opcion[numero+1] = crea_boton_menusg(col[0]/100,col[0]%100-1,"<",1,PGUP,0);
                              for (i=0;i<numero;i++)
                              {
                                 b_opcion[i] = crea_boton_menusg(col[i]/100,col[0]%100+i*macol,opcion[i]+d,macol,MID_MENUSG+i+menusg_w_dato,(((i+1) == elegida) ? 1 : 0));
                              }
                           }
                           else
                           {
   ...
   do {
      ...
      else
      {
						   ...
						   if (DespachadorExterno)
						   {
								(*DespachadorExterno)((int)DESPACHO_FOCOCONTROL,b_opcion[elegida-1],NULL);  
						   }
                           c = leecar();
                           pulsa_en_espacio = -1;
      }
...
int PonLogo()
{
         ...
         strcpy(tmp,rw_directorio("logonet.bmp"));
         ...
         */
         if (mi_bmp)
         {
			...
			des_bmp = GetCurrentObject(stdpan->vhdc,OBJ_BITMAP);
			GetObject (des_bmp, sizeof(BITMAP), &bmp2);			
            ...
            buffersize = bmp.bmWidthBytes * tby;
			buffersize2 = bmp2.bmWidthBytes * bmp2.bmHeight;

			buffer = (BYTE*)malloc(buffersize);
			buffer2 = (BYTE*)malloc(buffersize2);
			GetBitmapBits(mi_bmp,buffersize,buffer);
			GetBitmapBits(des_bmp,buffersize2,buffer2);
			...
			onx = (principal_rect.left+((onx-nx)/2))*4;
			ony = principal_rect.top+((ony-ny)/2);

			lpb1 = buffer;
			
			for (i = 0;i < tby;i++)
			{
				lpb2 = buffer2 + ((i+ony)*bmp2.bmWidthBytes) + onx;
				for (x = 0;x < tbx;x++)
				{
			...
			SetBitmapBits(des_bmp,buffersize2,buffer2);
			free(buffer);
			free(buffer2);
			DeleteObject(mi_bmp);
		    ...
		    while (i < buffersize) 
			{
			  if (buffer[i + 0] == buffer[i + 1] && buffer[i + 1] == buffer[i + 2] && buffer[i + 2] == 255)
			  {
				  buffer[i + 3] = 128;
			...
			SetBitmapBits(mi_bmp,buffersize,buffer);
			free(buffer);


			AlphaBlend(stdpan->vhdc,principal_rect.left+((onx-nx)/2),principal_rect.top+((ony-ny)/2),nx,
               ny,hMemoryDC,
               0,0,
               tbx,tby,blendFunction);
         ...
         if (proporcion_especial_bmp == 2)
            ...
            else
            {
               ...
               if (ofx ||ofy)
               {
				  ...
				  DS_FillRect(w->vhdc,&espacio,agi_brush_gris);
               }
...
void init_bmp_base(BMP_BASE_PAR *pPar)
{
		 ...
		 if ((pPar->flags&2))
		 {
			 ...
			 color_1 = pPar->color_1;
			 color_2 = pPar->color_2;
			 color_3 = pPar->color_3;
			 color_4 = pPar->color_4;
			 color_5 = pPar->color_5;
			 color_6 = pPar->color_6;
			 color_7 = pPar->color_7;
			 color_i1 = pPar->color_i1;
			 color_i2 = pPar->color_i2;
			 color_i3 = pPar->color_i3;
			 color_i4 = pPar->color_i4;
			 color_i5 = pPar->color_i5;
			 color_i6 = pPar->color_i6;
			 color_i7 = pPar->color_i7;
			 
			 color_fondo_1 = pPar->color_fondo_1;
			 color_fondo_2 = pPar->color_fondo_2;
			 color_fondo_3 = pPar->color_fondo_3;
			 color_fondo_4 = pPar->color_fondo_4;
			 color_fondo_5 = pPar->color_fondo_5;
			 color_fondo_6 = pPar->color_fondo_6;
			 color_fondo_7 = pPar->color_fondo_7;
			 

			 color_fondo       = pPar->cfondo_seleccion;
			 color_texto       = pPar->ctexto_seleccion;
		 }		 
         ...
         agitapiz_n0 = CreateCompatibleBitmap(hDC,principal_nx,principal_ny);
         ...
         agi_brush = CreateSolidBrush(color_fondo);
         agi_brush_gris = CreateSolidBrush(color_fondo);
		 ...
		 //FillRect(hMemoryDC, &principal_rect,agi_brush/*GetStockObject(LTGRAY_BRUSH)*/);

         hOldBitmap = SelectObject(hMemoryDC,agitapiz_n0);		 
		 if (DespachadorExterno)
			(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,0,(void *)hMemoryDC);         
         SelectObject(hMemoryDC,hOldBitmap);

		 if (agitapiz_n1)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,agitapiz_n1);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,1,(void *)hMemoryDC);
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (agitapiz_n2)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,agitapiz_n2);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,2,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (agitapiz_n3)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,agitapiz_n3);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,3,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_1)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_1);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,4,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_2)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_2);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,5,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_3)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_3);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,6,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_4)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_4);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,7,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_5)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_5);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,8,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_6)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_6);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,9,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }
		 ...
		 if (bmp_fondo_7)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_6);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,10,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }

		 if (bmp_fondo_campo)
		 {
			 hOldBitmap = SelectObject(hMemoryDC,bmp_fondo_campo);
			 if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_HAZ_VISIBLE,11,(void *)hMemoryDC);         
			 SelectObject(hMemoryDC,hOldBitmap);
		 }
...
static int w_fondocorriente = 0;

int w_sumafondo()
{
	w_fondocorriente++;
	switch(w_fondocorriente)
	{
	case 1:
		agitapiz = agitapiz_n1;
		break;
	case 2:
		agitapiz = agitapiz_n2;
		break;
	case 3:
		agitapiz = agitapiz_n3;
		break;
	default:
		agitapiz = agitapiz_n0;
	}
...
void fin_bmp_base()
{
   ...
   DeleteObject(agitapiz_n0);agitapiz_n0 = NULL;
   if (agitapiz_n1)
   {
	   DeleteObject(agitapiz_n1);agitapiz_n1 = NULL;
   }
   if (agitapiz_n2)
   {
	   DeleteObject(agitapiz_n2);agitapiz_n2 = NULL;
   }
   if (agitapiz_n3)
   {
	   DeleteObject(agitapiz_n3);agitapiz_n3 = NULL;
   }
   if (bmp_fondo_1)
   {
	   DeleteObject(bmp_fondo_1);bmp_fondo_1 = NULL;
   }
   if (bmp_fondo_2)
   {
	   DeleteObject(bmp_fondo_2);bmp_fondo_2 = NULL;
   }
   if (bmp_fondo_3)
   {
	   DeleteObject(bmp_fondo_3);bmp_fondo_3 = NULL;
   }
   if (bmp_fondo_4)
   {
	   DeleteObject(bmp_fondo_4);bmp_fondo_4 = NULL;
   }
   if (bmp_fondo_5)
   {
	   DeleteObject(bmp_fondo_5);bmp_fondo_5 = NULL;
   }
   if (bmp_fondo_6)
   {
	   DeleteObject(bmp_fondo_6);bmp_fondo_6 = NULL;
   }
   if (bmp_fondo_7)
   {
	   DeleteObject(bmp_fondo_7);bmp_fondo_7 = NULL;
   }
   if (bmp_fondo_campo)
   {
	   DeleteObject(bmp_fondo_campo);bmp_fondo_campo = NULL;
   }   
...
HDC hdc;
{
   ...
   default:
     ...
     {
     case 1:  /* azul */
        hcolor = color_fondo_1; 
		ds_bmp_fondo = bmp_fondo_1;
        break;
     case 2:  /* verde */
        hcolor = color_fondo_2;
		ds_bmp_fondo = bmp_fondo_2;
        break;
     case 3:   /* cyan */
        hcolor = color_fondo_3;
		ds_bmp_fondo = bmp_fondo_3;
        break;
     case 4:   /* rojo */
        hcolor = color_fondo_4;
		ds_bmp_fondo = bmp_fondo_4;
        break;
     case 5:   /* magenta */
        hcolor = color_fondo_5;
		ds_bmp_fondo = bmp_fondo_5;
        break;
     case 6:   /* amarillo */
        hcolor = color_fondo_6;
		ds_bmp_fondo = bmp_fondo_6;
        break;
     case 7:   /* blanco */
        if ((Patrib & 128))
         {
		...
		{
             hcolor = color_fondo_7;
			 ds_bmp_fondo = bmp_fondo_7;
		}
        break;
     case 0:   /* negro */
        hcolor = color_fondo;
		ds_bmp_fondo = agitapiz;
     default:
        break;
     }
     hcolor = GetNearestColor(hdc,hcolor);
...
VENT *w2;
{
   ...
   if (ifin > 23)
   {
      if (ifin == 24)
         ...
         hbr = CreateSolidBrush(color_fondo/*RGB(192,192,192)*/);
         i = 0;
         for (j=jini;j<jfin;j++)
         {
            k = 23*ancho+j;
         ...
         if (oa != a || og != g)
         {
            if (ntext)
            {
               ...
               if (ModoProfundo && (Patrib & 128))
               {
                  ...
                  DS_FillRect(hdc,&espacio,agi_brush);
         ...
         if (!(c = *(s+k)) || c == 219 || c == 176 || c == 177 || c == 178 || c == 1) {
            ...
            if (c == 1) /* tapiz */
            {
               ...
               if (win_fondo_neutro)
               {
                  ...
                  DS_FillRect(hdc,&espacio,agi_brush_gris);
               }
               else
               {
                  hOldBitmap = SelectObject(hMemoryDC,agitapiz);
...
*/   
     ...
     default:
        ...
        if(__wParam == PIDO_SUPERUSUARIO || __wParam == PIDO_VEREURO || __wParam == PIDO_SUPERBLOQUEO || __wParam == PIDO_SUPERDESBLOQUEO ||
		   __wParam == PIDO_SUPERCAMBIO  || __wParam == PIDO_SUPERTODO || __wParam == PIDO_SUPERDESTODO || __wParam == TECLA_COMBO || 
		   __wParam == ID_BUSCON || __wParam == ID_ACTIVAR_ERP || __wParam == ID_ACTIVAR_EVT || __wParam == ID_ACTIVAR_EVTPBX || __wParam == ID_ACTIVAR_EVTCRM)
        {            
               if (HaySeleccion == TRUE && wParam != MID_XCLIPCOPY)
               {
                  ClearSelection(principal_w, &Rect, (wParam & MK_SHIFT) ? SL_EXTEND | Shape : Shape);
...
int _c_log(int n) {
	...
	if (tralog != NULL) {
		...
		fprintf(tralog,"%s\r\n",buf);
		fflush(stdout);
		MessageBox(NULL,buf,"control llamada agisys",MB_OK);
	}
	*/
	return 0;
}
