...
#define POINT2MPOINT(pt, mpt)   ((mpt).x = (SHORT)(pt).x, (mpt).y = (SHORT)(pt).y)
    ...
    int tiempo;
		...
		int boton_flag(int nada,int flag)
		{
			return nada;
		}
...
/* que se desactivan pgup y pgdn */
{
   ...
   if (!menusg_virtual || menusg_virtual == CRETURN || menusg_virtual == -1)
   {
      ...
      else
      {
									if (lineas)      /* hay varias lineas  */
                           {
                              b_opcion[numero] = crea_boton_menusg(col[numero]/100,col[0]%100+numero*macol+2,"->",1,PGDN,0);
                              b_opcion[numero+1] = crea_boton_menusg(col[numero+1]/100,col[0]%100-2,"<-",1,PGUP,0);
                              for (i=0;i<numero;i++)
                              {
