...
for (i = 0;i < totalfichs;i++) {
   if (fichs[i]->fini >= 2000)
      {
      if (ya_esta_avisado < 2)
       {
       aviso(0,"No se pueden usar referencias a fichero en Agisystem 6.01P");
       ya_esta_avisado++;
	   }
	  }
...
for (i = 0;i < nprog;i++) {
   if (prog[i].i != 255 && (prog[i].i > 196 || prog[i].i == 150 || prog[i].i == 151 || prog[i].i == 154 || prog[i].i == 155
       || prog[i].i == 135 || prog[i].i == 136 || prog[i].i == 137 || prog[i].i == 138 || prog[i].i == 139
	   || prog[i].i == 140 || prog[i].i == 141 || prog[i].i == 142))
     {
      if (ya_esta_avisado < 2)
       {
	   char tmpxx[512];	   
	   sprintf(tmpxx,"Instruccion desconocida para Agisystem 6.01P --->%d<---",(unsigned int)prog[i].i);
       un_error(tmpxx,"");
       ya_esta_avisado++;
	   }
     }
...
if (nfi > 48)
   ...
   if (ya_esta_avisado < 2)
      {
      aviso(0,"Demasiados Ficheros para Agisystem 6.01P");
      ya_esta_avisado++;
	  }
   }
