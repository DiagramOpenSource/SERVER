...
FI_DEF *fi;
{
	...
	if (fi->f_fd == -1 || fi->grabable)
	{
		...
		if (fi->f_fd > -1)
		{
			rw_lseek_(fi->f_fd,1000000000L,SEEK_SET);
			locking(fi->f_fd,0,1L);	    
			rw_close_(fi->f_fd);
		}
		...
		if (fi->f_fd > -1) 
		{
			...
			rw_lseek_(fi->f_fd,1000000000L,SEEK_SET);
			if (locking(fi->f_fd,2,1L))
			{
				fi->grabable = 0;		   
				/* mal menor!! */
			}
			rw_lseek_(fi->f_fd,0L,SEEK_SET);
		}
	}
	else
	{	 
	...
	for (i=0;i<fi->f_ncampos;i++) {
		tamano += (unsigned int)rw_write_(file1,ca[i]->c_usi,
			strlen(ca[i]->c_usi)+1);
		tamano += (unsigned int)rw_write_(file1,ca[i]->c_mante->c_mensaje,
			strlen(ca[i]->c_mante->c_mensaje)+1);
		tamano += (unsigned int)rw_write_(file1,ca[i]->c_mante->c_defecto,
			strlen(ca[i]->c_mante->c_defecto)+1);
		tamano += (unsigned int)rw_write_(file1,ca[i]->c_minimo,
			strlen(ca[i]->c_minimo)+1);
		tamano += (unsigned int)rw_write_(file1,ca[i]->c_maximo,
	...
	
	{
		...
		rw_write_(file1,&suma,sizeof(short));
		rw_write_(file1,&checksum,sizeof(short));
	}
