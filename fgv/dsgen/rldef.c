...
int lee_def(char *nfi,FI_DEF *fi,int man)
{
	...
	if ((man/100) == 77) {
		...
		if ((file1 = rw_open_(nomfi,O_BINARY | O_RDONLY)) < 0) 
		    {
			return(-1);
			}
		}
	...
	pos_final = rw_lseek_(file1,0L,SEEK_END);
...
/*
	...
	rw_lseek_(file1,33L,SEEK_SET); /* inicio datos ... anterior cabecera */
    rw_read_(file1,fi->modulo,9);
    rw_read_(file1,(char *)&tamano,sizeof(short));
	if (tamano == FGVGEN_ID_MAS)
	    {
		rw_read_(file1,(char *)&fi->listable,sizeof(short));
		rw_read_(file1,(char *)&fi->generable,sizeof(short));
		}
	else
	    {
		fi->listable = 0;
	...
	rw_read_(file1,apuntador,i);


	tamano -= agi_carga_(fi->f_fichero,MAXPATH+1);
