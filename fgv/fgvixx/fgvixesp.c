...
static int dbf_crea_cabecera(char *nom,char *nombre,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef,int ixx_fd)
{
    int fd,xfd;
    char tmp[512];
    struct dbf_desc dbf;
            ...
            else
            {
                ...
                {          
					void *conrespaldo = 0;
					   ...
					   conrespaldo = respaldo_externo_ixx_init(ixx_fd,nom);
					#endif

                    ixx_dat_cierra(fd);
                    no_crear_otro = 1;
                    fd = dat_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
                    no_crear_otro = 0;
                    if (fd >= 0)
                    {			              
                        dlen = dtabla->reclen;
                        ...
                        {     
                            ...
                            no_crear_cabecera  = 1;
                            xfd = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
                            no_crear_cabecera  = 0;
                            if (xfd < 0)
                            {
                                free(tmp_buf);
                            ...
                            while(ixx_lee(fd,tmp_buf+1,dlen) == dlen)
                            {
                                ...
                                if (!tmp_buf[dlen]) {
									...
									dreg++;
									   ...
									   {
										   isrecnum = reg - dreg;
										   respaldo_externo_ixx_graba(conrespaldo,tmp_buf+1);
									   }
#endif
								}
...
int dbf_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef,int ixx_fd)
{
    int fd,xfd;
    ...
    {
        if (no_crear_cabecera || dbf_crea_cabecera(nom,nombre,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd) < 0)
        {
            return(-1);
        }
        fd = ixx_abre_dat(nombre);  
    ...
    if (fd >= 0)
    {
        ...
        if (ixx_lee(fd,&dbf,sizeof(dbf)) != sizeof(dbf) || (dbf.id != IXXDBFID && dbf.id != 0x3 && dbf.id != 0x30 /* && dbf.id != 0x83 */))
        {
            ...
            return(dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd));
        }
		/* auto correccion!! */
        ...
        if (difiere || faltan || sobran)
        {
            ...
            else
            {
                ...
                if (dbf_crea_cabecera(nom,nombre,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd) >= 0)
                {
                    recursion = 1;
                    fd_tmx = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
                    recursion = 0;
                    if (fd_tmx < 0)
                    {
                        free_dbf(i);
                ...
                while ((r = dbf_lee(i,reg,tmp_buf1,0 /* forzar lectura directa */)) >= 0)
                {
				   ...
				   {
					   isrecnum = reg - dreg;
					   respaldo_externo_ixx_graba(conrespaldo,tmp_buf2);
				   }
#endif
...
int dbf_lee(int fd,long reg,Uchar *buf,int len)
{
    ...
    */
    ...
    if (ixx_lee(fd,&c,1) != 1) eret(IXXEREAD,47000+errno,-1);
    if (c != '*')
    {
        if (ixx_lee(fd,local_buf,local_len) != local_len) eret(IXXEREAD,48000+errno,-1);
        if (dbf->cabeza.id != IXXDBFID && len)
        {
            /* conversion */
            offset1 = 0;
...
int dat_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef,int ixx_fd)
{
        ...
        if (fd < 0)
        {
            ...
            if (fdbtr == -1 && !no_crear_otro)
            {
                ...
                if (fd >= 0)
                {
                    ...
                    /* ojo hay un DBF */
                    no_crear_otro = 1;
                    fd = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
                    no_crear_otro = 0;					
                }
            }		  
            ...
            {			
                ...
                {					
                    ...
                    while ((r = dbf_lee(i,reg,tmpreg,len-1)) >= 0)
                    {
					   ...
					   {
						   isrecnum = reg - dreg;
						   respaldo_externo_ixx_graba(conrespaldo,tmpreg);
					   }
#endif
				...
				if (ixx_lock(ixxfic->fd,LK_NBLCK,1L) < 0)
				{
                    if ((errno == EACCES || errno == EAGAIN || errno == 13 || errno == 35))
                    {
					   ...
					   if (ixxind_recespera)
						  (*ixxind_recespera)(1);
					   sleep(5);
					   return dat_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
					}
					else
						ret( -1 );
				}
			...
			if (discrepa == 1)
			{
					...
					if (nfd >= 0)
					{
						...
						while(-1)
						{
							...
							if (tmpreg[tamreg]) {
									   ...
									   {
										   isrecnum = reg - dreg;
										   respaldo_externo_ixx_graba(conrespaldo,ntmpreg);
									   }
							}
#endif
				...
				{   
					...
					else
					{
						...
						no_report_error = 1;
						no_crear_cabecera = 1;
						xfd = dbf_abre(otro_nom,descamp,defectos,longi,ixxfic,&ojete,dtabla,pathdef,ixx_fd);
						no_crear_cabecera = 0;
						if (xfd < 0)
						{
							strcpy(nombre,nom);
...
int len;
{
    ...
    if (ixx_lseek(fd,pos,SEEK_SET) == -1L) eret(IXXELSEEK,errno,-1);
    if (ixx_lee(fd,buf,len) != len) eret(IXXEREAD,50000+errno,-1);
    if (ixx_lee(fd,&c,1) != 1) eret(IXXEREAD,51000+errno,-1);
    if (c) ret(0);
    ret(1);
}
...
int diu_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef, int ixx_fd)
{
    int i;
    int fd = (*ixxdiu_abre)(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
