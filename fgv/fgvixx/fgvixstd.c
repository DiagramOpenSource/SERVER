...
static int bloqueo_total = 0;
static int bloqueo_reindexa = 0;
static int lectura_multiple = 0;


static int (*_ixxmen_aviso)(char *);
static int (*_ixxind_aviso)(int,long,long);
static int (*_ixxind_recespera)(int);
static int (*_ixxdat_aviso)(int,long,char *);
...
static int  LCLAVE_POINTERS = 10;
static int  LCLAVE_TOTAL = 5;
static char VERSION_INDEX[10] = "0004   ";

extern double maximustremendus;

long IxxNumeroPosicion = 0L;


int ixxload_fd_nrecords = -1;
double *ixxload_nrecords = NULL;

/* estos buffers no estan depurados ... ni son necesarios */
#ifdef IXXCONBUFFERS 
...
static int ixx_buf_fin(int ixx_fd)
{
   ...
   for (i = 0;i < IXXMAXBUF;i++)
   {
      if (i < ixxBuffer->tope_buf && ixxBuffer->flag[i])
      {
         ...
         {
            (*ixxmen_aviso)(__Texto[161]);
         }
         if (fd < 0)
         {
            (*ixxmen_aviso)(__Texto[162]);
         }
...
int s,e,fd;
{
   ...
   */
   ...
   if (s == IXXEREAD) {
	   if (ixx_e_fich  >= 0 && ixxfichs[ixx_e_fich]) {   
		   no_report_error++;
		   if ((ixxfichs[ixx_e_fich]->flags_operacion & 1)) {
			   FILE *f;
			   char tmp2[256];
			   tmp2[0] = 0;
			   if (ixxfichs[ixx_e_fich]->nombreixx) {
				   strcpy(tmp2,ixxfichs[ixx_e_fich]->nombreixx);
			   }
			   f = fopen("c:\\infodslog.txt","a");
			   if (f) {
				   fprintf(f,"(%ld) ALERT[%d][%d](fd=%d)(ixx_e_fich=%d)%s\r\n",time(NULL),s,e,fd,ixx_e_fich,tmp2);
				   fclose(f);
			   }
			  bloqueo_reindexa = 1;
			  if (!ixx_desbloquea_lectura(ixxfichs[ixx_e_fich]->fd,ixxfichs[ixx_e_fich]->openid,ixxfichs[ixx_e_fich]->openmode)) 
				ixxfichs[ixx_e_fich]->flags_operacion &= ~1;		   
		   }
		   no_report_error--;
	   }   
   }
   ...
   if (s == IXXELSEEK || s == IXXEMULTIPLE) {
	   char tmp2[256];
	   bloqueo_total = 1;
	   tmp2[0] = 0;
	   if (ixx_e_fich  >= 0 && ixxfichs[ixx_e_fich] && ixxfichs[ixx_e_fich]->nombreixx) {
		   strcpy(tmp2,ixxfichs[ixx_e_fich]->nombreixx);
	   }
	   if (ixx_incidencia) {
		     sprintf(tmp,"ALERT[%d][%d](fd=%d)(ixx_e_fich=%d)%s",s,e,fd,ixx_e_fich,tmp2);
			(*ixx_incidencia)(0,tmp);
	   }
	   if (ixxmen_aviso) {
		   (*ixxmen_aviso)("Por razones de seguridad debe salir de esta tarea para abrir una nueva, debido a la sobrecarga del manejador de estructuras.");
	   }
	   exit(2);
   }  
   ...
   if (no_corrup_error && (s == IXXEREAD || s == IXXEWRITE || s == IXXELSEEK))
   {
	   s = IXXECORRUP;
   }
   ...
   if (s == IXXECORRUP && !no_corrup_error)
	   ...
	   {			  
			  if (ixxmen_aviso)
			  {
				 (*ixxmen_aviso)(__Texto[524]);
			  }
	   }
	   else
	   {
			  sprintf(tmp,__Texto[164],e);
			  if (ixxmen_aviso)
			  {
				 (*ixxmen_aviso)(tmp);
			  }
...
int ixx_abre_indice(nombre,modo,descamp,defectos,lfich,dtabla,pathdef)
char *nombre;
int modo;
char **descamp;
char *defectos;
...
char *pathdef;
{
    	IXXCABEZA header;
            ...
            else
            {
               if (ixxfichs[ixx_fd]->pseudo_cierra)
               {
                  if (ixxfichs[ixx_fd]->nombredat && !strcmp(nombre,ixxfichs[ixx_fd]->nombredat) && ixxfichs[ixx_fd]->nombreixx)
                  {
                     ...
                     ixxfichs[ixx_fd]->fd_dat = (*ixxdat_abre)(ixxfichs[ixx_fd]->nombredat,NULL,NULL,NULL,NULL,NULL,&(ixxfichs[ixx_fd]->dtabla),ixxfichs[ixx_fd]->pathdef,ixx_fd);
                     ixx_ojo_recursion = 0;
                     _e_log_open++;
#ifdef CONTROLNLM
                     pprint(2501,"        ");
		 ...
		 /*memset(ixxfichs[ixx_fd],0,sizeof(IXXFILE));*/
		 ...
		 if (dtabla)
			...
			memset(&(ixxfich->dtabla),0,sizeof(ixxfich->dtabla));
		 }
         ixxfich->tipo_datos = ixx_datos_dbf + default_dbf_id;
         ixxfich->flags_operacion = 0;

		 /*
         ixxfich->dtabla.Campo = (CampoTabla *)malloc(sizeof(CampoTabla) * ixxfich->dtabla.ncampos);
         memcpy(ixxfich->dtabla.Campo,dtabla->Campo,sizeof(CampoTabla) * ixxfich->dtabla.ncampos);
		 */
         /*ixxfich->dtabla.Campo = NULL;*/
         ...
         ixxfich->fd_dat = (*ixxdat_abre)(nombre,descamp,defectos,&(ixxfich->tamreg),ixxfich,&ojo_norec,&(ixxfich->dtabla),pathdef,ixx_fd);
         /* printf("punto 6\n");getchar(); */
#ifdef CONTROLNLM
         pprint(2501,"        ");
         sleep(1);
		 ...
		 ixxfich->pathdef = malloc(strlen(pathdef)+1);
         if (!ixxfich->pathdef)
         {
            ixx_ojo_recursion = 0;
            ixx_cierra_indice(ixx_fd);
		 ...
		 strcpy(ixxfich->pathdef,pathdef);
...
int fd,modo;
{
   ...
   while(-1)
   {
      ...
      if (errno != EACCES && errno != EAGAIN && errno != 13 && errno != 35) {
         eret(IXXEREAD,2000+errno,-1);
      }
	  if (ixx_lseek(fd,0L,SEEK_END) < LCKREZONAINI)
		  break;
      if (ixxind_recespera)
         (*ixxind_recespera)(0);
...
int ixx_nuevo_indice(nombre,nclaves,claves,tamreg,descamp,defectos,dtabla,pathdef)
char *nombre;
short nclaves;
...
char *pathdef;
{
   ...
   j = (total_len_clave + LCLAVE_POINTERS) * 30; /* era 20 y 1004 */
   if (j < 1004)
      j = 1004;
   else
      if (j > 8172)
      ...
      if (fd > -1)
      {
         ixx_cierra(fd);
         ixx_ojo_recursion = 0;
         if ( (fd = ixx_abre_indice(nombre,IXXCOMPARTE,descamp,defectos,tamreg,dtabla,pathdef)) >= 0)
         {
      ...
      nfd = ixx_abre_indice(nombre,IXXCOMPARTE,descamp,defectos,tamreg,dtabla,pathdef);
...
static int _IXX_reconstruye(ixx_fd,compactar)
int ixx_fd,compactar;
{   
   int fd,i,t,tamblock,sale_mal = 0,oldmode,preverr = ixxerrno,l;
   ...
   if (ixxfich->dtabla.ident && ixxfich->dtabla.ident[0] && strncmp(ixxfich->dtabla.ident,quita_path(ixxfich->nombredat),8)) {
       char tmpx1[512];
	   char tmpx2[512];
	   strcpy(tmpx1,ixxfich->dtabla.ident);
	   strcpy(tmpx2,quita_path(ixxfich->nombredat));
	   strmayus(tmpx1);
	   strmayus(tmpx2);
	   if (strcmp(tmpx1,tmpx2)) {
		   if (ixxmen_aviso) {
			   (*ixxmen_aviso)("Por razones de seguridad debe salir de esta tarea para abrir una nueva, debido a la sobrecarga del manejador de estructuras.");
		   }
		   exit(2);
...
#ifdef WIN32
   if (ixxfich->dtabla.ident && ixxfich->dtabla.ident[0] && GetFileNameFromHandle(fd,tmp) == TRUE) {
	   char *ppp;
	   quitab(tmp);
	   ppp = strstr(tmp,".ixx");
	   if (!ppp)
		   eret(IXXEMALFICH,0,-1);
	   *ppp = 0;	   
	   if (strncmp(ixxfich->dtabla.ident,quita_path(tmp),8)) {
		   char tmpx1[512];
		   char tmpx2[512];
		   strcpy(tmpx1,ixxfich->dtabla.ident);
		   strcpy(tmpx2,quita_path(tmp));
		   strmayus(tmpx1);
		   strmayus(tmpx2);
		   if (strcmp(tmpx1,tmpx2)) {
			   if (ixxmen_aviso) {
   ...
   if (ixxfich->nombreixx) {
	   ...
	   char tmpx3[512];
	   strcpy(tmpx3,ixxfich->nombreixx);
	   strcat(tmpx3,".ixx");
	   if (fstat( fd, &buffer1 ) != -1 && stat(tmpx3,&buffer2) != -1 && buffer1.st_ino != buffer2.st_ino) {
		   if (ixxmen_aviso) {
			   (*ixxmen_aviso)("Por razones de seguridad debe salir de esta tarea para abrir una nueva, debido a la sobrecarga del manejador de estructuras.");
		   }
		   exit(2);
   ...
   if (ixxfich->fd_dat < 0)
			{
      ixxfich->fd_dat = (*ixxdat_abre)(ixxfich->nombredat,NULL,NULL,NULL,NULL,NULL,&(ixxfich->dtabla),ixxfich->pathdef,ixx_fd);
			}
   else {
...
#ifndef DSDBODBC
   conrespaldo = respaldo_externo_ixx_init(ixx_fd,ixxfich->nombreixx);
#else
   conrespaldo = 0;
#endif
   ...
   if (ixx_lee(fd,&(ixxfich->cabecera.updatecount),sizeof(long)) != sizeof(long))
			{
      IXXerror(IXXEREAD,6000+errno,-1);
      sale_mal = 1;
      goto recon_sale;
			}

      if (ixx_lseek(fd,OTRANSN,SEEK_SET)== -1L)
      {
		  IXXerror(IXXEREAD,7000+errno,-1);
		  sale_mal = 1;
		  goto recon_sale;
      }
      if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
      {
		  IXXerror(IXXEREAD,8000+errno,-1);
		  sale_mal = 1;
		  goto recon_sale;
      }	  

    ixxfich->cabecera.numero_transaccion++;
   ...
   if (reg > 0L)
      ...
      while(i)
      {
         ixxfich->cabecera.nrecords = reg-dreg;
         no_read_error = 1;
         ...
         else
         {
            ...
            if (dreg)
            {
               (*ixxdat_graba)(ixxfich->fd_dat,reg-dreg,bloque,ixxfich->tamreg,0);
            }
            */
            isrecnum = reg - dreg;
            if (!ixx_graba_indice(ixx_fd,bloque,IXXGSOLOI))
            {
			   ...
			   {
				   isrecnum = reg - dreg;
				   respaldo_externo_ixx_graba(conrespaldo,bloque);				   
			   }
#endif
            }
            else
            {
               ixxlibera_registro(reg);
               (*ixxdat_borra)(ixxfich->fd_dat,reg - dreg,ixxfich->tamreg);
            }
         }
   ...
   for (i = 0;i < (clave->clave_ncampos-mode);i++)
   {
      ...
      switch (tip)
                  ...
                  case IXXCDOUBLE:  /* DOUBLE TIPO */
					  /*
					 if (*((double *)datos_clave) >= maximustremendus)
					 {
						 return -99999;
					 }
					 */
...
long ixxnuevo_registro_datos()
{
   ...
   if ( blo )
      ...
      {
         IXXerror(IXXEREAD,14000+errno,-1);
         return( 0L );
      }
...
long ixxnuevo_bloque()
{
         ...
         if ( blo )
         {
            ...
            {
               IXXerror(IXXEREAD,16000+errno,-1);
               return( 0L );
            }
...
int modo;
{
   ...
   {
      ixx_ojo_recursion = 0;
      eret(IXXEMALARGU,3,-1);
   }
   
   if (modo != IXXGNUEVO && modo != IXXGACTUA && modo != IXXGSOLOI && modo != IXXGPOSIC) 
   {
      ixx_ojo_recursion = 0;
      eret(IXXEMALARGU,4/*1*/,-1);
   }
   ...
   /******************************************/
    if (ixx_haydiu)
	{
		acceso_a_ciu(ixxfich->fd_dat,ixxfich->nombredat);
	}
   ...
   if (ixxload_nrecords && modo == IXXGNUEVO && ixxload_fd_nrecords == ixx_fd) {
	   double rrrr;
	   rrrr = (double)time(NULL);
	   rrrr *= 10000;
	   rrrr += (double)((ixxfich->cabecera.nrecords+1)%10000);
	   *ixxload_nrecords = rrrr;
   }
   ...
   if (!solo_indice || ixxfich->openmode != IXXEXCLUYE)
   {
      ...
      {
         IXXerror(IXXEREAD,22000+errno,-1);
         sale_mal = 1;
         goto graba_sale;
      }
      if (ixx_lseek(fd,OTRANSN,SEEK_SET)== -1L)
      ...
      if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
      {
         IXXerror(IXXEREAD,23000+errno,-1);
         sale_mal = 1;
         goto graba_sale;
      }	  
   }
   ...
   if (rwupdate != ixxfich->cabecera.updatecount)
   {
      ...
      {
         IXXerror(IXXEREAD,24000+errno,-1);
         sale_mal = 1;
         goto graba_sale;
      }
   ...
   if (modo == IXXGACTUA)
   {
      ...
      */
      ...
      if ((*ixxdat_clave)(ixxfich,Dat_Cur,Datos2,-1,reg,0,0,0) < 0)
	  {
         IXXerror(IXXEMALARGU,5,-1);
         sale_mal = 1;
         goto graba_sale;
      }
      ...
      if ((*ixxdat_clave)(ixxfich,dat_reg,Datos,-1,reg,0,0,0) <0)
	  {
         IXXerror(IXXEMALARGU,6,-1);
         sale_mal = 1;
         goto graba_sale;
	  }
   ...
   for (laclave = 0;laclave < ixxfich->nclaves;laclave++)
   {
	  ...
	  {
         if ((*ixxdat_clave)(ixxfich,dat_reg,Datos,laclave,reg,0,0,0)< 0)
		 {
			 IXXerror(IXXEMALARGU,7,-1);
			 sale_mal = 1;
			 goto graba_sale;
		 }		 
	  }
      ...
      if (t == 3)
      {
         ...
         {
            IXXerror(IXXEREAD,25000+errno,-1);
            sale_mal = 1;
            goto graba_sale;
         }
         if (!blo)
...
graba_sale:  
  ...
  if (por_posicion)
  {		 
	 for (;ixxfich->cabecera.nrecords < (reg-1);)
	 {
		 (ixxfich->cabecera.nrecords)++;
		 /*(*ixxdat_graba)(ixxfich->fd_dat,ixxfich->cabecera.nrecords,dat_reg,ixxfich->tamreg,2);*/
		 (*ixxdat_borra)(ixxfich->fd_dat,ixxfich->cabecera.nrecords,ixxfich->tamreg);
		 /*ixxlibera_registro(ixxfich->cabecera.nrecords);*/
		 /*(ixxfich->cabecera.registros_reservados)++;*/
	 }
	 if (reg > ixxfich->cabecera.nrecords)
	 {
  ...
  {
	  if (!sale_mal && (!solo_indice || ixxfich->openmode != IXXEXCLUYE))
	  {		  
		  ixxfich->cabecera.numero_transaccion++;
		  if (ixx_lseek(fd,OTRANSN,SEEK_SET)== -1L)
		  {
          ...
          if (ixx_graba(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
		  {
               IXXerror(IXXEWRITE,errno,-1);
               sale_mal = 1;
               goto graba_salextra;
...
graba_salextra:  
  ...
  if (!sale_mal && !solo_indice)
  {
     ...
     if ((*ixxdat_graba)(ixxfich->fd_dat,reg,dat_reg,ixxfich->tamreg,flagd) < 0)
	  {
        sale_mal = 1;
	  }
     else
	  {
		 ...
		 if (0)
		 {
			 /* sistema asegurate, a ampliar el fichero cerramos y abrimos */        
...
int modo;
{
   ...
   {
      ixx_ojo_recursion = 0;
      eret(IXXEMALARGU,8,-1);
   }
   
   if (modo != IXXBCLAVE && modo != IXXBACTUA)
   {
      ixx_ojo_recursion = 0;
      eret(IXXEMALARGU,9/*1*/,-1);
   }
   ...
   if (ixx_lee(fd,(unsigned char *)&rwupdate,sizeof(long)) != sizeof(long))
   {
      IXXerror(IXXEREAD,26000+errno,-1);
      sale_mal = 1;
      goto borra_sale;
   }
   ...
   if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
   {
      IXXerror(IXXEREAD,27000+errno,-1);
      sale_mal = 1;
      goto borra_sale;
   }
   ...
   if (rwupdate != ixxfich->cabecera.updatecount)
   {
      ...
      if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera),sizeof(IXXCABEZA)) != sizeof(IXXCABEZA))
      {
         IXXerror(IXXEREAD,28000+errno,-1);
         sale_mal = 1;
         goto borra_sale;
      }
...
static int __ixx_pon_contenido(int ixx_fd,Uchar *con,int len)
{
   int n;
   int nlen;   
...
int len;
{
   ...
   {	   
	   ...
	   {		   
		   c = *p1;
		   *p1 = 0;
		   __ixx_pon_contenido(ixx_fd,conte,olen);
		   *p1 = c;
		   olen = 0;
		   p1 += 2;
		   conte = p1;
   ...
   if (olen)
   {
	   return __ixx_pon_contenido(ixx_fd,conte,olen);
   }
   return 0;
}
...
int flags;
{
   ...
   int orig_queclave = queclave;
   /*static int pillabug = 0;*/
...
repeticion_extrana:
      ...
      if (ixx_lee(fd,(unsigned char *)&rwupdate,sizeof(long)) != sizeof(long))
      {
         IXXerror(IXXEREAD,29000+errno,-1);
         sale_mal = 1;
         goto lee_sale;
      }
      if (ixx_lseek(fd,OTRANSN,SEEK_SET)== -1L)
      ...
      if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
      {
         IXXerror(IXXEREAD,30000+errno,-1);
         sale_mal = 1;
         goto lee_sale;
      }
      ...
      if (rwupdate != ixxfich->cabecera.updatecount)
      {
         ...
         if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera),sizeof(IXXCABEZA)) != sizeof(IXXCABEZA))
         {
            IXXerror(IXXEREAD,31000+errno,-1);
            sale_mal = 1;
            goto lee_sale;
         }
      ...
      if (mode != IXXNEXT && mode != IXXPREV && mode != IXXCURR && mode != IXXFIRST && mode != IXXLAST)
      {
         ...
         if ((*ixxdat_clave)(ixxfich,dat_reg,Datos,laclave,pos,0,parte,mode) < 0)
		 {
            sale_mal = 1;
            goto lee_sale;
		 }
...
caso_especial_no_estructurado:
           ...
           {             
              ...
              do {
                 if (!repitio)
                 {
                    ...
                    if (ixx_lee(fd,ixxbloque,ixxfich->tamblock) != ixxfich->tamblock)
                    {
                       IXXerror(IXXEREAD,32000+errno,-1);
                       sale_mal = 1;
                       goto lee_sale;
                    }
                    if (*((short *)ixxbloque) != IXXIDBCLAVE)
                 ...
                 else
                 {
                    if (posiavance)
                    {
                       if (mode == IXXNEXT)
                       {
                          ...
                          { /* saltar duplicados */
							  if (clave->clave_tipo == IXXCLAVEDUP)
							  {
								  if (!levelcon)
								  {
								  ...
								  {
									  /* en un nivel superior solo se puede saltar duplicados  
										 si hay mas de dos bloques duplicados ya que en el ultimo
										 puplicado pueden haber otras claves */
									  while(posiori < (20+(nkeys-2)*(len+LCLAVE_POINTERS)) && !ixxcompara_clave(ixxbloque3+20,ixxbloque+posiori+(len+LCLAVE_POINTERS),clave,1))
									  {
										  posiori += (len+LCLAVE_POINTERS);
									  }
								  }
                ...
                if (mode == IXXCURR)
                {
                   ...
                   if (ojo)
                   {
					  ...
					  if (ojo == 2)
					  {
                          if (ixxcompara_clave(ixxfich->DatosCurr[laclave],ixxbloque+posiori,clave,0) != 0)
                          {
                             IXXerror(IXXECORRUP,150,-1); /* ultimocorrup */
                             sale_mal = 1;
                             goto lee_sale;
                          }
					  }
        ...
        if (mode == IXXFIRST || mode == IXXLAST)
        {
           ...
           if (ixx_lee(fd,(unsigned char *)&pos,sizeof(long)) != sizeof(long))
           {
              IXXerror(IXXEREAD,33000+errno,-1);
              sale_mal = 1;
              goto lee_sale;
           }
           if (!pos)
           ...
           do {
              ...
              if (ixx_lee(fd,ixxbloque,ixxfich->tamblock) != ixxfich->tamblock)
              {
                 IXXerror(IXXEREAD,34000+errno,-1);
                 sale_mal = 1;
                 goto lee_sale;
              }
              if (*((short *)ixxbloque) != IXXIDBCLAVE)
...
lee_sale:
        ...
        if (ixx_lee(fd,(unsigned char *)&rwupdate,sizeof(long)) != sizeof(long))
        {
           IXXerror(IXXEREAD,35000+errno,-1);
           sale_mal = 1;
           goto lee_salextra;
        }
...
int i;
{
   ...
   if (i >= 0 && i < IXXMAXFILES && ixxfichs[i])
      ...
      {
         ixxfich->fd_dat = (*ixxdat_abre)(ixxfich->nombredat,NULL,NULL,NULL,NULL,NULL,&(ixxfich->dtabla),ixxfich->pathdef,i);
      }
			}
   return(0);
}
...
int r = 0;
  ...
  if (lectura_multiple == 1) {
	_ixxmen_aviso = ixxmen_aviso;
	_ixxind_aviso = ixxind_aviso;
	_ixxind_recespera = ixxind_recespera;
	_ixxdat_aviso = ixxdat_aviso;
	ixxmen_aviso = nada_de_nada;
	ixxind_aviso = nada_de_nada;
	ixxind_recespera = nada_de_nada;
	ixxdat_aviso = nada_de_nada;
  }
...
int fd;
  ...
  {     
	 ixxfich->flags_operacion &= ~1;
	 lectura_multiple--;
	  if (lectura_multiple == 0) {
		ixxmen_aviso = _ixxmen_aviso;
		ixxind_aviso = _ixxind_aviso;
		ixxind_recespera = _ixxind_recespera;
		ixxdat_aviso = _ixxdat_aviso;
	  }
     ret(-1);
  }
...
int o_mode;
  ...
  if (ixx_lee(fd,(unsigned char *)&rwupdate,sizeof(long)) != sizeof(long))
  {
     IXXerror(IXXEREAD,39000+errno,-1);
     sale_mal = 1;
     goto posi_sale;
  }
  ...
  if (ixx_lee(fd,(unsigned char *)&(ixxfich->cabecera.numero_transaccion),sizeof(long)) != sizeof(long))
  {
     IXXerror(IXXEREAD,40000+errno,-1);
     sale_mal = 1;
     goto posi_sale;
  }
  ...
  pos += 9;
    ...
    if (ixx_lee(fd,(unsigned char *)&pos,sizeof(long)) != sizeof(long))
    {
      IXXerror(IXXEREAD,41000+errno,-1);
      sale_mal = 1;
      goto posi_sale;
    }
    if (!pos)
    ...
    do
    {
        ...
        if (ixx_lee(fd,ixxbloque,tamblock) != tamblock)
        {
           IXXerror(IXXEREAD,42000+errno,-1);
           sale_mal = 1;
           goto posi_sale;
        }
        ...
        if (!level)
        {
            ...
            if ((*ixxdat_clave)(ixxfich,dat_reg,ixxfich->DatosCurr,-1,pos,0,0,0) < 0)
			{
               IXXerror(IXXECORRUP,3,-1);
               sale_mal = 1;
               goto posi_sale;
			}
            memcpy(ixxfich->PosCurr,_Posblock,sizeof(short)*IXXMAXLEVEL);
...
int ixx_a_version_4_()
{
LCLAVE_POINTERS = 10;
LCLAVE_TOTAL = 5;
strcpy(VERSION_INDEX,"0004   ");
return(0);
}

int ixx_lee_datos(ixx_fd,dat_reg,reg)
int ixx_fd;
Uchar *dat_reg;
long reg;
{   
   ...
   return (*ixxdat_lee)(ixxfich->fd_dat,reg,dat_reg,ixxfich->tamreg);
}


int ixx_graba_datos(ixx_fd,dat_reg,reg)
int ixx_fd;
Uchar *dat_reg;
long reg;
{   
   ...
   if (ixxfich->cabecera.nrecords < reg)
	   ixxfich->cabecera.nrecords = reg;
   return (*ixxdat_graba)(ixxfich->fd_dat,reg,dat_reg,ixxfich->tamreg,2);
}

int ixx_borra_datos(ixx_fd,reg)
int ixx_fd;
long reg;
{   
   /*AjustaPunterosSegunDatos(ixx_fd);*/
   ...
   if (ixxfich->cabecera.nrecords < reg)
	   ixxfich->cabecera.nrecords = reg;
   return (*ixxdat_borra)(ixxfich->fd_dat,reg,ixxfich->tamreg);   
}


int ixx_directo_abre_datos(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef,int ixx_fd)
{
	return (*ixxdat_abre)(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef,ixx_fd);
}

int ixx_directo_cierra_datos(int fd)
{
	return (*ixxdat_cierra)(fd);
}

int ixx_directo_tamfichero_datos(int fd,int tamreg,long total,long *reales)
{
	return (*ixxdat_tamfichero)(fd,tamreg,total,reales);
}

int ixx_directo_lee_datos(int fd,long reg,Uchar *buf,int len)
{
	return (*ixxdat_lee)(fd,reg,buf,len);
}

int ixx_directo_graba_datos(int fd,long reg,Uchar *buf,int len)
{
	return (*ixxdat_graba)(fd,reg,buf,len,0);
}

int ixx_directo_borra_datos(int fd,long reg,int len)
{
	return (*ixxdat_borra)(fd,reg,len);	
}
