...
int in, out;   /* input and output file descriptors */
{
   ...
   while (insize != 0 && (int)insize != EOF) {
      ...
      if (read_interno)
      {
         insize = (*read_interno)(&in, (char*)inbuf, INBUFSIZ);	  
      }
      else
         insize = rw_base_read_(in, (char*)inbuf, INBUFSIZ);
   }
...
int eof_ok;          /* set if EOF acceptable as a result */
{
         ...
         do {
            if (read_interno)
            {
               len = (*read_interno)(&ifd, (char*)inbuf+insize, INBUFSIZ-insize);
            }
            else
               len = rw_base_read_(ifd, (char*)inbuf+insize, INBUFSIZ-insize);
         ...
         if (insize == 0) {
            if (eof_ok || abandonalo_ya) return EOF;
            read_error();
         }
         bytes_in += (ulg)insize;
         inptr = 1;
...
unsigned  cnt;
{
   ...
   if (write_interno)
   {
      (*write_interno)(fd, buf, cnt);
      return;
   }
   while ((n = rw_base_write_(fd, buf, cnt)) != cnt) {
      if (n == (unsigned)(-1)) {
         write_error();
						}
      cnt -= n;
