...
char **paqs;
{
    ...
    {   
		sprintf(tmp,__Texto[169],lpServName,PORTRWNETCON);
		aviso(0,tmp);     
		closesocket(rwsock);
		rwsock = INVALID_HANDLE_VALUE;
#ifdef FGVDOS
    ...
    sprintf(tmp,__Texto[223],server_request+4);
    mensaje(0,tmp);  	
	...
	closesocket(rwsock);  
	rwsock = INVALID_HANDLE_VALUE;
	sprintf(tmp,__Texto[224],lpServName);
	mensaje(0,tmp);
    ...
    {
		sprintf(tmp,__Texto[174],lpServName,RWPORTIP);
		aviso(0,tmp);
    ...
    sprintf(tmp,__Texto[225],lpServName);
    mensaje(0,tmp);
...
static int actualiza_este_ini(char *nuevo)
{
	...
	if (!file)
	{
		strcpy(nom,rw_directorio("agi.ini"));
...
char **argv;
{
	...
	ini_sig();
	cls();
	sprintf(tmp,__Texto[178],rtme_version1,rtme_version2,rtme_versionletra);
	mensaje(0,tmp);
	_nvolume = 0;
	...
	if (salvapar == (char *)0) 
	{
		...
		if (argc == 1)
		{
			...
			{
				do {
					pprint(2401,__Texto[423]);
					server_request[4] = 0;
					i = entra(2438,32,"","",server_request+4,1,server_request+4);
					if (i == 1 || i == 7)
					{
						if (confirmar(2440,__Texto[441],__Texto[448]))
						{
							abandonar = 1;
							goto insagifinal;
						}
			...
			{		
				...
				else
				{
					...
					do {
						...
						pprint(2401,__Texto[424]);
						i = entra(2433,32,"","",origen,1,origen);
						if (i == 1 || i == 7)
						{
							if (confirmar(2440,__Texto[425],__Texto[448]))
							{
								abandonar = 1;
								goto insagifinal;
							}
	 ...
	 else
     {
		 ...
		 Bl(24);
		 do {
			 pprint(2401,__Texto[426]);
			 i = entra(2427,32,"","",tmp,1,tmp);
			 if (i == 1 || i == 7)
			 {
				 if (confirmar(2440,__Texto[442],__Texto[448]))
				 {
					 abandonar = 1;
					 goto insagifinal;
				 }
   ...
   if (Eliminador_Diagram && puede < 1)
   {
	   ...
	   if (instalacion_silenciosa < 2)
	   {
		   mensaje(0,__Texto[450]);
		   if (!instalacion_silenciosa)
		   {
			   if (confirmar(2400,__Texto[443],__Texto[447]))
			   {
				   puede = 1;
			   }	  
			   else
			   {
				   pito(185);pito(185);pito(185);pito(185);
				   if (confirmar(2400,__Texto[444],__Texto[447]))
				   {
					   puede = 1;
				   }	  
			   }
			   ...
			   if (confirmar(2440,__Texto[445],__Texto[448]))
			   {
				   puede = 1;
			   }	  
	   }
	   ...
	   if (hay_que_copiar || ext != -1)
	   {
		   ...
		   do 
		   {
			   ...
			   if (ext != -1)
			   {
				   ...
				   {
					   sprintf(tmp,__Texto[184],quita_path(argv[i]),secu);
					   aviso(0,tmp);
					   continue;
				   }
			   }
			   ...
			   if (err)
			   {
				   if (!_nvolume)
				   {
					   sprintf(tmp3,__Texto[229],quita_path(argv[i]));
					   if (instalacion_silenciosa < 2)
						   mensaje(0,tmp);
					   mi_volume = 0;
					   break;
				   ...
				   {
					   mi_volume++;		   
					   sprintf(tmp3,__Texto[230],mi_volume,tmp);
					   tmp3[80] = 0;
					   mensaje(0,tmp3);pito(186);
				   }
				   /* posibilidad de cambio */
				   strcpy(tmp3,origen);
				   do {
					   pprint(2401,__Texto[427]);
					   j = entra(2435,32,"","",tmp3,1,tmp3);
					   if (j == 1 || j == 7)
					   {
						   if (confirmar(2440,__Texto[446],__Texto[448]))
						   {
							   abandonar = 1;
							   goto insagifinal;
						   }
...
Sin_COPIAR:
	  ...
	  else
	  {
		  if (instalacion_silenciosa < 2)
		  {       
			  sprintf(tmp,__Texto[187],quita_path(argv[i]));
			  aviso(0,tmp);
			  break;
		  }
...
int era_server;
{
	...
	for (i=2;i<j;i++) {
		if (*(salvapar+i)=='$') {
			hasta = i;
			for(l=i+1,c=0;l<j && *(salvapar+l)!='$';l++,c++)
				pathmen[c]=*(salvapar+l);
			pathmen[c]='\0';
			for(k=l+1,c=0;k<j && *(salvapar+k)!='$';k++,c++)
				dirmen[c]=*(salvapar+k);
			dirmen[c]='\0';
			for(l=k+1,c=0;l<j && *(salvapar+l)!='$';l++,c++)
				dirbase[c]=*(salvapar+l);
			dirbase[c]='\0';
			break;
		}
	}
	*(salvapar+hasta)='\0';
	...
	if (strlen(server_request))
	{
		...
		args[nn]=server_request;
		nn++;
	}
	
	if (strlen(other_server))
	{  
		args[nn]=other_server;
		nn++;
	}
	...
	if (era_server)
	{
		...
		if (CreateProcess(NULL,tmp,NULL,NULL,FALSE,NORMAL_PRIORITY_CLASS | CREATE_NEW_PROCESS_GROUP | DETACHED_PROCESS,NULL,NULL,&stinfo,&proi) != FALSE)
		{
			exit(0);
		}  
	}
	...
	Terinici();
	sprintf(salva,__Texto[232],pathmen,errno);
	mensaje(0,salva);
	parada();
	Terfinal();
	/* rw_error(0); */
