...
static int carga_sqllink()
{
	...
	if (sqlsrv_con_sql)
	{
		...
		{
			int i;
			strcpy(buffer,sqlsrv_con_sql+1);
			i = strlen(buffer) - 1;
			if (i >= 0)
			{
				if (buffer[i] == '"')
...
static char **sql_lista_filtro = NULL;
static int filtro_fds[IXXMAXFILES];

int respaldo_carga_filtro()
{
    FILE *f = fopen(sql_externo_filtro,"r");
    if (f)
    {
        char tmp_nom[128];
        int lineas = 0,kk;
        ...
        rewind(f);
        sql_lista_filtro = (char **)malloc(sizeof(char *)*(lineas+1));
        kk = 0;
        while(kk < lineas && fgets(tmp_nom,120,f))
        {
			...
			if (tmp_nom[0])
			{
				sql_lista_filtro[kk] = malloc(strlen(tmp_nom)+1);
				strcpy(sql_lista_filtro[kk],tmp_nom);
				kk++;
			}
        }
        sql_lista_filtro[kk] = NULL;
        fclose(f);
		return 1;
    }
	return 0;
...
static char *da_memoria_respaldo(int memoria)
{
  if (!respaldo_buffer)
  {
...
{   
	...
	if (!sql_externo_filtro || (filtro = respaldo_pasa_fd(fd)))
	{
...
{ 
	...
	if (sql_externo_filtro)
	{
		...
		if (!respaldo_pasa_filtro(pfi->f_fichero,pfi->f_fd))
		{
			return 0;
		}
	}
...
{
	if (!sql_externo_filtro || respaldo_pasa_fd(pfi->f_fd))
	{
		int memoria = sizeof(int)*3+sizeof(short)+pfi->f_lfich;
		char *buffer = da_memoria_respaldo(memoria);
