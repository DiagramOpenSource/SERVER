...
extern char bufpan[PANBUFSIZE];     /* buffer de pantalla */
...
static char ult_fispool[MAXPATH]  = ""; 
static char ult_ejspool[MAXPATH] = "";
static char ult_fispo[MAXPATH]    = "";
...
#define Pseudo10 1
#define fgv_crear_plantilla_doc 0
#define fgv_ver_euro 0
...
FILE *pfich;
{
         ...
         for (i = 0;i < nfich;i++) {
            ...
            if (text[k] == '#') {
               ...
               if (f < 0) {
                  if (nfi >= MAXDEF) {printf("demasiados def en informe");pito(20);continue;}
                  if (nfi == 48) nfi++;                  
...
int memoria = sizeof(FI_DEF_LEE) + MAXPATH + sizeof(CAMPO_DEF) * fi->f_ncampos + sizeof(KEY_DEF) * fi->f_nclaves;
int i;
char *p,*buf;
  ...
  *((int *)p) = memoria;
  p += sizeof(int);
  *((short *)p) = FGVGEN_ID2_MAS;
  p += sizeof(short);

  *((FI_DEF_LEE *)p) = *((FI_DEF_LEE *)fi);
  p += sizeof(FI_DEF_LEE);

  memcpy(p,fi->f_fichero,MAXPATH);
  p += MAXPATH;
...
FI_DEF *ajusta_inline_temp_pfi(char *p,int memoria,int tipo)
{	
    FI_DEF *fi;
    int i;
	int tam;
	int extra = 0;
    if (tipo == FGVGEN_ID2_MAS) {
		extra = MAXPATH;
	}
	tam = memoria+((sizeof(FI_DEF)-sizeof(FI_DEF_LEE))-extra)+((FI_DEF_LEE *)p)->f_lfich + 1;
	fi = (FI_DEF *)malloc(tam);
	memset((char *)fi,0,tam);
	memcpy((char *)fi,p,sizeof(FI_DEF_LEE));
    if (tipo == FGVGEN_ID2_MAS) {		
		memcpy(fi->f_fichero,p+sizeof(FI_DEF_LEE),extra);
	}
	else {
		memcpy(fi->f_fichero,fi->f_oldfichero,sizeof(fi->f_oldfichero));		
	}
	memcpy(((char *)fi)+sizeof(FI_DEF),p+sizeof(FI_DEF_LEE)+extra,memoria-(sizeof(FI_DEF_LEE)+extra));
	...
	fi->f_buf = ((char *)fi) + memoria + ((sizeof(FI_DEF)-sizeof(FI_DEF_LEE))-extra);
    return fi;
}
...
int remoto_maneja_def_temporal(char *buf)
{
	...
	case FGVGEN_ID2_MAS:
	case FGVGEN_ID_MAS:
		pfi = ajusta_inline_temp_pfi(p,memoria,id);
		if (pfi)
		{
                        ...
                        else if (no_memo_leer)
                        {
						   ...
						   else
						   {
							   ...
							   {								 
									 ...
									 {
										 double anal = 1.0;
										 (*asigna_euro)("EURO",&anal);
										 anal = redondea(anal,3);
										 if (anal == 166.386)
										 {
											if (fgv_ver_euro)
												eseuro = 0;
...
{   
        ...
        argv[npar++] = getenv("COMSPEC");

        argv[npar++] = "/c";
        argv[npar++] = (char *) tmpcmd;
		...
		if (tmp[i] == ' ') {
			i = strlen(tmp);
			len = i;
			if (!memcmp(tmpcmd,tmp,i)) {
				j = 0;
				_catch = _chdir(tmp);
				//tmpcmd[j++] = '"';
				//memcpy(tmpcmd+j,command,len);
				//j += len;

				do  {
					while(command[i] && command[i] != ' ') tmpcmd[j++] = command[i++];
					//tmpcmd[j++] = '"';
					if (command[i] == ' ') {
						tmpcmd[j++] = ' ';
						i++;
						while(command[i] && command[i] == ' ') tmpcmd[j++] = command[i++];
						if (!memcmp(command+i,tmp,len)) {
							//argv[npar++] = tmpcmd+j;
							//tmpcmd[j++] = '"';
							//memcpy(tmpcmd+j,command+i,len);
							//j += len;
							i += len;
						}
						else {
...
#ifdef FGVWIN
                  fin_w_printer();
               if (!fispo[0] || (fispo[0] == 2 && fispo[1] == 0) || (fispo[0] == 3 && fispo[1] == 0))
               {
				  if (fispo[0] == 2 || fispo[0] == 3)
				  {
					  nombredev[0] = 2;
					  nombredev[1] = 0;
					  strcpy(nombredev+2,otrospool);
...
char *memo5009;
{
   ...
   if (nombredev[0] == 3 && nombredev[1] == 0)
   {
	   sinacabo = 1;
	   nombredev[0] = 0;
   }   
   ...
   if (_imp == (FILE *)0)
   {
        ...
        {	 
		   char tmp[512];
		   tmp[0] = 0;
           (*DespachadorExterno)((int)DESPACHO_LISTADO,2,tmp);
        }
   }
