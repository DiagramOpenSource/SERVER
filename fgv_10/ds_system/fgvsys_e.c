...
#ifdef CONORACLE
extern int oracle_manager(char *buffer);
#endif

#ifdef FGVDOS
int (WINAPI *fnDspython)(char *ejecuta,char *pars,void *callback);
#else
int (*fnDspython)(char *ejecuta,char *pars,void *callback);
#endif
void *AccesoPython(int f,void *p,int *flag);
#ifndef FGVDOS
static void *plib = (void *)0;
#else
static HANDLE plib = NULL;
#endif
...
int (*siscal[256])() = {/* (!) == Cuidado con estos ASCII */
...
/*
 "RECIBE_FICHERO"   ,"9595"          ,240,NULL,
 "ABRE_WORD"        ,"9895"          ,241,NULL,
 "SALVA_WORD"       ,"9895"          ,242,NULL,
 "PROPIEDADES_WORD" ,"989595"        ,243,NULL,
 "CARGA_TEXTO_WORD" ,"989895"        ,244,NULL,                 
*/      

    extern int fgv_ventana();
    extern int fgv_finventana();
	extern int fgv_pcontexto();
...
int saca_dato_del_def(char *dato,M_VAL *val,int *f0,int *c0)
{
   ...
   *c0 = 0;
   ...
   {
       val->vald = (double)v_mpfi(f)->f_ipfi;
       tipo = DOUBLETIPO;
   }
   else if (!strcmp(dato,"ID"))
   {
       val->vald = (double)v_mpfi(f)->f_id;
       tipo = DOUBLETIPO;
   }
   else if (!strcmp(dato,"IDGRID"))
   {
       val->vald = (double)v_mpfi(f)->f_idgrafico;
       tipo = DOUBLETIPO;
   }
   else
   {
       ...
       if (dato[0] == 'C')
       {
           if (numero >= 0 && numero < v_mpfi(f)->f_ncampos)
           {
               *c0 = numero;
               ...
               else if (!strcmp(dato,"WID"))
               {
				   if (v_mpfi(f)->f_campowids)
				   {
					   val->vald = (double)v_mpfi(f)->f_campowids[numero];
				   }
				   else
				   {
					   val->vald = (double)-1;
               ...
               else if (!strcmp(dato,"TAB"))
               {
				   if (v_mpfi(f)->f_taborder)
				   {
					   val->vald = (double)v_mpfi(f)->f_taborder[numero];
				   }
				   else
				   {
					   val->vald = (double)-1;
               ...
               else if (!strcmp(dato,"ORDEN"))
               {
				   if (v_mpfi(f)->f_camporden)
				   {
					   val->vald = (double)v_mpfi(f)->f_camporden[numero];
				   }
				   else
				   {
					   val->vald = (double)-1;
               ...
               else if (!strcmp(dato,"FLAGORDEN"))
               {
				   if (v_mpfi(f)->f_orderflags)
				   {
					   val->vald = (double)v_mpfi(f)->f_orderflags[numero];
				   }
				   else
				   {
					   val->vald = (double)-1;
...
*/
       ...
       else if (dato[0] == 'P')
       {
           if (numero >= 0 && numero < v_mpfi(f)->f_npanta)
           {
               ...
               if (!strcmp(dato,"WID"))
               {
				   if (v_mpfi(f)->f_pantaids)
				   {
					   val->vald = (double)v_mpfi(f)->f_pantaids[numero];
				   }
				   else
				   {
				       val->vald = (double)-1;
			   ...
			   else if (!strcmp(dato,"SID"))
			   {
				   if (v_mpfi(f)->f_pantasids)
				   {
					   val->vald = (double)v_mpfi(f)->f_pantasids[numero];
				   }
				   else
				   {
				       val->vald = (double)-1;
			   ...
			   else if (!strcmp(dato,"PATH"))
			   {
                   strcpy(val->valc,v_mpfi(f)->f_pantalla[numero].p_nombrepan);
                   tipo = CHARTIPO;
			   }
			   else if (!strcmp(dato,"CINI"))
			   {                   
				   val->vald = (double)v_mpfi(f)->f_pantalla[numero].p_campoini;
                   tipo = DOUBLETIPO;
			   }
			   else if (!strcmp(dato,"CFIN"))
			   {                   
				   val->vald = (double)v_mpfi(f)->f_pantalla[numero].p_campofin;
                   tipo = DOUBLETIPO;
			   }
			   else if (!strcmp(dato,"PREF"))
			   {                   
				   val->vald = (double)v_mpfi(f)->f_pantalla[numero].p_posref;
                   tipo = DOUBLETIPO;
			   }
			   else if (!strcmp(dato,"PFIN"))
			   {                   
				   val->vald = (double)v_mpfi(f)->f_pantalla[numero].p_posfin;
                   tipo = DOUBLETIPO;
			   }
           }
       }
	   ...
	   if (dato[0] == 'X')
	   {
		   ...
		   {
			   val->vald = (double)v_mpfi(f)->f_controles[numero];
               tipo = DOUBLETIPO;
		   }
	   }
   }
               ...
               if ( fr < 0 ) {
                  if (sal == 0) {
                     if (i == LETRA || i == ESFECHA) {
						extern int control_ojo_al_dado ;						
                        if (no_memo_interna(cr)) {
						   control_ojo_al_dado = 1;
                           free(v_memo(cr)->alfabetica);
						   control_ojo_al_dado = 0;
						...
						/*
						if (control_ojo_al_dado == 2) {
							if (strcmp(resu.valc,"EUR") && strcmp(resu.valc,"   ") && strcmp(resu.valc,"")) {
								aviso(0,"ojo la dato");
								tracer_int = 1;
							}
						}
...
int faclin;
{   
   sprintf(v_memo(5004)->alfabetica,"%d",pinppan(v_prog(prog[p].indice%100),prog[p].indice/100));
   return(0);
}
...
{   
   ...
   sprintf(v_memo(5004)->alfabetica,"%d",_pinppan(v_prog(prog[p].indice%100),prog[p].indice/100,(int)v_memo(prog[p+1].indice)->numerica,0));
   n_np = 2;
   return(0);
}
...
{      
    ...
    BorraDescendencia(v_mpfi(ipfi)->f_pantaids[n],default_id,(ipfi+1)+n*1000,-1,-1);
...
int faclin;
{
   ...
   if (v_mpfi(ff)->f_campopos[ cc ] != -1 && v_mpfi(ff)->f_campopos[ cc ] != v_mpfi(ff)->f_campos[cc].c_mante.c_qp)
   {
      cursor(v_mpfi(ff)->f_campopos[ cc ]);
      printcam_p(v_mpfi(ff),(cc + 1));
   }
   else
   {   
...
int faclin;
{
   ...
   v_scroll(-1,-1,par[0],par[1],par[2]);
   n_np = 3;
   return(0);
}
...
int faclin;
{
   ...
   if (modo) {
	  if (f >= 0 && f < nfi && f != 48 && v_mpfi(f)->f_bufcompara && c >= 0 && c < v_mpfi(f)->f_ncampos)
		  v_mpfi(f)->f_crccampos[c] = -1;
   }
   ...
   if (i >= 36 && i <= 42) 
   {
      ...
      case 37:
         if (modo)
		 {
			...
			if (r)
			{
			   if (DespachadorExterno && v_mpfi(f)->f_campowids[c] > -1)
			   {
				   (*DespachadorExterno)((int)DESPACHO_DESTRUYECONTROL,v_mpfi(f)->f_campowids[c],NULL);
				   v_mpfi(f)->f_campowids[c] = -1;
			   }
			}
		 }
         else
      ...
      case 43:
         if (modo)
         {
            v_mpfi(f)->f_campopos[ c ] = r;
            /*v_mpfi(f)->f_campos[c].c_mante.c_qp = r;*/
         }
         else
         {
            r =  v_mpfi(f)->f_campopos[ c ];
      ...
      if (modo)
      {
          int cwid = v_mpfi(f)->f_campowids[c];
          if (cwid != -1)
          {
             pinta_campo(f,c+1,faclin,0);
          }
      }
      */
...
int faclin;
{
	     unsigned int c = (unsigned int)v_memo( prog[p++].indice )->numerica; /* caracter */
         unsigned int f = (unsigned int)v_memo( prog[p++].indice )->numerica; /* fondo */
         Color(c,f);
         n_np = 2;
         return(0);
}
...
int faclin;
{
   ...
   if (!memcmp(v_memo(5004)->alfabetica,"::",2))
   {
	   char *p;
	   strcpy(tmp,v_memo(5004)->alfabetica+2);
	   control = tmp;
	   for (p = tmp;*p;p++)
	   {
		   if (*p == ',')
   ...
   i = (unsigned int)parada_(control,foco);
...
{  
   ...
   {      
      ...
      sal = strlen(tmp);
      tmp[sal] = 1;
	  if (v_memo( d )->alfabetica[0] == ':') {
		  tmp[sal+1] = 'C';
		  strcpy(tmp+sal+2,v_memo( d )->alfabetica);
	  }
	  else
...
int fgv_i_pdir(char *tmp)
{
   ...
   if (dd)
   {
      ...
      if (_buf)
      {
         strcpy(quita_path(tmp),_buf->d_name);
         sal = 0;
      }
      else
      {
...
int faclin;
{
   ...
   char tmp[300];
   char conjava[300];
   static char versionjava[300] = "";
   conjava[0] = 0;
   if ((remoto_es_sistema_grafico & 128)) {
	   strcpy(conjava," JAVA ");
#ifdef CONJAVA
	  if (DespachadorExterno && !versionjava[0]) {	  	 
		 (*DespachadorExterno)((int)DESPACHO_MULTIPLE,778,versionjava);		 
	  }
	  strcat(conjava,versionjava);
#endif
	   
   }
   sprintf(tmp,"%d.%02d%c%s-%d",rtme_version1,rtme_version2,rtme_versionletra,conjava,numero_build);
   if (no_memo_interna(o))
   {
      free(v_memo(o)->alfabetica);
      v_memo(o)->alfabetica = strmalloc(strlen(tmp)+1);
...
char *version;
{
   ...
   if (!file)
   {
      aviso(0,"Fichero de relacion de defs no accesible");
      return(-1);
   }
   ...
   {   
      ...
      if (!r)
      {
         defecto_ficha(_pfi);
         cls();
         cabecera_v(NULL,"",_pfi->f_titulo,empresa);	   
...
char *version;
{
   ...
   if (!file)
			{
      aviso(0,"Fichero de relacion de menu no accesible");
      return(-1);
			}
   if (leemenu(menu) < 0)
			{
      fclose(file);
      aviso(0,"Menu no accesible");
      return(-1);
			}
   while(lee_linea(tmp,255,file))
			{   
...
int faclin;
{
   ...
   if (!buffer)
   {
						aviso(0,"Por fallo en la memoria no hay claves!!");
                  n_np = 1;
                  return(0);
   }
   ...
   if (ll > 8000L)
   {
						sprintf(buffer,"%s es demasiado grande, hay que quitarle comentarios.",tmp);
                  aviso(0,buffer);
                  free(buffer);
                  n_np = 1;
                  return(0);
   ...
   {
#ifndef FGVDOS
						aviso(0,"El sistema dll de momento solo disponible en entorno win32");
#else
                  en_daclaves(buffer);
                  recibido = 1;
#endif
...
int faclin;
{
   ...
   if (!fgv_activa_progdll)
   {
      if (!fgv_activa_progdll_aviso)
      {
         ...
         aviso(0,"Falta Instalar el Modulo para Enlace Excel");
      }
      n_np = 1;
      return(0);
   }
...
{   
   ...
   {   
      ...
      if (mis_controles[d])
      {
         p_ejecuta(mis_controles[d],O_NOR,0,14);
      }
   }
}
...
int faclin;
{
   if (DespachadorExterno)
   {	  
      int control = (int)v_memo(prog[p].indice)->numerica;         
	  /*
      if (control > 0 && control <= 100 && mis_controles[control-1])
      {
		 control--;
	  ...
	  */
	  {
		 (*DespachadorExterno)((int)DESPACHO_DESTRUYECONTROL,control,NULL);
		 agi_cola_timer();
	  }
   }
   n_np = 1;
...
int sinbotones = 0;
   ...
   if (!strcmp(v_memo(5004)->alfabetica,"::ENDATOS::SINBOTONES::")) {
    ...
    sprintf(v_memo(5004)->alfabetica,"%d",pregun_clave(&clave,f,faclin,modo,sinbotones));
...
int faclin;
{
    ...
    if (DespachadorExterno)
    {
        (*DespachadorExterno)((int)DESPACHO_FOCOCONTROL,control,NULL);
        /*agi_cola_timer();*/
    }
    n_np = 1;
    return 0;
...
int faclin;
{
    ...
    if (DespachadorExterno)
    {       
       (*DespachadorExterno)((int)DESPACHO_REFRESCACONTROL,control,NULL);         
       /*agi_cola_timer();*/
    }
    n_np = 1;
    return 0;
...
void FGVHookControl(char *bloque,control_pars *par,int el_punto)
{
	...
	if (((control_pars *)RetroPar[el_punto])->structid != 0x3113)
		return;	
    ...
    if (operacion == 999)
    {
		...
		if (par->sesion_id == ap_sesion_id && par->ipfi > 0 && par->ipfi <= nfi) {
			int ipfi = par->ipfi-1;
			int campo = par->campo;
			if (campo >= 0 && campo < v_mpfi(ipfi)->f_ncampos && v_mpfi(ipfi)->f_campowids[campo] == par->id) {
    ...
    if (par->evento)
    {
		strcpy(v_memo(5001)->alfabetica,bloque);
...
int faclin;
{
   ...
   if (DespachadorExterno)
   {
	  ...
	  if (tmpxx[4] > -1)
	  {		  
		  if (creacampo && oipfi >= 0 && oipfi < nfi)
		  {
			  if (par) {
				  par->ipfi = oipfi+1;
				  par->pan = pan;
			  ...
			  if (n < v_mpfi(oipfi)->f_ncampos) {
				  v_mpfi(oipfi)->f_campowids[n] = tmpxx[4];
				  v_mpfi(oipfi)->f_campos[n].c_mante.c_modif = 1;
			  }
			  else {
				  inserta_campo(v_mpfi(oipfi),n,"","",0,"","","");
				  InsertaCampoEnPantalla(oipfi,pan,n,9999,tmpxx[4]);
			  }
			  if (conteclaoevento)
				  v_mpfi(oipfi)->f_campos[n].c_mante.c_modif = 0;
		      if (v_mpfi(oipfi)->f_nclaves)
			  {       
			     char tmp[512];
			     strcpy(tmp,"CUSTOM1001"); /* 1001 Desactivar mantenimento */
			     (*DespachadorExterno)((int)DESPACHO_ESTADOCONTROL,tmpxx[4],(void *)tmp);
			  }
		  }		  
	  }
      sprintf(v_memo(5004)->alfabetica,"%d",tmpxx[4]);      
   }
   n_np = 5;
   return 0;
}
...
int faclin;
{
   ...
   if (DespachadorExterno)
   {
      ...
      char *pc;
      px1[1] = default_id;
      px1[2] = (int)v_memo(prog[p++].indice)->numerica;
      if (px1[2] == -1)
      {
          px1[2] = default_sid;
      }
      px1[3] = (int)v_memo(prog[p++].indice)->numerica;
      px1[4] = (int)v_memo(prog[p++].indice)->numerica;
      pc = v_memo(prog[p++].indice)->alfabetica;
      px1[5] = (int)v_memo(prog[p++].indice)->numerica;

      px1[0] = sizeof(int)*6 + strlen(pc)+1;
      strcpy((char *)(px1+6),pc);
      (*DespachadorExterno)((int)DESPACHO_BOTONSALIDA,0,(void *)tmp);
	  tecla = px1[5];
	  (*DespachadorExterno)((int)DESPACHO_DIF_FOCOCONTROL,tecla,NULL);
	  tecla = leecar_foco(1,default_id,-1,-1,-1,-1,-1,NULL);
   }
...
{
   char tmp[5000];
   int funcion = (int)v_memo(prog[p++].indice)->numerica;
   int m1 = prog[p++].indice;
   int m2 = prog[p++].indice;
   int m3 = prog[p++].indice;
   int m4 = prog[p++].indice;
   int m5 = prog[p++].indice;
   int *p1 = (int *)tmp;

   p1[0] = sizeof(int)*2 + 2500;
   p1[1] = 0;

   strcpy(tmp+sizeof(int)*2,v_memo(m1)->alfabetica);
   strcpy(tmp+sizeof(int)*2+500,v_memo(m2)->alfabetica);
   strcpy(tmp+sizeof(int)*2+1000,v_memo(m3)->alfabetica);
   strcpy(tmp+sizeof(int)*2+1500,v_memo(m4)->alfabetica);
   strcpy(tmp+sizeof(int)*2+2000,v_memo(m5)->alfabetica);
   if (DespachadorExterno)
	(*DespachadorExterno)((int)DESPACHO_MAINCOMMAND,(int)funcion,tmp);
   sprintf(v_memo(5004)->alfabetica,"%d",p1[1]);
   memo_strcpy(m1,tmp+sizeof(int)*2);
   memo_strcpy(m2,tmp+sizeof(int)*2+500);
   memo_strcpy(m3,tmp+sizeof(int)*2+1000);
   memo_strcpy(m4,tmp+sizeof(int)*2+1500);
   memo_strcpy(m5,tmp+sizeof(int)*2+2000);
   n_np = 6;
   return 0;
}
...
int faclin;
{
	if (DespachadorExterno)
	{
		int entero = (int)v_memo(prog[p++].indice)->numerica;
	    (*DespachadorExterno)((int)DESPACHO_ESTADOCONTROL,entero,v_memo(prog[p++].indice)->alfabetica);
	}
    n_np = 2;
    return 0;
}
...
static int saca_var(char *n)
{
  int i,j;

  if (strcmp(n,"Campo") == 0) return(0); 
  if (strcmp(n,"Contenido") == 0) return(1);
  if (strcmp(n,"Control") == 0) return(2);
  if (strcmp(n,"FEntrada") == 0) return(3);
  if (strcmp(n,"FSalida") == 0) return(4);
  if (strcmp(n,"E_sup") == 0) return(5);
  if (strcmp(n,"E_inf") == 0) return(6);
  if (strcmp(n,"Pos") == 0) return(7);
  if (strcmp(n,"Usuario") == 0) return(8);
  if (strcmp(n,"Impresora") == 0) return(9);
  if (strcmp(n,"PARAMETRO") == 0) return(10);
  if (strcmp(n,"Fichero@") == 0) return(11);
  if (strcmp(n,"PARAMETRO_LOCAL") == 0) return(12);
...
static int saca_campo(char *campo,int *icampo)
{
	char tmp[512];
	char *p;
	int nf;
	...
	if (tmp[0] == '#')
	{
		...
		*icampo = atoi(p);
		for (nf = 0;nf < nfi;nf++)
		{
			if (!strcmp(tmp+1,quita_path(v_mpfi(nf)->f_PathDef)))
			{
				return nf;
			}
		}
		for (nf = 0;nf < nfi;nf++)
		{
			if (!strcmp(tmp+1,quita_path(v_mpfi(nf)->f_fichero)))
			{
				return nf;
			}
		}
	...
	{		
		...
		*icampo = atoi(p);
		for (nf = 0;nf < nfi;nf++)
		{
			if (!strcmp(tmp,quita_path(v_mpfi(nf)->f_PathDef)))
			{
				return nf;
			}
		}
		for (nf = 0;nf < nfi;nf++)
		{
			if (!strcmp(tmp,quita_path(v_mpfi(nf)->f_fichero)))
			{
				return nf;
			}
		}
...
void *AccesoPython(int f,void *p,int *flag) // acceso directo para el servidor, para que sea lo mas rapido posible
{
	static char tmp[512];
	switch(f)
	{
	case  0:
		{
			int var = saca_var((char *)p);
			if (var > 0)
			{
				if (m_ex[var].m_tipo == DOUBLETIPO)
				{
					*flag = 3;
					return (void *)&( m_ex[var].mv.vald );
				}
				*flag = 0;
				return (void *)m_ex[var].mv.valc;
			}
		}
		break;
	case 1:
		{
			int f,c = -1;
			f = saca_campo((char *)p,&c);
			if (f >= 0 && f < nfi && c >= 0)
			{
				*flag = v_mpfi(f)->f_campos[c].c_tipo;
				return (void *)(v_mpfi(f)->f_buf+v_mpfi(f)->f_campos[c].c_offset);
			}
		}
		break;	
    case 2:
	...
	case 3:
		{
		    ...
		    for (i = 0;i < np_ex;i++)
			{
			   if (strcmp(p,p_ex[i].nombre) == 0) 
			   {
			 	  p_ejecuta(p_ex[i].indice,O_NOR,0,16);
				  *flag = p_ex[i].indice+1;
				  return NULL;
			   }
			}	
			break;
		}
	case 4: /* leetecla ... desde interface pillada en diagramx */
		{

		}
	/* del 500 al 998 reservados para diagramx.exe */
	case 999:
		{
		    strcpy(tmp,rw_directorio("python/"));
#ifndef FGVDOS
		    strcat(tmp,"libdspython.so");
#else
		    strcat(tmp,"dspython.dll");
	...
	return "problemas en el pseudo entorno python-basico";
}
...
int faclin;
{
   ...
   else if (!strcmp(ins,"CREA_EMPRESA_MYSQL"))
   {
   ...
   else if (!strcmp(ins,"CARGAPYTHON"))
   {
	  char *modo = v_memo(i++)->alfabetica;
	  strmayus(modo);	  
	  if (!strcmp(modo,"SERVIDOR"))
	  {	  
		  if (!plib)
		  {		
			  strcpy(tmp,rw_directorio("python/"));
			  ...
			  strcat(tmp,"libdspython.so");
			  dlerror();
			  plib = dlopen(tmp,RTLD_LAZY); 
			  if (plib)
			  {
				 dlerror();
				 fnDspython = (int (*)(char *,char *,void *))dlsym(plib, "fnDspython");
				 if (!fnDspython)
					  sprintf (tmp, "%s", dlerror());
			  }
			  else
				  sprintf (tmp, "%s", dlerror());
#else    
			  ...
			  strcat(tmp,"dspython.dll");
			  cambia_barra_dos(tmp);		  
			  plib = LoadLibrary(tmp);
			  if (plib)
			  {
				  fnDspython = (int (WINAPI *)(char *,char *,void *))GetProcAddress(plib,"fnDspython");
				  if (!fnDspython)
					  strcpy(tmp,"no se encuentra fnDspython");
			  }
			  else
			  {
					LPVOID lpMsgBuf;
			  ...
			  strcpy(tmp,"Python ya cargado");
	  }
	  else
		  strcpy(tmp,"??? Solo modo SERVIDOR disponible.");
	  strcpy(v_memo(5004)->alfabetica,tmp);
      n_np = 2;
      return 0;
   }
   else if (!strcmp(ins,"DESCARGAPYTHON"))
   {
	   if (plib)
	   {

		    if ((*fnDspython)("","-D",(void *)AccesoPython) == 9999999)
			{
#ifndef FGVDOS
				dlclose(plib);
				plib = (void *)0;
				...
				plib = NULL;
#endif
  				strcpy(v_memo(5004)->alfabetica,"0 ok");
				n_np = 2;
				return 0;
			}
	   }
   }
   else if (!strcmp(ins,"LLAMAPYTHON"))
   {
	 if (plib && fnDspython)
	 {
		 char *eje = v_memo(i++)->alfabetica;
		 char *args = v_memo(i++)->alfabetica;
		 r = (*fnDspython)(eje,args,(void *)AccesoPython);
		 if (r == 9999999)
		 {
#ifndef FGVDOS
			dlclose(plib);
   ...
   else if (!strcmp(ins,"AUTOCOMPLETARFIJO"))
   {
	   int f1,c1;
	   int f2,c2;
	   int wcid;
	   ...
	   if (que_memo_tipo(i) == DOUBLETIPO)
	   {
			f1 = (int)v_memo(i)->numerica;
	   }
	   else
	   {
		   f1 = ind_rel(v_memo(i)->alfabetica);
	   }
	   i++;
	   c1 = (int)v_memo(i++)->numerica;
	   if (que_memo_tipo(i) == DOUBLETIPO)
	   {
		   f2 = (int)v_memo(i)->numerica;
	   }
	   else
	   {
		   f2 = ind_rel(v_memo(i)->alfabetica);
	   }
	   i++;	   
	   c2 = (int)v_memo(i++)->numerica;
	   altura = (int)v_memo(i++)->numerica;
 	   if (f1 >= 0 && f1 < nfi)
	   {
		   if (c1 >= 0 && c1 < v_mpfi(f1)->f_ncampos)
		   {
			   wcid = v_mpfi(f1)->f_campowids[c1];
			   if (wcid < 0)
			   {
				   strcpy(v_memo(5004)->alfabetica,"El campo 1 no esta creado en un dialogo");
			   }
 			   else if (f2 >= 0 && f2 < nfi)
			   {
				   if (c2 >= 0 && c2 < v_mpfi(f2)->f_ncampos)
				   {
					   char buffer[10000];
					   char *pbuf;
					   int a_fd,fd,len,nr,ciclo,total;
					   int faltan;
					   a_fd = v_mpfi(f2)->f_fd;
					   v_mpfi(f2)->f_fd = -1;
					   fd = abre_ind(v_mpfi(f2),ISINOUT + ISMANULOCK);
					   if (fd >= 0)
					   {						   
						   ciclo = 0;						   
						   total = 0;
						   ...
						   {
							   nr = 0;
							   pbuf = buffer+sizeof(int)*4;
							   len = 0;							   
							   if (ciclo || _consulta_lee_first(v_mpfi(f2)) >= 0/*lee_reg(v_mpfi(f2),ISFIRST)*/ >= 0)
							   do
							   {
								   ld_campo(c2,v_mpfi(f2),tmp);
								   quitab(tmp);
								   strcpy(pbuf,tmp);							   
								   pbuf += (strlen(tmp)+1);
								   len += (strlen(tmp)+1);
								   nr++;
								   total++;
								   if (_consulta_lee_next(v_mpfi(f2)) < 0)
								   {
									   faltan = 0;
									   break;
								   }
							   } while(len < (sizeof(buffer)-1024));

							   len += (sizeof(int)*4);
							   memcpy(buffer,&len,sizeof(int));
							   memcpy(buffer+sizeof(int),&wcid,sizeof(int));
							   memcpy(buffer+sizeof(int)*2,&nr,sizeof(int));
							   memcpy(buffer+sizeof(int)*3,&altura,sizeof(int));
							   
							   if (DespachadorExterno)
							   {
								   if (ciclo)
									   (*DespachadorExterno)((int)DESPACHO_MULTIPLE,13,buffer);
								   else
								       (*DespachadorExterno)((int)DESPACHO_MULTIPLE,12,buffer);
							   }
							   ciclo++;
						   } while(faltan);

						   cierra_ind(v_mpfi(f2));
						   v_mpfi(f2)->f_fd = a_fd;

						   sprintf(v_memo(5004)->alfabetica,"Enviados %d campos",total);
					   }
					   else
						   strcpy(v_memo(5004)->alfabetica,"No se abre fichero 2");
				   }
				   else
					   strcpy(v_memo(5004)->alfabetica,"campo 2 incorrecto!");
			   }
			   else 
				   strcpy(v_memo(5004)->alfabetica,"fichero 2 incorrecto!");
		   }
		   else
			   strcpy(v_memo(5004)->alfabetica,"campo 1 incorrecto!");
	   }
	   else
		   strcpy(v_memo(5004)->alfabetica,"fichero 1 incorrecto!");

	   n_np = 2;
	   return 0;
   }
   else if (!strcmp(ins,"ESTADO_VENTANA"))
   {
	  int wid,id = -1,sid = -1,f,n;
	  long modo = 1;	  
   	  long tam = 0;
	  char *opcion;
	  char *ptmp;

	  wid = (int)v_memo(i++)->numerica;
	  f = (int)v_memo(i++)->numerica;
	  n = (int)v_memo(i++)->numerica;
	  id = (int)v_memo(i++)->numerica;
	  opcion = v_memo(i++)->alfabetica;

	  if (f < nfi)
	  {
		   if (f >= 0 && (n < 0 || n >= v_mpfi(f)->f_npanta))
		   {
			   strcpy(v_memo(5004)->alfabetica,"Pantalla fuera de rango!");
		   }
		   else
		   {
			  if (f >= 0)
			  {
				  id = v_mpfi(f)->f_id;
				  sid = v_mpfi(f)->f_pantasids[n];
			  }
			  else if (wid == -1 && f == -1)
			  {				  
				  sid = n;
			  }
			  tam = sizeof(long)*7 + strlen(opcion) + 1;
			  ptmp = tmp;
			  memcpy(ptmp,&tam,sizeof(long));ptmp+=sizeof(long);	  
			  memcpy(ptmp,&tam,sizeof(long));ptmp+=sizeof(long);
			  memcpy(ptmp,&modo,sizeof(long));ptmp+=sizeof(long);

			  memcpy(ptmp,&wid,sizeof(int));ptmp+=sizeof(int);
			  memcpy(ptmp,&id,sizeof(int));ptmp+=sizeof(int);
			  memcpy(ptmp,&sid,sizeof(int));ptmp+=sizeof(int);
			  strcpy(ptmp,opcion);ptmp += strlen(opcion);

			  if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_BINARIO,(int)7,tmp);

			  memcpy(&wid,tmp+(sizeof(long)*3),sizeof(long));
			  sprintf(v_memo(5004)->alfabetica,"%d",wid);	   
		   }
	  }
	  else
		  strcpy(v_memo(5004)->alfabetica,"Referencia a def erronea!");
    
	  n_np = 2;
	  return 0;
   }
   else if (!strcmp(ins,"MUEVE_PANTALLA"))
   {
	   int f,n,pos,kk,iniref,campo;
	   f = (int)v_memo(i++)->numerica;
	   n = (int)v_memo(i++)->numerica;
	   pos = (int)v_memo(i++)->numerica;	   
 	   if (f >= 0 && f < nfi)
	   {
		   if (n < 0 || n >= v_mpfi(f)->f_npanta)
		   {
			   strcpy(v_memo(5004)->alfabetica,"Pantalla fuera de rango!");
		   }
		   else
		   {
			   iniref = v_mpfi(f)->f_pantalla[n].p_posref;
			   v_mpfi(f)->f_pantalla[n].p_posref = pos;			   
			   for (kk = v_mpfi(f)->f_pantalla[n].p_campoini;kk <= v_mpfi(f)->f_pantalla[n].p_campofin;kk++)
			   {
				   campo  = v_mpfi(f)->f_taborder[kk-1];
				   v_mpfi(f)->f_campopos[campo] = (v_mpfi(f)->f_campos[campo].c_mante.c_qp-iniref)+pos;
			   }               
			   strcpy(v_memo(5004)->alfabetica,"Ok!");
		   }
	   }
	   else
		  strcpy(v_memo(5004)->alfabetica,"Fichero fuera de rango");
	   n_np = 2;
	   return 0;
   }
   else if (!strcmp(ins,"GETDATACONTROL"))
   {
	  int cwid = -1;
	  int f,c;
	  f = (int)v_memo(i++)->numerica;
	  c = (int)v_memo(i++)->numerica;
	  if (f < 0)
	  {
		  cwid = c;
	  }
	  ...
	  *((int *)tmp) = MAXDEFECTO + sizeof(int) * 2 + 1;
	  ...
	  strcpy(v_memo(5004)->alfabetica,tmp+sizeof(int)*2);
	  n_np = 2;
	  return 0;
   }
   else if (!strcmp(ins,"COMMANDCONTROL"))
   {	  
	  char tmpxx[512];
	  int *p1 = (int *)tmpxx;
	  int cwid = (int)v_memo(i++)->numerica;
	  int codigo = (int)v_memo(i++)->numerica;
	  char *dato = v_memo(i++)->alfabetica;
	  memset(tmpxx,0,sizeof(tmpxx));
	  p1[0] = /*sizeof(int)*3 + strlen(dato) + 1*/500;
	  p1[1] = ID_COMMANDCONTROL;
	  p1[2] = codigo; 
	  strcpy((char *)(p1+3),dato);
	  /*aviso(0,dato);*/
	  if (DespachadorExterno)
		(*DespachadorExterno)((int)DESPACHO_COMMANDCONTROL,cwid,(void *)tmpxx);
	  /*aviso(0,"vuelta del despacho");*/
	  sprintf(v_memo(5004)->alfabetica,"%d",p1[2]);
	  ((char *)(p1+3))[249] = 0;
	  strcpy(v_memo(5001)->alfabetica,(char *)(p1+3));
	  n_np = 2;	 	  
	  return 0;
   }
   ...
   else if (!strcmp(ins,"JAVA"))
   {
	   /* url\nclase[(int:1,String:"jj")]\nret metodo()*/
	   tmp[0] = 0;
	   if (DespachadorExterno)
	   {		    
		   strcpy(tmp,v_memo(i)->alfabetica);
		   strcat(tmp,"\n");
		   strcat(tmp,v_memo(i+1)->alfabetica);
		   strcat(tmp,"\n");
		   strcat(tmp,v_memo(i+2)->alfabetica);
		   (*DespachadorExterno)((int)DESPACHO_MULTIPLE,669,tmp);
	   }
	   tmp[250] = 0;
	   memo_strcpy(i,tmp);
	   strcpy(v_memo(5004)->alfabetica,tmp);
   ...
   */
   ...
   {	  
	   CargaVInterface(v_memo(i)->alfabetica,(unsigned long)atoi(v_memo(i+1)->alfabetica) & 0x7fffffff);
	   r = 0;
   }
   else if (!strcmp(ins,"IDBOTONTECLA"))
   {
	   extern int dame_id_botontecla(int nivel);
	   r = dame_id_botontecla((int)v_memo(i)->numerica);
	   v_memo(i+1)->numerica = (double)r;	   
   }
...
#ifdef RWSERVER
	   extern char remoto_dirbass[MAXPATH];
	   /*memo_strcpy(i,cambia_espacios(remoto_dirbass,0))*/;
...
int fgv_ventana(p,tip,faclin)
int p;
int tip;
int faclin;
{
	int pi = pasa_a_int(v_memo(prog[p++].indice)->numerica);
	int pf = pasa_a_int(v_memo(prog[p++].indice)->numerica);
	int id = pasa_a_int(v_memo(prog[p++].indice)->numerica);
	int flag = pasa_a_int(v_memo(prog[p++].indice)->numerica);
	char *nombre = v_memo(prog[p++].indice)->alfabetica;
    extern int sin_replicante;

	sin_replicante = default_sid;
	 nuevo_pseudo_id(pi,pf,nombre,flag,id);
	sin_replicante = -1;

	sprintf(v_memo(5004)->alfabetica,"%d",default_id);

	n_np = 5;
	return 0;
}

int fgv_finventana(p,tip,faclin)
int p;
int tip;
int faclin;
{
...
int fgv_pcontexto(p,tip,faclin)
int p;
int tip;
int faclin;
{
	...
	if (!modo)
	{
		v_memo(prog[p].indice)->numerica   = default_wid;
		v_memo(prog[p+1].indice)->numerica = default_id;
		v_memo(prog[p+2].indice)->numerica = default_sid;
	}
	else
	{
		default_wid = (int)v_memo(prog[p].indice)->numerica;
		default_id = (int)v_memo(prog[p+1].indice)->numerica;
		default_sid = (int)v_memo(prog[p+2].indice)->numerica;
	}
	n_np = 4;
	return 0;
}

int fgv_oracle(p,tip,faclin)
int p;
int tip;
int faclin;
{
    int i = pasa_a_int(v_memo(prog[p].indice)->numerica);
	int len = 0;
	int n,j,k;
	char *buffer;
	...
	for (k = 0;k < n;k++)    
	{
		len += strlen(v_memo(j++)->alfabetica);
	}
	len++;
	buffer = malloc(len);
	buffer[0] = 0;
	while(n > 0)
	{
		strcat(buffer,v_memo(i++)->alfabetica);
		n--;
	}
#ifdef CONORACLE
	sprintf(v_memo(5004)->alfabetica,"%d",oracle_manager(buffer));
#else
	sprintf(v_memo(5004)->alfabetica,"%d Oracle no implementado en este runtime",-1);
#endif
...
p_ejecuta(int t,int tip,int faclin,int control)
{
   ...
   else 
   {
      ...
      if (proc_tabla[t] != programa_activo)
      {
         ...
         {
            /*aviso(0,"Error: No se puede acceder a un PRC.");*/
		   {
			   FILE *f;
			   char tmpc[512];
			   sprintf(tmpc,"%saviso64_X.txt",rw_directorio("logs/"));
   ...
   if (p < 0 || p >= nprog) {printf("llamada directa erronea(mal enlace de rutina)");getchar();return(-1);}
   ...
   {      
	  ...
	  //if (!strcmp(quita_path(en_pr[programa_activo].prc),"pcegz001") && p == 2214) {
	//	  tracer_int = 1;
	  //}
      ...
      if (mimodo_debug)
      {
       ...
       {
          printf("Error no hay programa_activo");getchar();
       }
