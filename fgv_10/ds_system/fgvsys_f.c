...
static void mensaje_secuencia()
{
	...
	//mensaje_en_cabecera("SECUENCIA %10.10s",saca_num_secuencia(),0);
}
                     ...
                     if (m == 1) 
                     {
                        ...
                        if (la_doble != -1) /* doblete */
                        {
                           *regis = 0L;
                           ...
                           res = leer_reg(ipfi,leo,bloq,trato);
                        ...
                        if (res < 0) 
                        {
                           ...
                           {
                              leer_reg(ipfi,leo,0,trato);m = 1;*regis = 0L;continue;/* pasar */
                           }
                        }
...
short ipfi,modo,*calc,tipo,base,z,faclin,m;
{
   ...
   while(*calc != -32535 && operaerror == 0) {
      if (((*calc) / base) == tipo) {
         ...
         {
            p_ejecuta(en_pr[programa_activo].tabla_gosubs[i],z,faclin,18);
         }
         if (m ==  0) return(-1);
      }
      calc++;
