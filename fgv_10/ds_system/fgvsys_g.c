...
extern char user_batch[110];
extern char user_batch_psw[110];

extern char nomapli[25];

extern char original_rw_dirbase[MAXPATH];


char usuario_nueve[81] = "";
   ...
   if (remoto_que_sistema_es == 11)
   {
      ...
      if (remoto_que_sistema_es == 13)
      {
         ...
         if (remoto_que_sistema_es == 7)
         {
            ...
            strcat(ver," modo Cliente/Servidor ");
            return(0);
}
#endif
#endif
...
#endif
#endif
   sprintf(buf,",Licencia:%06ld ",nume_serie);
...
static void mensaje_usuario(int con)
{

   if (!con)
   {
       char tmp[512];
       sprintf(tmp,"%s(%d)",rw_user,subnumero_de_usuario+1);
       mensaje_en_cabecera("%s",tmp,3);
   }
   else
   {
       mensaje_en_cabecera("%s","",3);
   }
...
int entra_user(int p,char *usuario,char *password,char *cambio)
{
   ...
   if  (*usuario == 1) {
	   ...
	   pp = usuario;
	   n = 0;
	   while(*usuario && *usuario != 1) {usuario++;n++;}
	   memcpy(usuario_nueve,pp,n);
	   usuario_nueve[n] = 0;
	   if (*usuario == 1)
		   usuario++;
...
char *buffer;
{
   ...
   {
	   char buffer_[1024];
		strcpy(buffer_,funcion);
		strcpy(buffer_+31,buffer);
		if (DespachadorExterno)
			(*DespachadorExterno)((int)DESPACHO_FUNCIONES_IMPRESION,998,(void *)buffer_);
		strcpy(buffer,buffer_+31);
   }
#else
#ifdef ESDOS
    ...
    static int fgv_activa_impredll_aviso = 0;
         if (!fgv_activa_impredll)
         {
            if (!fgv_activa_impredll_aviso)
            {
               ...
               aviso(0,"Falta Instalar el Modulo para Impresion Avanzada");
            }
            return(-1);
         }
