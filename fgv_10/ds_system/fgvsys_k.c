...
int faclin;
{
    if (prog[p].i == 255)
    {
        int pi = pasa_a_int((v_memo(prog[p].indice)->numerica / 10000));
        int pf = pasa_a_int(((long)v_memo(prog[p].indice)->numerica % 10000));
        int sid = (int)v_memo(prog[++p].indice)->numerica;
        BorraDescendencia(-1,default_id,sid,pi,pf);
        default_sid = pi*10000+pf;
        default_wid = -1;
        sprintf(v_memo(5004)->alfabetica,"%d",default_sid);
        n_np = 2;
    }
    else
    {
...
int faclin;
{
   ...
   sprintf(v_memo(5004)->alfabetica,"%d",que_qjes(parada_(control,foco),0));
   
   return(0);
}

static int _push[50];
static int dsid_push[50];
static int n_push = 0;

int pushv(int pi,int pf)
{
   if (n_push < (sizeof(_push)/sizeof(int)))
   {
      /*rwdebug = 0;*/
      _push[n_push] = default_id;
      dsid_push[n_push] = default_sid;
      ...
      {  
            ...
            (*DespachadorExterno)((int)DESPACHO_PANTALLAS,(int)6,tmpx);            
      }
...
int popv(int nulo)
{
   if (n_push)
   {
      ...
      {  
            ...
            (*DespachadorExterno)((int)DESPACHO_PANTALLAS,(int)7,(void *)tmpx);            
            ...
            if (default_id == _push[n_push])
            {
                default_sid = dsid_push[n_push];
            }
            else
            {	
				/*
				default_id = _push[n_push];
				default_sid = dsid_push[n_push];				
				*/
                /*DebugBreak();*/
            }
...
int faclin;
{
   pushv(pasa_a_int((v_memo(prog[p].indice)->numerica / 10000)),pasa_a_int(((long)v_memo(prog[p].indice)->numerica % 10000)));
   return(0);
}
...
{   
   if (v_memo(prog[p].indice)->alfabetica[0])
	   cabecera_v(NULL,v_memo(prog[p].indice)->alfabetica,titulo,empresa);
   else
...
int faclin;
{
   ...
   sprintf(v_memo(5004)->alfabetica,"%d",mante_ficha(v_prog((i%10000)),(i/10000),0,(char *)0),-1);
...
int faclin;
{
static long cuando_pulse = 0L;   
static long cuando_burst = 0L;   
static long cuantos_pulse = 0L;   
static int ultima_tecla = -1;
...
int burst = 0;
   ...
   if (cuando_pulse) {
	   pasado = ahora-cuando_pulse;
	   if (pasado < 2) {
		   if (cuantos_pulse > 15) {
			   sprintf(v_memo(5004)->alfabetica,"%d",ultima_tecla);
			   return 0;
		   }
		   cuando_burst = ahora;
		   burst = 1;
...
int faclin;
{
   ptec((int)v_memo(prog[p].indice)->numerica);
   return(0);
}
...
int faclin;
{
   ...
   if ((f = memoria_alfa(p)) > -1)
      ...
      if (externo && en_conexion_externa)
      {
         int sal = 0;
         char tmp[301];
		 if (v_memo( f )->alfabetica[0] == ':') {
			 tmp[0] = 'C';
			 strcpy(tmp+1,v_memo( f )->alfabetica);
		 }
		 else
			 strcpy(tmp,v_memo( f )->alfabetica);
...
#endif
      sprintf(v_memo(5004)->alfabetica,"%d",agi_compress(v_memo(f)->alfabetica,1));
			}
   n_np = 1;
   return(0);
}
...
int faclin;
{
         ...
         if ((f1 = memoria_alfa(p)) > -1 && (f2 = memoria_alfa(p+1)) > -1)
         {
            ...
            if (externo && en_conexion_externa)
            {
               ...
               char tmp[700];
			   memset(tmp,0,sizeof(tmp));
			   if (v_memo( f1 )->alfabetica[0] == ':') {
				   tmp[0] = 'C';
				   strcpy(tmp+1,v_memo( f1 )->alfabetica);
			   }
			   else
				   strcpy(tmp,v_memo( f1 )->alfabetica);
			   if (v_memo( f2 )->alfabetica[0] == ':') {
				   tmp[300] = 'C';
				   strcpy(tmp+301,v_memo( f2 )->alfabetica);
			   }
			   else
...
int llama_evento_desc(void *nulo)
{
	  ...
	  for (i = 0;i < np_ex;i++)
	  {
		 if (strcmp(tmp,p_ex[i].nombre) == 0) 
		 {
			   ...
			   p_ejecuta(p_ex[i].indice,O_NOR,0,35);
               if (memoplin > 2) 
			   {
...
static FI_DEF *pfi_completar = NULL;
static FI_DEF *pfi_memo = NULL;

void InicializaAutoCompletar()
{
	FI_DEF *pfi;
	FI_DEF *pfi2;
	int fd;
	...
	if (auto_completar)
	{
		crea_dir(rw_directorio("dshint/dshint.mas"));
	}
	else
		return;
	...
	pfi->f_titulo[0] = 0;		
	strcpy(pfi->f_PathDef,rw_directorio("dshint/dshint.mas"));
	strcpy(pfi->f_fichero,rw_directorio("dshint/dshint"));

    inserta_campo(pfi,0,"codigo","codigo",11,"","","");
	inserta_campo(pfi,1,"clave","clave",30,"","","");
	inserta_campo(pfi,2,"cuando","cuando",0,"0","99999999999","99999999999");
	inserta_campo(pfi,3,"campo","campo",250,"","","");
	inserta_campo(pfi,4,"memo","memo",0,"0","9999","");

	pfi->f_nclaves = 1;
	pfi->f_clave = (KEY_DEF *)reserva_malloc(sizeof(KEY_DEF)*2,3); /* memoria para dos claves */
	pfi->f_clave[0].k_ncampos = 3;
	pfi->f_clave[0].k_fflag = 0;
	pfi->f_clave[0].k_campo = (short *)malloc(sizeof(short) * 4);
	pfi->f_clave[0].k_orden = (short *)malloc(sizeof(short) * 4);
	pfi->f_clave[0].k_orden[0] = 1;
	pfi->f_clave[0].k_campo[0] = 1;
	pfi->f_clave[0].k_orden[1] = 1;
	pfi->f_clave[0].k_campo[1] = 2;
	pfi->f_clave[0].k_orden[2] = -1;
	pfi->f_clave[0].k_campo[2] = 3;
	...
	pfi->f_titulo = strmalloc(1);
	pfi->f_titulo[0] = 0;		
	strcpy(pfi->f_PathDef,rw_directorio("dshint/dshintm.mas"));
	strcpy(pfi->f_fichero,rw_directorio("dshint/dshintm"));

    inserta_campo(pfi,0,"memo","memo",0,"0","9999","");
	inserta_campo(pfi,1,"linea","linea",0,"0","9999","");
	inserta_campo(pfi,2,"dato","dato",250,"","","");

	pfi->f_nclaves = 1;
	pfi->f_clave = (KEY_DEF *)reserva_malloc(sizeof(KEY_DEF)*2,3); /* memoria para dos claves */
	pfi->f_clave[0].k_ncampos = 2;
	pfi->f_clave[0].k_fflag = 0;
	pfi->f_clave[0].k_campo = (short *)malloc(sizeof(short) * 3);
	pfi->f_clave[0].k_orden = (short *)malloc(sizeof(short) * 3);
	pfi->f_clave[0].k_orden[0] = 1;
	pfi->f_clave[0].k_campo[0] = 1;
	pfi->f_clave[0].k_orden[1] = 1;
	pfi->f_clave[0].k_campo[1] = 2;
...
static char *auto_codigo = NULL;
static int auto_cwid = -1;

int SetAutoCompletar(char *codigo,int cwid)
{
	auto_codigo = codigo;
	auto_cwid = cwid;
	return 0;
...
int ObtenAutoCompletar(char *codigo,char *frase,char *resultado)
{
	if (!pfi_completar)
		return -1;

	defecto_ficha(pfi_completar);
	st_campo(0,pfi_completar,codigo);
	st_campo(1,pfi_completar,frase);
	if (lee_reg(pfi_completar,ISGTEQ) >= 0)
	{
		char buf[512];
		ld_campo(3,pfi_completar,buf);
		if (!memcmp(frase,buf,strlen(frase)))
		{
			char buf2[128];
			buf2[0] = 0;
			ld_campo(4,pfi_completar,buf2);
			if (!atoi(buf2))
			{
				strcpy(resultado,buf);
				return 0;
			}		    
			if (pfi_memo)
			{
				int i = 0;
				resultado[0] = 0;
				st_campo(0,pfi_memo,buf2);
				st_campo(1,pfi_memo,"0");
				while(lee_reg(pfi_memo,ISEQUAL) >= 0)
				{
					ld_campo(2,pfi_memo,buf);
					quitab(buf);
					strcat(resultado,buf);

					i++;
					sprintf(buf2,"%d",i);
					st_campo(1,pfi_memo,buf2);
				}
				return 0;
			}
		}
...
int CodigoObtenAutoCompletar(char *frase)
{
	if (!pfi_completar)
		return -1;

	if (auto_codigo && auto_cwid != -1)
	{	   
	   char resultado[10240];
	   if (ObtenAutoCompletar(auto_codigo,frase,resultado+5) >= 0)
	   {
		   memcpy(resultado,"DATA:",5);
		   quitab(resultado);
		   if (DespachadorExterno)
				(*DespachadorExterno)((int)DESPACHO_ESTADOCONTROL,auto_cwid,(void *)resultado);
	   }	   
	}
...
int GrabaAutoCompletar(char *codigo,char *frase,char *resultado,int memo)
{
	int r;
	int nuevo = 0;
	int n;
	...
	if (memo && pfi_memo)
	{			
		if (lee_reg(pfi_memo,ISLAST) >= 0)
		{
			ld_campo(0,pfi_memo,buf2);
			n = atoi(buf2)+1;
			sprintf(buf2,"%d",n);			
		}
	}	

	if (lee_reg(pfi_completar,ISEQUAL) < 0)	
	{
		st_campo(3,pfi_completar,resultado);
		st_campo(4,pfi_completar,buf2);
		r = graba_nuevo(pfi_completar);
	}
	else	
	{
		st_campo(3,pfi_completar,resultado);
		st_campo(4,pfi_completar,buf2);
		r = graba_actual(pfi_completar);	
	}

	if (memo && r >= 0 && pfi_memo)
	{
		ld_campo(4,pfi_completar,buf2);
	    ...
	    do 
		{
			...
			memcpy(buf2,resultado,((250 < r) ? 250 : r));
			buf2[((250 < r) ? 250 : r)] = 0;

			st_campo(2,pfi_memo,buf2);

			if (graba_nuevo(pfi_memo) < 0)
			{
				borra_reg(pfi_memo);
				graba_nuevo(pfi_memo);
			}				
			r -= 250;
			resultado += 250;
		} while(r > 250);
...
void FinalizaAutoCompletar()
{
	if (pfi_completar)
	{
		cierra_ind(pfi_completar);
		free_defpri(pfi_completar);
		pfi_completar = NULL;
	}
	if (pfi_memo)
	{
		cierra_ind(pfi_memo);
		free_defpri(pfi_memo);
		pfi_memo = NULL;
	}
}