...
char *clave;
{
   ...
   if (url_ayuda_f1 || (remoto_es_sistema_grafico & 128))
   {	   
	   if (!url_ayuda_f1)
		   strcpy(envio,"http://www.diagram.es");
	   else
   ...
   {      
      ...
      if (!ayuda_de_basico)
      {
         ...
         memcpy(&remoto_registro,tmpy,sizeof(long));
   ...
   if (!ayuda_de_basico) 
   {
      ...
      {
         if (!confirmar(2400,"NO hay AYUDA para esta opción!, Desea editarla? ","S"))
         {
            aviso(0,"Despues de editar vuelva a pedir la ayuda en la misma opcion");
            crearla = 1;
         }
      }
   ...
   if (!ayuda_de_basico) 
   {
      ...
      if (!f)
      {
         ds_rtfhlp(nombre2,"Preparando la ayuda de %s por primera vez ...");
         f = fopen(bufpagina,"r");
      }
      ...
      {   
         ...
         if (hacer) ds_rtfhlp(nombre2,"Actualizando la ayuda de %s ...");
   ...
   aviso(0,"NO dispone de AYUDA para esta opción");
   return(0);
#else
