...
int CorreCampoPan(FI_DEF *fi,int pan,int desde)
{
	int i,k,p;
	int dif = fi->f_pantalla[pan].p_campoini - desde;

	fi->f_pantalla[pan].p_campoini = desde;
	fi->f_pantalla[pan].p_campofin -= dif;
	for (i = fi->f_pantalla[pan].p_campoini - 1;i < fi->f_pantalla[pan].p_campofin;i++)
	{
		k = fi->f_campos[i+dif].c_mante.c_actual % 1000;
		p = (fi->f_campos[i].c_mante.c_actual/1000)*1000;
		fi->f_campos[i].c_mante.c_actual = k + p;
	}
	return 0;
}
...
int copia_defn(FI_DEF **_fi,FI_DEF *fo)
{
	...
	*_fi = fi;   
	...
	{        
        fi->f_clave[i].k_campo = (short *)malloc(sizeof(short) * (fi->f_clave[i].k_ncampos+1));
        memcpy((char *)fi->f_clave[i].k_campo,(char *)fo->f_clave[i].k_campo,sizeof(short) * fi->f_clave[i].k_ncampos);                
        fi->f_clave[i].k_orden = (short *)malloc(sizeof(short) * (fi->f_clave[i].k_ncampos+1));
	...
	{        
     ...
     fi->f_campopos  = (int *)malloc(sizeof(int) * (fi->f_ncampos+1));
     memset((char *)fi->f_campopos,0xff,sizeof(int) * (fi->f_ncampos+1));
     ...
     for (i=0;i<fi->f_ncampos;i++) 
     {
        ...
        fi->f_campopos[i] = fi->f_campos[i].c_mante.c_qp;        
     }        
     ...
     fi->f_pantaids = (int *)malloc(sizeof(int) * (fi->f_npanta+1));
     memset((char *)fi->f_pantaids,0xff,sizeof(int) * (fi->f_npanta+1));

     fi->f_pantasids = (int *)malloc(sizeof(int) * (fi->f_npanta+1));
     memset((char *)fi->f_pantasids,0xff,sizeof(int) * (fi->f_npanta+1));

     fi->f_pantamix = (PAN_DEF **)malloc(sizeof(PAN_DEF *) * (fi->f_npanta+1));
     memset((char *)fi->f_pantamix,0,sizeof(PAN_DEF *) * (fi->f_npanta+1));
         
     fi->f_id = -1;


     fi->f_campowids = (int *)malloc(sizeof(int) * (fi->f_ncampos+1));
     memset((char *)fi->f_campowids,0xff,sizeof(int) * (fi->f_ncampos+1));

     fi->f_camporden = (int *)malloc(sizeof(int) * (fi->f_ncampos+1));
     memset((char *)fi->f_camporden,0xff,sizeof(int) * (fi->f_ncampos+1));                  

     fi->f_taborder  = (int *)malloc(sizeof(int) * (fi->f_ncampos+1));
     memset((char *)fi->f_taborder,0xff,sizeof(int) * (fi->f_ncampos+1));

     fi->f_orderflags  = (int *)malloc(sizeof(int) * (fi->f_ncampos+1));
     memset((char *)fi->f_orderflags,0,sizeof(int) * (fi->f_ncampos+1));

     fi->f_fi_clave_lineas = 0;
     fi->f_fi_modo_lineas  = 0;
     fi->f_fi_scroll_lineas= -1;
     fi->f_ipfi = -1;
         

     k = 0;
     i = 0;
     for (j = 0;j < fi->f_npanta;j++)
     {
         if (fi->f_pantalla[j].p_campoini < 1 || fi->f_pantalla[j].p_campoini > fi->f_ncampos ||
             fi->f_pantalla[j].p_campofin < 1 || fi->f_pantalla[j].p_campofin > fi->f_ncampos)
         {
             fi->f_pantalla[j].p_campoini = i+2;
             fi->f_pantalla[j].p_campofin = i+1;
         }
         for (i = fi->f_pantalla[j].p_campoini-1;i < fi->f_pantalla[j].p_campofin;i++)
         {
             if (k >= fi->f_ncampos) break;
             k = fi->f_campos[i].c_mante.c_actual % 1000;
             fi->f_camporden[k] = i;
             fi->f_taborder[i] = k;
         }
    }
...
FI_DEF **_fi;
{
   ...
   *_fi = fi;   
   ...
   memcpy(fi->f_fichero,fi->f_oldfichero,sizeof(fi->f_oldfichero));
   ...
   strcpy(fi->f_PathDef,nomfi);
		 ...
		 /* chequeo integridad correlacion pantallas ... */
		 if (fi->f_npanta > 0)
		 {
			 if (fi->f_pantalla[0].p_campofin > fi->f_ncampos)
			 {
				 fi->f_pantalla[0].p_campofin = fi->f_ncampos;
			 }
			 if (fi->f_pantalla[0].p_campoini > 1)
				CorreCampoPan(fi,0,1);
		 }
		 for (j = 1;j < fi->f_npanta;j++)
		 {	
			if (fi->f_pantalla[j].p_campofin > fi->f_ncampos)
			{
				 fi->f_pantalla[j].p_campofin = fi->f_ncampos;
			}
			if (fi->f_pantalla[j].p_campoini > (fi->f_pantalla[j-1].p_campofin+1))
			{
				CorreCampoPan(fi,j,fi->f_pantalla[j-1].p_campofin+1);
			}		    
		 }

		 /*
		 if (!strcmp(quita_path(nomfi),"agifa024")) {
			 fi->f_npanta++;
			 fi->f_pantalla = (PAN_DEF *)realloc((void *)fi->f_pantalla,sizeof(PAN_DEF) * fi->f_npanta);
			 for (j = fi->f_npanta-1;j > 0;j--) {
				 memcpy(&(fi->f_pantalla[j]),&(fi->f_pantalla[j-1]),sizeof(PAN_DEF));
			 }
			 sprintf(fi->f_pantalla[0].p_nombrepan+strlen(fi->f_pantalla[0].p_nombrepan)+1,"%c%c||",1,2);
			 sprintf(fi->f_pantalla[1].p_nombrepan+strlen(fi->f_pantalla[1].p_nombrepan)+1,"%c%c))02",1,2);
			 fi->f_pantalla[0].p_campofin = 2;
			 fi->f_pantalla[0].p_posfin = 280;
			 fi->f_pantalla[1].p_campoini = 3;			 
		 }
		 */
...

		 {
			 ...
			 for (j = 0;j < fi->f_npanta;j++)
			 {
				 ...
				 for (i = fi->f_pantalla[j].p_campoini-1;i < fi->f_pantalla[j].p_campofin;i++)
				 {
					 ...
					 tmpi = fi->f_campos[k].c_mante.c_actual%1000;					 
					 fi->f_campos[k].c_mante.c_actual = j*1000+tmpi;
				 }
			 }
		 }
...
FI_DEF *fi;
{
   ...
   if (fi) 
   {
      ...
      if (fi->f_pantaids)
         free((char *)fi->f_pantaids);	  

      if (fi->f_pantasids)
         free((char *)fi->f_pantasids);	  

      if (fi->f_campowids)
         free((char *)fi->f_campowids);

      if (fi->f_camporden)
          free((char *)fi->f_camporden);
      
      if (fi->f_taborder)
          free((char *)fi->f_taborder);

      if (fi->f_campopos)
          free((char *)fi->f_campopos);      

      if (fi->f_orderflags)
          free((char *)fi->f_orderflags);
      ...
      if (fi->f_pantamix)
      {
          for (i = 0;i < fi->f_npanta;i++)
          {
              if (fi->f_pantamix[i])
              {
                  free((void *)fi->f_pantamix[i]);
              }
          }
          free((void *)fi->f_pantamix);
      }

      /*reserva_free((char *)fi->f_pantalla,1);*/
      free((char *)fi->f_pantalla);
...
{	
	...
	tamano += (unsigned int)rw_write_(file1,fi->f_fichero,O_MAXPATH+1);
