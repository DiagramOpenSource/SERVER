...
#define DS_DEBUG_BREAK		  98
#define SOCORRO			      99
#define DIME_COLGADOS        100
#define RWNETCON_TENGO_10    110
...
static unsigned short local_nivel_mensaje = 0;
static unsigned short remoto_nivel_mensaje = 0;


static unsigned char last_cabecera[2048];
...
static int prueba_controles = 1;


long server_nume_serie = 0;
extern long nume_serie;

int  menusg_virtual = 0; /* AQUITAR!!!*/

static int atencion_control_x  = 0;

extern int extra_de_relleno_enviafichero;

extern int port_fijo_de_server;
...
static DATAMSG *terminal_cola_mensajes[2048];
static int nterminal_cola_mensajes = 0;
...
unsigned char _rwsockc[MAXRDATA+1024];
...
static char dir_solicitado[MAXPATH];

char server_dirbase1[300];
static char server_dirbase2[300];
int  server_ver1 = 0;
int  server_ver2 = 0;
unsigned char server_verletra = 0;
int  server_que_sistema_es = 0;
int  server_es_sistema_grafico = 0;
...
extern char remoto_dirbass[MAXPATH];
#endif
...
extern int avancelinea;
/*extern int qpi_rt;*/
/*********/

static int  rwmax_buffer = 0x13000;
...
static unsigned char  soporta_letra='A';


static int soportas_ver1=9;
static int soportas_ver2=0;
static unsigned char  soportas_letra='A';
/***********************/
...
int _leecontrol_en_server();


/* LOG ESPECIAL */

extern char *log_enviafichero;
...
int comprueba_ethernet(char *la_ethernet_)
{
	char tmp[512];
	char tmpe[512];
	FILE *file;
...
BOOL flush_mysock()
{
         ...
         else
         {
     	 ...
     	 if (haylog_mensajes_cs)
		 {
			log_mensajes_cs(2,(void *)p,p->hdr.length);
		 }		 
         r = rwi_senddata(MySock.sock,(char *)p,p->hdr.length);  
         rwp_send_buffer = 0;    
...
static BOOL senddatamessage (SOCKET sock, DATAMSG *sendbuf)
{
         ...
         if (sock != MySock.sock)
         {
            return(rwi_senddata(sock,(char *)sendbuf,totalbytes));
         }
         
         
         if (totalbytes > (MAXRDATA-HDRSIZE))
         {
            aviso(0,"EXCEDIDO MENSAJE INTERNO");
            return(FALSE);
         }
         ...
         if ( (rwp_send_buffer + totalbytes) >= ((rwmax_buffer/2)-2048)) // 512
         {
            DATAMSG *guarda_sendbuf;
			guarda_sendbuf = malloc(sizeof(DATAMSG));
            memcpy(guarda_sendbuf,sendbuf,sizeof(DATAMSG));
            flush_mysock(); 
            memcpy(sendbuf,guarda_sendbuf,sizeof(DATAMSG));
			free(guarda_sendbuf);
         }
         
		 {				 
			 sendbuf->hdr.command |= (unsigned char)128;
			 sendbuf->hdr.length += 2;
			 totalbytes -= HDRSIZE;
			 *((unsigned short *)(sendbuf->data+totalbytes)) = local_nivel_mensaje;
			 totalbytes = sendbuf->hdr.length;
		 }
...
static unsigned read_gunzip(int *pfd,unsigned char *buf,unsigned tam)
{
   int i;
   unsigned leido;
   unsigned char mascara;
...
BOOL recvdatamessage(SOCKET sock, DATAMSG *recvbuf)
{
   ...
   do      {         
			 ...
			 errno = 0;
                  ...
                  {		   
					 char tmp2[512];
					 sprintf(tmp2,"paquete desconocido: %d %d %d",pr->hdr.signature,recvbuf->hdr.command,recvbuf->hdr.length);
					 /*
                    rw_base_close_(file);
					file = -1;
					*/
					 aviso(0,tmp2/*"control fichero 5"*/);
                     return FALSE;
                  }
				  ...
				  {
									 char tmp2[512];
									 sprintf(tmp2,"fallo inminente: %d %d",recvbuf->hdr.command,recvbuf->hdr.length);
									 /*
                                    rw_base_close_(file);
									file = -1;
									*/
				  ...
				  /*
				  if (pr->hdr.command == SIGNAL_TERMINAL)
				  {
					 continue;
				  }
				  */
                  ...
                  if (pr->hdr.command == BLOQUE_COMPRIMIDO)
                  {
                     ...
                     if (pr->hdr.command != BLOQUE_DE_MENSAJES && sock == MySock.sock)
                     {
			 ...
			 errno = 0;
                     ...
                     else if (pr->hdr.command == FILE_TRANSFER_OPEN)
                     {
                        ...
                        if (pr->hdr.command == FILE_TRANSFER_DATA)
                        {
							int antiguo = 0;
#ifdef RWSERVER
						   if (en_conexion_externa && terminal_internal_build < 61) 
							   antiguo = 1;
#else
						   if (en_conexion_terminal && server_internal_build < 61)
							   antiguo = 1;
#endif
						   if (antiguo) {
...
salta_aqui_close:
                           ...
                           {			
                              ...
                              else if (pr->hdr.command == BLOQUE_COMPRIMIDO)
                              {
                                    _write_interno = write_interno;
                                    _read_interno = read_interno;
	...
	*/
	...
	do
	{
		 ...
		 if (!err)
		 {
			  ...
			  {				 
				 p1 = (int *)_tmp;
				 _p = _tmp+sizeof(int)*3;
				 *_p = 0;
				 p1[0] = sizeof(int)*3 + 2;     
				 p1[1] = 1;
				 p1[2] = 99999999;
				 DespachadorLocal_hebra((int)DESPACHO_MAINCOMMAND,ID_TICSCON,(void *)_tmp);
			  }      
		 ...
		 if (infor.id == 0x172)
		 {
			 ...
			 {
				 if (tiempo_reenvio < time(NULL))
					 reenvio = 1;
				 else if (strcmp(buffglobo,tmp2))
					 reenvio = 1;
				 else
				 {
					 tmp[0] = 0;
					 DespachadorLocal_hebra((int)DESPACHO_MULTIPLE,667,tmp);					 
			 ...
			 {				 
				 ...
				 if (!(mensajes_globo_enviados-reenvio))
				 {
					 ...
					 DespachadorLocal_hebra((int)DESPACHO_MULTIPLE,667,buffglobotarea);
				 ...
				 {				 
					 p1 = (int *)_tmp;
					 _p = _tmp+sizeof(int)*2;
					 strcpy(_p,mensaje_globo_netcon);
					 p1[0] = sizeof(int)*2 + strlen(mensaje_globo_netcon) + 2;     
					 p1[1] = 1;					 
					 DespachadorLocal_hebra((int)DESPACHO_MAINCOMMAND,ID_AVISOGLOBO,(void *)_tmp);
				 }      
				 printf("0x172-4\r\n");fflush(stdout);
...
void EstoyVivo(void *_s)
{
	...
	do
	{
		 ...
		 if (cuanto > 1024)
			 ...
			 errno = 0;
#endif
		 if(recv(s,buffer,cuanto,MSG_WAITALL) != cuanto)
		 {
			 Sleep(100);
			 continue;
		 }
...
int modo;
{
   ...
   if (!memcmp(server_request+1,"(C:",3))
   {
                  ...
                  if (_pon_incidencia)
                  {
                     sprintf(tmp,"CLIENTE A (%s",server_request+4);
                     (*_pon_incidencia)(0,tmp);
                  }
                  modo_reconexion = 0;
   }
   else
						if (!memcmp(server_request+1,"(/:",3))
                  {  
                     remoto = 1;  
                     sprintf(tmp,"RECONECTANDO CON (%s",server_request+4);
                     mensaje(0,tmp);  
                     if (_pon_incidencia)
                     {
                        sprintf(tmp,"RECONECTADO CLIENTE A (%s",server_request+4);
                        (*_pon_incidencia)(0,tmp);
                     }
                     modo_reconexion = 1;
                     server_request[2] = 'C';
                     ...
                     if (!memcmp(server_request+1,"(S:",3))
                     {  
                        remoto = 0;  
                        modo_reconexion = 0;
                     }
                        ...
                        */
                        if (!gzip_buffer)
                        {
                           gzip_buffer = malloc(rwmax_buffer);
                        }
                        if (!rwsend_buffer)
                        {
                           rwsend_buffer = malloc(rwmax_buffer);
                        }
                        if (!rwrcv_buffer)
                        {
                           rwrcv_buffer = malloc(rwmax_buffer);
                           punt_rcvb = rwrcv_buffer;
                        }
		...
		if((dtest =  socket(AF_INET,SOCK_STREAM,IPPROTO_TCP)) != SOCKET_ERROR) {

			MySock.addr.sin_family = AF_INET;

			MySock.addr.sin_addr.s_addr = inet_addr("0.0.0.0"); 								MySock.addr.sin_family = AF_INET;  

			MySock.addr.sin_port = htons(1366);

			bind(dtest,(struct sockaddr *) &(MySock.addr),sizeof (MySock.addr));
						   ...
						   do
						   {
							   if((rwsock =  socket(AF_INET,SOCK_STREAM,IPPROTO_TCP)) == SOCKET_ERROR)
							   {
								  ...
								  { 
									 ...
									 printf("Imposible abrir un socket:%d.\r\n",errno);
#ifdef FGVDOS
								  WSACleanup();
#endif
								  libera_x_buffers();
							   ...
							   {  								  
								  sprintf(tmp,"No hay servicio RWNETCON en (%s:%u)",lpServName,PORTRWNETCON2);
								  if (remoto)
								  {	 
#ifdef WIN32
									  MessageBox(NULL,tmp,"ATENCION!",MB_OK);
#else
									 aviso(0,tmp);     
#endif
								  }
...
#else
									 xferbuf.hdr.command = DIME_COLGADOS; /* truco para compabilizar 8-9-10 */
#endif
								  }
							   }
							   ...
							   {
								  if (remoto)
									 aviso(0,"DENEGADO EL SERVICIO POR RWNETCON! (Posible mantenimiento del servidor)");
								  else
									 printf("DENEGADO EL SERVICIO POR RWNETCON!\r\n");
								  closesocket(rwsock);
								  rwsock = INVALID_HANDLE_VALUE;
#ifdef FGVDOS
								  WSACleanup();
...
#ifdef FGVJAVA
							   //pthread_create(&thread,NULL,EstoyVivo,(void *)rwsock);
							   DespachadorLocal((int)DESPACHO_MULTIPLE,668,(void *)&rwsock);
  ...
  */
  ...
  if((MySock.sock =  socket(AF_INET,MySock.type,MySock.protocol)) == SOCKET_ERROR)
  {
     ...
     { 
        ...
        printf("Imposible abrir socket de servidor:%d.\r\n",errno);
#ifdef FGVDOS
     WSACleanup();
#endif
     libera_x_buffers();
  ...
  if (!remoto)
  {
	  if (port_fijo_de_server) {
		  FILE *ff = fopen("/tmp/puertopreferente.txt","w");
		  if (ff) {
			  fprintf(ff,"%d\r\n",port_fijo_de_server);
			  fclose(ff);
		  }
		  RWPORTIP = htons(port_fijo_de_server);
	  }
	  else if (!RWPORTIP)
	  {
		  RWPORTIP = htons(15047 + (time(NULL)%512));
	  }
	  else
	  {
		  maximoports = RWPORTIP;
		  RWPORTIP = htons(15047 + (time(NULL)%maximoports));
	  }
  }
  ...
  if (remoto)
  {
     ...
     if (tmerr == SOCKET_ERROR)
     {
        if (remoto)
        {
           sprintf(tmp,"No existe conexion a servidor en (%s:%u)",lpServName,ntohs(RWPORTIP));
           aviso(0,tmp);
        }
	 ...
	 */
     ...
     while(i)
     {
        ...
        if (bind(MySock.sock,(struct sockaddr *) &(MySock.addr),sizeof (MySock.addr)) == SOCKET_ERROR)
        {
           if (rwsock != INVALID_HANDLE_VALUE)
           {
              ...
              {
				 MySock.addr.sin_port = htons(15047 + (rand()%maximoports));				 
				 if (MySock.addr.sin_port == RWPORTIP)
				 {
					 MySock.addr.sin_port = ntohs(RWPORTIP);
					 if (MySock.addr.sin_port <= 15047)
					 {
						 MySock.addr.sin_port++;
					 }
					 else
     ...
     {	
        sprintf(tmp,"%d.%d.%d.%d:%u",S_un.S_un_b.s_b1,S_un.S_un_b.s_b2,S_un.S_un_b.s_b3,
           S_un.S_un_b.s_b4,MySock.addr.sin_port);
        xferbuf.hdr.signature = RWNIDENTIFICACION;                 
  ...
  {
     aviso(0,"ESTE SISTEMA SOLO PUEDE SER CLIENTE!");
     closesocket(MySock.sock);
#ifdef FGVDOS
     WSACleanup();
#endif
  ...
  if (!modo)
  {
     if (!remoto)
     {
        ...
        if (!modo_recuperacion && !ver_si_es_servidor())
        {
           ...
           printf("DESCONECTADO: REALMENTE NO SOY UN SERVIDOR (NO TENGO LICENCIA)\r\n");					
           return(2);
        } 
        else
        {					  /* aceptar sesion */
           ...
           memcpy(xferbuf.data,&usando_compresion,sizeof(int));
           rwi_senddata(MySock.sock,(char *)&xferbuf,xferbuf.hdr.length);
           flush_mysock();
           ...
           if (_pon_incidencia)
           {
              sprintf(tmp,"SERVIDOR A %d.%d.%d.%d",S_un.S_un_b.s_b1,S_un.S_un_b.s_b2,S_un.S_un_b.s_b3,S_un.S_un_b.s_b4);
              (*_pon_incidencia)(0,tmp);
           }
        }
        ...
        xferbuf.hdr.command = COMANDO_TERMINAL;                 
			  ...
			  *pdata = 0;
              ...
              rwi_senddata(MySock.sock,(char *)&xferbuf,xferbuf.hdr.length);
              if (responde_sesion(&xrcbbuf) != CONTROL_DE_TERMINAL || (xrcbbuf.data[0] != 7 && xrcbbuf.data[0] != 20 && xrcbbuf.data[0] != 21))
              {
                 establece_desconexion(0);
                 printf("DESCONECTADO: EL TERMINAL NO ACEPTA MI VERSION\r\n");					
                 return(2);
              }
              if (xrcbbuf.data[0] != 7)
              {
                 while (xrcbbuf.data[0] != 21)
                 {
                    ...
                    if (responde_sesion(&xrcbbuf) != CONTROL_DE_TERMINAL) break;
                 }
                 sleep(10);
                 establece_desconexion(0);                      
                 return(2);
              ...
              pdata++;
              memcpy(&redita_atrib,pdata,sizeof(int));
              pdata += sizeof(int);
              memcpy(&redita_cc,pdata,sizeof(int));
              pdata += sizeof(int);
              memcpy(&redita_cf,pdata,sizeof(int));
              pdata += sizeof(int);
                 ...
                 if (remoto_que_sistema_es == 13)
                 {
                    ...
                    if (remoto_que_sistema_es == 11)
                    {
                       ...
                       if (remoto_que_sistema_es == 7)
                       {
                          ...
                          if (remoto_ver1 == rtme_version1)
                          {                 
                             for (i = 0;i < nPaq;i++)
                             {
                                if (Paq[i].i.tipo == kki && Paq[i].i.version1 == rtme_version1 && !strcmp(Paq[i].i.sistema,tmp))
                                   break;
                             }
                          }
                          else
                          ...
                          if (i < nPaq)
                          {
                             ...
                             Bl(24);
                             if (confirmar(2400,"HAY QUE ACTUALIZAR EL RUNTIME CLIENTE, ¿CONTINUAR? ","S") != 0)
                             {
                                Bl(24);
                                pprint(2401,"Desconectando ...");
                                flush_mysock();
                                sleep(5);
                                establece_desconexion(0);
                                return(2);
                             ...
                             if (!rw_envia_fichero(tmp,4))
                             {
							    aviso(0,"Runtime enviado! Pulse Aceptar para continuar.");
                                xferbuf.hdr.signature = RWNIDENTIFICACION;
                                xferbuf.hdr.command = COMANDO_TERMINAL;
                                xferbuf.hdr.length = HDRSIZE + 1;
                                xferbuf.data[0] = 102; /* solicitar reinicio del terminal */
                                rwi_senddata(MySock.sock,(char *)&xferbuf,xferbuf.hdr.length);
                                responde_sesion(&xrcbbuf);
                                sleep(5);
                                establece_desconexion(0);                      
                                return(2);
                             }
                          ...
                          if (!time(NULL))
                          {
                             aviso(0,"IMPOSIBLE ACTUALIZACION AUTOMATICA: LA FECHA DEL SERVIDOR ES INCORRECTA!");
                          }
                          else
                          {
                             aviso(0,"SU VERSION ES INCORRECTA: IMPOSIBLE ACTUALIZACION AUTOMATICA");
                          }
                          /***********************************/
                          xferbuf.hdr.signature = RWNIDENTIFICACION;
                          xferbuf.hdr.command = COMANDO_TERMINAL;
                          ...
                          sleep(2);
                          establece_desconexion(0);
                          printf("DESCONECTADO: NO SE ACEPTA LA VERSION DE TERMINAL\r\n");
                          return(2);
              }
              ...
              */
              ...
              {               /* solicitar session */
                 ...
                 if (responde_sesion(&xrcbbuf) != SESION_ACEPTADA)
                 {
                    sprintf(tmp,"CONEXION %s DENEGADA! x(%d)",lpServName,xrcbbuf.hdr.command); 
                    aviso(0,tmp);
                    establece_desconexion(0);
                    return(2);
                 }
...
int responde_xarxa(int id,DATAMSG *xfer)
{
   ...
   while(-1)
      ...
      if (rwp_rcv_buffer || FD_ISSET(MySock.sock,&setread))
      {
         ...
         {
#ifdef FGVDOS
            MessageBox(NULL,"MAL MENSAJE 1","ERROR",MB_OK);
#endif
            continue;
         }
         if (id == AGI_SORT)
...
int responde_sesion(DATAMSG *xrcbbuf)
{
   unsigned long cuanto;   
   int err;
...
static int ignora_procesamensajes = 0;

int envia_retro_al_server(void *puntero)
{
int funcion;
...
DATAMSG2 __xrcbbuf;
   ...
   if ((modo & 1))
      ...
      envia_control_a_server(tmp,1,168);
	  waitfor = 167;
	  */
	  recibidos = _leecontrol_en_server(NULL,0,&waitfor);
      if (recibidos < (long)(sizeof(long)+sizeof(int)+1))
	  {	
...
char *s;
{
   ...
   memcpy(xferbuf.data+2,&Pcursor,sizeof(int));
   memcpy(xferbuf.data+2+sizeof(int),&PCcolor,sizeof(int));
   memcpy(xferbuf.data+2+(sizeof(int)*2),&PFcolor,sizeof(int));
...
char c;
{
         ...
         memcpy(xferbuf.data+1,&Pcursor,sizeof(int));
...
int a;
{
   ...
   memcpy(xferbuf.data+1,&a,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(a);
}
...
int cls_en_terminal()
{
   ...
   memcpy(xferbuf.data+3,&PCcolor,sizeof(int));
   memcpy(xferbuf.data+3+sizeof(int),&PFcolor,sizeof(int));
   memcpy(xferbuf.data+3+(sizeof(int)*2),&win_fondo_neutro,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int pop_mensaje(DATAMSG *msg,int waitfor,int modo);
...
int *waitfor;
{
   ...
   while(-1)
   {
      ...
      flush_mysock();
	  if (pop_mensaje(&xrcbbuf,wwww,1) <= 0) {
		  if (!rwp_rcv_buffer)
		  {  
repite_amigo1:
			 FD_ZERO(&setread);
      ...
      if (xrcbbuf.hdr.command == DS_DEBUG_BREAK) {
		 tracer_int = 1;
         goto repite_amigo1;
	  }
      ...
      {
		 pump_mensaje(&xrcbbuf);
         goto repite_amigo1;
      }

	  if (xrcbbuf.data[0] == 199) 
	  {
		  if (auto_completar)
			  CodigoObtenAutoCompletar(xrcbbuf.data+1+sizeof(int)*12);
		  continue;
	  }
      ...
      {  
		...
		memcpy(&funcion,puntero,sizeof(int));
		puntero += sizeof(int);
		...
		len = sizeof(int);
		memcpy(buffer+len,&tam,sizeof(long));
		len += sizeof(long);
		tam -= sizeof(long);
        ...
        while(tam)
		   ...
		   do
		     {
             if ((recibidos = leecontrol_en_terminal(NULL,0,&waitfor)) < 0) {
			    free(buffer);
			    return -1;
				}
		     } while(waitfor != 68);
		...
		{
			/*
            envia_control_a_terminal(tmp,1,67);
		    waitfor = 0;
		    do
		    {
			...
			*/
            /*leecontrol_en_terminal(NULL,0,NULL);*/
        }
        if (RetroDespachador((void*)buffer) < 0)
			return -1;
		...
		if ((modo & 1))
		   ...
		   char tmp2[11000];
		   long envio;
		   long tam1; /* tamaño de envio , en tam pondremos el de respuesta ... */

		   puntero = buffer;		   

		   memcpy(tmp2,puntero,sizeof(int));
		   puntero += sizeof(int);
		   len = sizeof(int);
		   memcpy(&tam1,puntero,sizeof(long));
		   memcpy(&tam,puntero+sizeof(long),sizeof(long));
		   memcpy(tmp2+len,&tam,sizeof(long));
		   len     += sizeof(long);
		   puntero += sizeof(long);   
		   tam     -= sizeof(long);
		   ...
		   if (tam > 10000)
			  ...
			  tmp2[len] = 1;
			  len++;
			  memcpy(tmp2+len,&envio,sizeof(long));
			  len += sizeof(long);
			  }
		   else
		      {
         ...
         if (xrcbbuf.data[0] == 42) /* control */
            ...
            if (xrcbbuf.data[0] == 15)
            {
               ...
               if (*dat)
               {
                  dat++;
                  dato_entra = dati;
                  memcpy(&dato_entra_len,dat,sizeof(int));
                  dat += sizeof(int);
               ...
               if (xrcbbuf.data[0] == 10)
               {
                  ...
                  dat = xrcbbuf.data+1;
                  memcpy(&err,dat,sizeof(int));
                  dat += sizeof(int);
                  memcpy(&_M_Cursor,dat,sizeof(int));
                  dat += sizeof(int);
                  memcpy(&_ID_Cursor,dat,sizeof(int)); 
                  dat += sizeof(int);
                  memcpy(&_M_EXTRA_AVANCE,dat,sizeof(int)); 
                  dat += sizeof(int);
                  memcpy(&redita_col,dat,sizeof(int));    
                  dat += sizeof(int);
                  _dato_entra = dato_entra;
                  ...
                  memcpy(tmp,buf_tecla,sizeof(int)*200);
                  buf_tecla[0] = err; /* perentorio */
                  buf_tecla[1] = -1;
                  leecar_server_puro = 1;
                  i = leecar();
                  memcpy(buf_tecla,tmp,sizeof(int)*200);   
...
int pulsatecla_en_terminal()
{
   ...
   do {
      ...
      memcpy(&err,xrcbbuf.data+1,sizeof(int)); 
...
int leecar_en_terminal()
{
   ...
   do {
      ...
      xferbuf.data[0] = 5;
      xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int);
      memcpy(xferbuf.data+1,&modo_de_validacion,sizeof(int));
      senddatamessage(MySock.sock, &xferbuf);
      ...
      memcpy(&err,xrcbbuf.data+1,sizeof(int));
      memcpy(&_M_Cursor,xrcbbuf.data+1+sizeof(int),sizeof(int));
      memcpy(&_ID_Cursor,xrcbbuf.data+1+(sizeof(int)*2),sizeof(int));
      memcpy(&_M_EXTRA_AVANCE,xrcbbuf.data+1+(sizeof(int)*3),sizeof(int));
...
int i;
{
   ...
   memcpy(xferbuf.data+1,&i,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(int),&Pcursor,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(i);
}
...
VENT *w;
{
   ...
   *pdata = 7; /* refresca */
   ...
   memcpy(pdata,&Pcursor,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&w,sizeof(w));
   pdata += sizeof(int);
   memcpy(pdata,&win_fondo_neutro,sizeof(int));
   pdata += sizeof(int);
   *pdata = Patrib;
...
int c2;
{
   ...
   xferbuf.data[1] = a;
   xferbuf.data[2] = Pgrafi;
   memcpy(xferbuf.data+3,&c1,sizeof(int));
   memcpy(xferbuf.data+3+sizeof(int),&c2,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
VENT *w;
{
         ...
         memcpy(xferbuf.data+1,&w,sizeof(w));
         memcpy(xferbuf.data+1+sizeof(w),&(w->vini),sizeof(int));
         memcpy(xferbuf.data+1+sizeof(w)+sizeof(int),&(w->vfin),sizeof(int));
         senddatamessage(MySock.sock, &xferbuf);
         return(0);
}
...
char c;
{
   ...
   memcpy(xferbuf.data+2,&w,sizeof(w));
   memcpy(xferbuf.data+2+sizeof(w),&Pcursor,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int pi,pf;
{
   ...
   xferbuf.hdr.length = HDRSIZE + 1 + sizeof(w) + (sizeof(int)*2);
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&pi,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(w)+sizeof(int),&pf,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int pi,pf,tipo;
{
   ...
   memcpy(xferbuf.data+1+sizeof(w),&pi,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(w)+sizeof(int),&pf,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(w)+(sizeof(int)*2),&tipo,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
VENT *w,*wo;
{
   ...
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&wo,sizeof(wo));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int modo;
{
   ...
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&wo,sizeof(wo));
   memcpy(xferbuf.data+1+(sizeof(w)*2),&modo,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
char *tmp;
{
   ...
   memcpy(pdata,&(w->vini),sizeof(int));
   pdata += sizeof(int);  
   
   memcpy(pdata,&(w->vfin),sizeof(int));
   pdata += sizeof(int);
   
   
   memcpy(pdata,&(w->size),sizeof(int));
   pdata += sizeof(int);  
   ...
   for (i = 1;i < k;i++)
   {
      ...
      else
      {
         if (yyy > 6)
         {
            ...
            {
               ppp = iii;
               memcpy(pdata,(char *)&ppp,sizeof(short));
               pdata += sizeof(short);
...
int l;
{
   ...
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&l,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int lin;
{
         ...
         xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int);
         memcpy(xferbuf.data+1,&lin,sizeof(int));
         senddatamessage(MySock.sock, &xferbuf);
         return(0);
}
...
int ini,fin,n;
{
   ...
   memcpy(xferbuf.data+1,&ini,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(int),&fin,sizeof(int));
   memcpy(xferbuf.data+1+(sizeof(int)*2),&n,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
char *str;
{
   ...
   memcpy(xferbuf.data+1,&pos,sizeof(int));
...
int parada_en_terminal()
{
   ...
   do {
      ...
      pdata = xrcbbuf.data+1;
      memcpy(&bien,pdata,sizeof(int));
      pdata += sizeof(int);
      memcpy(&eqj,pdata,sizeof(int));
      return(bien);
   } while(-1);
}
...
char *defec;
{
   ...
   do {
      ...
      memcpy(xferbuf.data+1,&modo_de_validacion,sizeof(int));
      memcpy(xferbuf.data+1+sizeof(int),&pos,sizeof(int));
...
int m;
{
   ...
   xferbuf.data[0] = 27; /*  */
   xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int);
   memcpy(xferbuf.data+1,&m,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int l;
{
   ...
   xferbuf.data[0] = 28; /*  */
   xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int);
   memcpy(xferbuf.data+1,&l,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int lado;
{
   ...
   memcpy(xferbuf.data+1,&lado,sizeof(int));
...
char *version,*proceso,*titulo,*empresa;
{
   ...
   pdata = xferbuf.data+1;
   memcpy(pdata,&fecha_protegida,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&linea_25,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&duplicarusuario,sizeof(int));
   pdata += sizeof(int);
   ...
   if (rw_extranumuser)
   {
      *pdata = 1;
      ...
      memcpy(pdata,rw_extranumuser,sizeof(int));
      pdata += sizeof(int);
   }
   else
   {
...
int lineas;
{
   ...
   do {
      ...
      pdata = xferbuf.data + 1;
      memcpy(pdata,&modo_de_validacion,sizeof(int));
      pdata += sizeof(int);
      n = strlen(letras);
      memcpy(pdata,&n,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&pos,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&defecto,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&lineas,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&menusg_virtual,sizeof(int));
      pdata += sizeof(int);
      ...
      senddatamessage(MySock.sock, &xferbuf);
      if (leecontrol_en_terminal(col,sizeof(int)*3,NULL) < 0)
      {
         continue;
      }
      memcpy(&i,col,sizeof(int));
      memcpy(&_M_Cursor,col+sizeof(int),sizeof(int));
      memcpy(&_M_EXTRA_AVANCE,col+sizeof(int)*2,sizeof(int));
      memcpy(&menusg_virtual,col+(sizeof(int)*3),sizeof(int));
      return(i);
   } while(-1);
}
...
char *s;
{
   ...
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&Pcursor,sizeof(int));
...
int p;
{
   ...
   xferbuf.hdr.length = HDRSIZE + 1 + sizeof(w) + sizeof(int);
   memcpy(xferbuf.data+1,&w,sizeof(w));
   memcpy(xferbuf.data+1+sizeof(w),&p,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int pi,pf,m;
{
   ...
   xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int) * 3;
   memcpy(xferbuf.data+1,&pi,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(int),&pf,sizeof(int));
   memcpy(xferbuf.data+1+sizeof(int)*2,&m,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
   return(0);
}
...
int len;
{
   ...
   memcpy(xferbuf.data+1,&len,sizeof(int));
   if (len < 2)
			{
...
int ejecutae_terminal()
{
         ...
         do 
		 {
            ...
            xferbuf.hdr.length = HDRSIZE + 1 + sizeof(int);  
            memcpy(xferbuf.data+1,&server_id,sizeof(int));
            senddatamessage(MySock.sock, &xferbuf);
...
int entrada_usuario_terminal()
{
         ...
         pdata = xferbuf.data+1;
         memcpy(pdata,&user_who,sizeof(int));
         pdata += sizeof(int);
         memcpy(pdata,&subnumero_de_usuario,sizeof(int));
         pdata += sizeof(int);
...
int pi,pf,id,estado;
{
   ...
   pdata = xferbuf.data + 1;
   memcpy(pdata,&pi,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&pf,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&id,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&estado,sizeof(int));
   pdata += sizeof(int);
   senddatamessage(MySock.sock, &xferbuf);
}
...
int a,b,c,d;
{
   ...
   pdata += sizeof(int);
   memcpy(pdata,&c,sizeof(int));
   pdata += sizeof(int);
   memcpy(pdata,&d,sizeof(int));
   senddatamessage(MySock.sock, &xferbuf);
}
...
int len;
{
   ...
   do {
      ...
      pdata += sizeof(int);
      memcpy(pdata,p,len);
	  if (ff == 10 && *((int *)p) == DESPACHO_TRAEFICHERO)
		  trae = 1;
      senddatamessage(MySock.sock, &xferbuf);
      ...
      if (f >= 1/* && f <= 6*/ && !modeless)
      {
		  if (trae) {
			 int waitfor = 9991;
			 if ((nr = leecontrol_en_terminal(p,-1,&waitfor)) < 0)
			 {
				continue;
			 }
			 if (waitfor != 98) {
...
int llama_a_despacho_en_lineas(int pi,int pf,int modo)
{
   ...
   memcpy(pdata,&f,sizeof(int));
   pdata += sizeof(int);
   j = modo;
   memcpy(pdata,&j,sizeof(int));
   pdata += sizeof(int);
   *pdata = 1;
   pdata++;
   memcpy(pdata,&pi,sizeof(int));
...
void llama_a_despachador(int f,int parametro,char *puntero)
{
   ...
   pilatmp = malloc(101300);
 
   tmp = pilatmp;
   
   _f = (int *)tmp;
   if (f == DESPACHO_FUNCIONES_IMPRESION && parametro == 998)
	   *_f = -1;
   else
	   *_f = f;
   *(_f+1) = parametro;   
   ...
   char *p = puntero;
	 ...
	 if (puntero)
        ...
        memcpy(&tam,p,sizeof(long));
        p += sizeof(long);		
        memcpy(&envio,p,sizeof(long)); /* longitud esperada de vuelta */
        p += sizeof(long);		
        memcpy(&modo,p,sizeof(long));  /* flags */
        p += sizeof(long);		
    ...
    while (porenviar)
     ...
     if ((modo & 1))
		...
		/* doble mensaje para sincronizar .... */
		/*
		do 
		  {
          if ((recibidos = leecontrol_en_terminal(NULL,0,&waitfor)) < 0) pila! return;
		  } while(waitfor != 66);
        envia_control_a_terminal("",1,66);
		*/
		do
		  {
			if ((recibidos = leecontrol_en_terminal(NULL,0,&waitfor)) < (sizeof(long)+1)) 
			{
				free(pilatmp);
				return;
			}
		...
		pp = xrcbbuf.data+1;
        memcpy(&tam,pp,sizeof(long));
		pp += sizeof(long);
		parcial = *pp;
		pp++;
		memcpy(p,&tam,sizeof(long));
        tam -= sizeof(long);
		p += sizeof(long);
		if (parcial)
		   {
		   memcpy(&envio,pp,sizeof(long));
		   pp += sizeof(long);
		   }
		else
		   envio = tam;
   ...
   if (f == DESPACHO_PROCESAMENSAJES || f == DESPACHO_DATA_PROCESAMENSAJES)
   {
      int tam = sizeof(int);
      if (parametro != 2 && parametro != 4) tam *= 10;


      tmp[len] = 1; /* lleva puntero asociado ... */
      len++;            
	  if (f == DESPACHO_DATA_PROCESAMENSAJES)
	  {
		  tam += *((int *)puntero);
		  memcpy(tmp+len,puntero,sizeof(int)*3);
		  len += sizeof(int)*3;		  
	  }

	  /*
      memcpy(tmp+len,puntero,tam);
      len += tam;
	  */
      ...
      if (parametro !=2)
      {
          ...
          memcpy(puntero,tmp,tam);
      }
      else
      {
		    /*funcion_windows_cliente(10,tmp,len);*/
            funcion_windows_cliente(1010,tmp,len); /* NO requiere confirmacion */
            /*flush_mysock();*/
      }
	  free(pilatmp);
      return;
              ...
              f == DESPACHO_BOTONSALIDA)
   {
      ...
      tmp[len] = 1; /* lleva puntero asociado ... */
      len++;
      if (((f == DESPACHO_CCONTROL || f == DESPACHO_DIF_CCONTROL) && parametro == -1) || (f == DESPACHO_MAINCOMMAND && parametro == ID_DLGINDICES))
      {
          noret = 1;
          parametro = 0;
		  n = 0;
      }
	  else if (f == DESPACHO_DIF_CCONTROL || f == DESPACHO_DIF_CEDIT)
		  noret = 2;

	  tam = 0;
      do
      {
		 if (tam > 10000 && !heaptmp)
		 {
			 an = n;
			 alen = len;
			 app = pp;
			 do
			 {
				 ...
				 alen += px1[0];
				 app += px1[0];
				 if ((f != DESPACHO_CCONTROL && f != DESPACHO_CEDIT && f != DESPACHO_DIF_CCONTROL && f != DESPACHO_DIF_CEDIT) || !an) 
					 break;
				 an--;
			 } while(-1);
			 heaptmp = malloc(alen+1000);
			 memcpy(heaptmp,tmp,len);
			 tmp = heaptmp;
		 }
         px1 = (int *)pp;
         tam += px1[0];

		 if (f == DESPACHO_DIF_CCONTROL || f == DESPACHO_DIF_CEDIT)
		 {			 
			 ...
			 if (px1[4] == 2 || px1[4] == 7)
			 {
				 if (px1[10] != -1)
				 {
					if (f == DESPACHO_DIF_CEDIT && !((unsigned int)px1[10] & 0x40000000))
					{
						*_f = DESPACHO_CEDIT;
						f = DESPACHO_CEDIT;
						prev_id = 0xfffffffe;
			 ...
			 {
				 if (prueba_controles > 4194048)
					 prueba_controles = 1;

				 prev_id = prueba_controles++;				 
		         prev_id <<= 8;
			 }
			 if (prev_id != 0xfffffffe)
			 {
				 prev_id |= 0x40000000; 
				 prev_id &= 0xffffff00;
				 px1[2] &= 0xff;
      ...
      if (!noret && (f != DESPACHO_CEDIT || !(parametro & 256)))
      {
         funcion_windows_cliente(10,tmp,len); /* requiere confirmacion */
		 if (f == DESPACHO_CEDIT || f == DESPACHO_CCONTROL)
		 {
			 int *px2 = (int *)tmp;
			 pp = (char *)puntero;			 
			 n = parametro;
   ...
   {      
      ...
      memcpy(tmp+len,puntero,512);
      len += 512;
	  /*
	  if (parametro == 2 || parametro == 12)
	  {
		 funcion_windows_cliente(10,tmp,len);
		 memcpy(puntero,tmp,sizeof(int));
	  }
	  else */if (parametro == 1 || parametro == 7 || parametro == 8)
	  {
		 funcion_windows_cliente(10,tmp,len); /* requiere confirmacion */
		 memcpy(puntero,tmp,512);
	  }
	  else
	  {
		 funcion_windows_cliente(1010,tmp,len);
		 if (parametro == 2 || parametro == 12)
		 {
			 if (!(*((unsigned int *)puntero) & 0x80000000) && (*((unsigned int *)puntero) & 0x40000000))
			 {
				*((int *)puntero) &= 0xffffff00;
			 }
		 }
	  }
	  free(pilatmp);
   ...
   else if (f == DESPACHO_MI_CONTROL)
   {
         ...
         else
         {
            ...
            {   
               tmp[len] = 1;
               len++;
			   if (f == DESPACHO_MULTIPLE && (parametro == 12 || parametro == 13))
			   {				   
				   memcpy(tmp+len,puntero,*((int *)puntero));
				   len += *((int *)puntero);
			   }
			   else
			   {
	     ...
	     if (puntero && (f == DESPACHO_MULTIPLE || f == DESPACHO_MENUITEM || f == DESPACHO_INICIO_TRAZA || f == DESPACHO_FIN_TRAZA || f == DESPACHO_PUNTO_TRAZA || f == DESPACHO_TRAEFICHERO || f == DESPACHO_ENLACEWORD || f == DESPACHO_BROWSE || f == DESPACHO_SUPERUSUARIO))
		    {
		 ...
		 {
			if (/*f == DESPACHO_ESTADOCONTROL || */ f == DESPACHO_FOCOCONTROL || f == DESPACHO_REFRESCACONTROL /* || f == DESPACHO_DESTRUYECONTROL */)
			{
				funcion_windows_cliente(10,tmp,len);
			}
			else
...
int modo;
{
   ...
   do {
      ...
      memcpy(pdata,&ff,sizeof(FILE *));
      pdata += sizeof(FILE *);
      memcpy(pdata,&modo,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&pos,sizeof(long));
...
int tot;
{
   ...
   do {
      ...
      pdata = xferbuf.data+1;
      memcpy(pdata,&tot,sizeof(int));
      pdata += sizeof(int);
      memcpy(pdata,&file,sizeof(int));
      pdata += sizeof(int);
...
DWORD w_w;
{
   ...
   /*
   sprintf(tt,"non trobata[%ld] [%d][%d]",w_w,nextventfree,xrcbbuf.data[0]);
   aviso(0,tt);
   */
   return(stdpan);
}
...
int comando;
{
   ...
   */
	if (atencion_control_x && comando != 98) {
		atencion_control_x = 1;
	}   
   xferbuf.hdr.signature = RWNIDENTIFICACION;
...
int envia_control_a_server_oob(pdata,len,comando)
char *pdata;
int len;
int comando;
{
...
VENT *wo;
{
   ...
   while(*tmp)
   {
      ...
      if (p[i] == '.')
         punto = i;
      tmp++;
      if (i > 120) {aviso(0,"Nombre largo");return(-1);}
      i++;
   }
   p[i] = *tmp;
...
char *nom,*modo;
{
   ...
   if (*modo != 'r')
   { 
      aviso(0,"ERROR EN SISTEMA DE CACHE DE FICHEROS CLIENTE-SERVIDOR A1");
      return(fopen(nom,modo));
   }
   ...
   if (en_conexion_terminal)
   {
      ...
      do {
		 ...
		 */
         ...
         if (flagi || tmpl < (buf.st_mtime-1) || tmpl > (buf.st_mtime+1))
         {
...
char *pdata;
{
   ...
   memcpy(&server_id,pdata,sizeof(int)); /* en teoria deberia ya ser el mismo id */
   n = 0;
...
void pump_mensaje(DATAMSG *msg)
{
	...
	if (nterminal_cola_mensajes >= 2048)
	{
		aviso(0,"cola excedida");
		return;
	}
	if ((msg->hdr.command & 0x7f) != CONTROL_DE_SERVIDOR &&
		(msg->hdr.command & 0x7f) != CONTROL_DE_TERMINAL &&
		(msg->hdr.command & 0x7f) != COMANDO_TERMINAL)
	{
		return;
	}
	if ((msg->hdr.command & 0x7f) == FILE_TRANSFER_DATA || (msg->hdr.command & 0x7f) == FILE_TRANSFER_CLOSE || (msg->hdr.command & 0x7f) == FILE_TRANSFER_ACK || (msg->hdr.command & 0x7f) == FILE_TRANSFER_NACK)
		return;

	sendbuf = terminal_cola_mensajes[nterminal_cola_mensajes] = malloc(sizeof(DATAMSG));
	*sendbuf = *msg;

    sendbuf->hdr.command |= (unsigned char)128;	
	*((unsigned short *)(sendbuf->data+sendbuf->hdr.length-HDRSIZE)) = remoto_nivel_mensaje;
	sendbuf->hdr.length += 2;

	nterminal_cola_mensajes++;

	/*
    sprintf(tmp,"[%ld] control pruebas c/s",nterminal_cola_mensajes);
    mensaje(0,tmp);
	*/
...
int pop_mensaje(DATAMSG *msg,int waitfor,int modo)
{
	...
	if (!nterminal_cola_mensajes)
		return 0;
	for (i = 0;i < nterminal_cola_mensajes;i++)
	{
		_msg = terminal_cola_mensajes[i];
		if (modo == 1) {			
			if ((_msg->hdr.command & 0x7f) == CONTROL_DE_TERMINAL)
			{
				if (waitfor != -1) {
					if (_msg->data[0] != waitfor)
					{
			...
			if (waitfor == -1)
			{
				if ((_msg->hdr.command & 0x7f) != COMANDO_TERMINAL)
					continue;
			}
			else
			{
				if ((_msg->hdr.command & 0x7f) == CONTROL_DE_SERVIDOR)
				{
					if (_msg->data[0] != waitfor)
					{
						if (waitfor != 0 && waitfor != 1)
		...
		if (!(_msg->hdr.command & (unsigned char)128))
			break;
		if (*((unsigned short *)(_msg->data+_msg->hdr.length-HDRSIZE-2)) == local_nivel_mensaje)
		    break;
		if (*((unsigned short *)(_msg->data+_msg->hdr.length-HDRSIZE-2)) > local_nivel_mensaje)
		    break;
	}
	if (i >= nterminal_cola_mensajes)
		return 0;

	*msg = *terminal_cola_mensajes[i];
	free(terminal_cola_mensajes[i]);
	nterminal_cola_mensajes--;
	for (;i < nterminal_cola_mensajes;i++)
	{
		terminal_cola_mensajes[i] = terminal_cola_mensajes[i+1];
	}	
	if ((msg->hdr.command & (unsigned char)128))
	{
		msg->hdr.command &= ~(unsigned char)128;
		msg->hdr.length -= 2;
		remoto_nivel_mensaje = *((unsigned short *)(msg->data+msg->hdr.length-HDRSIZE));
	}
	else
		remoto_nivel_mensaje = 0;
...
int *waitfor;
{
   ...
   while(-1)
   {
				  ...
				  if (skip_pop || !pop_mensaje(&xrcbbuf,*waitfor,0))
				  {
					  if (skip_pop)
						  skip_pop--;
			 ...
			 errno = 0;
					  ...
					  {
						 if (finalizando_conexion) return(-1); 
						 aviso(0,"MAL MENSAJE 4");
						 return(-1);
					  }
				  }


				  if (remoto_nivel_mensaje < local_nivel_mensaje)
				  {
					  pump_mensaje(&xrcbbuf);
				      continue;
				  }
...
int leemensaje_oob_1(int *contador)
{
   ...
   if (cuanto > (HDRSIZE*2+1+sizeof(int)*3+1+5))
   {
	   if (recv(MySock.sock,(char *)&buffer,HDRSIZE*2+1+sizeof(int)*3+1+5,MSG_PEEK) == SOCKET_ERROR)
		   return -1;

	   p = buffer.data+1;
		   
	   if (buffer.hdrbloque.command == BLOQUE_DE_MENSAJES && (buffer.hdr.command&0x7f) == COMANDO_TERMINAL && buffer.data[0] == 61 && 
		   *((int *)p) == 10 && 
		   *((int *)(p+sizeof(int))) == DESPACHO_ESTADOCONTROL && !memcmp(p+sizeof(int)*3+1,"DATA:",5))
	   {
            int funcion;
            int parametro;
            char *puntero;
			
	        if (rwi_recvdata(MySock.sock,(char *)&buffer,HDRSIZE+buffer.hdr.length) == FALSE)
				return -1;
			...
			p += sizeof(int);
            memcpy(&funcion,p,sizeof(int));
            p += sizeof(int);
            memcpy(&parametro,p,sizeof(int));
            p += sizeof(int);
            if (*p)
               puntero = p+1;
            else
...
int temp;
{
   ...
   */
   ...
   if (log_enviafichero)
	   ff = fopen(log_enviafichero,"a");
   if (ff)
	   fprintf(ff,"enviafichero(%ld):%s\r\n",tam,nom);fflush(ff);
   ...
   err = 0;   
   if (!rwp_rcv_buffer && (err = select(MySock.sock+1,&setread,NULL,&seterror,NULL)) == SOCKET_ERROR || FD_ISSET(MySock.sock,&seterror))
   {	   
#ifndef WIN32
	 if (err == SOCKET_ERROR && errno == EINTR)
	 {
   ...
   if (xrcbbuf.hdr.command == FILE_TRANSFER_END)
   {
      ...
      if (en_conexion_terminal && DespachadorExterno)
      {
	   ...
	   if (ff)
		   fprintf(ff,"transfer_end\r\n",xrcbbuf.hdr.command);fflush(ff);
	   return 0;
   }

   if (xrcbbuf.hdr.command == FILE_TRANSFER_NOK) {
      rw_base_close_(file);
      if (en_conexion_terminal && DespachadorExterno)
      {
         (*DespachadorExterno)((int)DESPACHO_PROGRESO,2,(void *)NULL);
   ...
   {
	   if (xrcbbuf.hdr.command != FILE_TRANSFER_DATA && xrcbbuf.hdr.command != FILE_TRANSFER_CLOSE && xrcbbuf.hdr.command != FILE_TRANSFER_ACK && xrcbbuf.hdr.command != FILE_TRANSFER_NACK)
		   pump_mensaje(&xrcbbuf);
	   if (ff)
		   fprintf(ff,"paquete out of sync:%d\r\n",xrcbbuf.hdr.command);fflush(ff);
	   continue;
   }
   break;
   } while(-1);
   ...
   do {
	   ...
	   if (cuanto >= HDRSIZE) {
	       ...
	       if (ff)
		       fprintf(ff,"paquete (2) out of sync:%d\r\n",xrcbbuf2.hdr.command);fflush(ff);
		   pump_mensaje(&xrcbbuf2);
	   }
	   else
		   break;
...
*/
         ...
         )
      {
         ...
         establece_desconexion(0);
         aviso(0,"No se acepta la version del servidor");
         return(2);
      }
      /* peticion de sistema */
      pdata = tmp;
   ...
   case 40:
	  ...
	  nombredev[0] = 2;
	  nombredev[1] = 0;
	  strcpy(nombredev+2,"$$!/tmp/test_java.pdf");
   	  i_nlineas = -1;					 
	  i_anchocarro = 80;	  
      _imp = (FILE *)(unsigned long)0x00000003L;
	  */
   ...
   case 55:
      /* rentra */
      DebugBreak_(); /* eliminada todas las de rentra */
      break;
   case 56:
      /* timeout de teclado */
      memcpy(&k,pdata,sizeof(int));
      ...
      case 10:
         if (DespachadorExterno)
         {
			...
			if (funcion == -1) {
                ...
                memcpy(&funcion,pdata,sizeof(int));
				pdata += sizeof(int);
                memcpy(&ida,pdata,sizeof(int));
				pdata += sizeof(int);
                memcpy(&vuelta,pdata,sizeof(int));
				pdata += sizeof(int);
			}
            if (*pdata)
               puntero = pdata+1;
			...
			if (alternativo) {
				if (puntero)
					memcpy(tmp,puntero,ida);
				puntero = tmp;
				(*DespachadorExterno)(funcion,parametro,puntero);
				envia_control_a_server(tmp,vuelta,46);
			}
            else switch(funcion)
            {
			case DESPACHO_BINARIO:
			   if (!puntero)
				  ...
				  if ((modo&1))
				  {					 					 
                     envia_control_a_server(tmp,1,66); 
			         _leecontrol_en_server(NULL,0,NULL);
                  }
				  */
            ...
            case DESPACHO_MI_CONTROL:
               {                   
                  memcpy(tmp,puntero,1024);
                  (*DespachadorExterno)((int)DESPACHO_MI_CONTROL,parametro,tmp);
                  if (parametro == -1)
                  {                     
                     /*envia_control_a_server(tmp,1024,46);*/
                  }
                  else
                  {                     
                     envia_control_a_server(tmp,sizeof(int),46);
                  }                  
               }
               break;
			case DESPACHO_DATA_PROCESAMENSAJES:
			   memcpy(tmp,puntero,sizeof(int)*3);
            case DESPACHO_PROCESAMENSAJES:				
               if (parametro != 2)
               {
				    int len = sizeof(int)*10;
					if (funcion == DESPACHO_DATA_PROCESAMENSAJES)
					{
						len += *((int *)tmp);
					}
					server_espera_control++;
					ignora_procesamensajes++;
					do
					{
						(*DespachadorExterno)((int)funcion,parametro,tmp);
						if (*((int *)tmp) == TECLA_HINT)
						{	
							
							//flush_mysock();
							envia_control_a_server_oob(tmp,len,199);
					...
					ignora_procesamensajes--;
					server_espera_control--;
					envia_control_a_server(tmp,len,65);
					flush_mysock();
               }
			   else
			   {
            ...
            case DESPACHO_BOTONSALIDA:
               (*DespachadorExterno)(funcion,parametro,puntero);
               {
                  ...
                  if ((funcion == DESPACHO_CCONTROL && parametro == -1) || (funcion == DESPACHO_MAINCOMMAND && parametro == ID_DLGINDICES))
                  {
                      noret = 1;
                      parametro = 0;
					  n = 0;
				  ...
				  {
					  if ((funcion == DESPACHO_CEDIT && !(parametro & 256)) || funcion == DESPACHO_CCONTROL)
					  {
						  int tmp5[2048];
						  tam = 0;
						  do
						  ...
						  tam *= sizeof(int);
						  envia_control_a_server((char *)tmp5,tam,65);
						  break; /* del switch */
					  }
				  }
                  ...
                  do
                  {
                     ...
                     tam += px1[0];
                     pp  += px1[0];
                     if ((funcion != DESPACHO_CCONTROL  && funcion != DESPACHO_CEDIT) || !n)
                         break;
                     n--;
                  } while (-1);
                  if (!noret && (funcion != DESPACHO_CEDIT || !(parametro & 256)))
                  {
                     envia_control_a_server(puntero,tam,65);
                  }
               }
               break;
            ...
            case DESPACHO_PANTALLAS:
			   ...
			   /*if (parametro == 2 || parametro == 12)
			   {				   
				   envia_control_a_server(puntero,sizeof(int),65);				   
			   } else */
			   if (parametro == 1 || parametro == 7 || parametro == 8)
			   {
				   envia_control_a_server(puntero,512,65);
			   }
            ...
            case DESPACHO_TRAEFICHERO:
               ...
               envia_control_a_server(tmp,strlen((char *)tmp)+1,98);
			   atencion_control_x = 0;
               break;
			case DESPACHO_PONINI:
			   ...
			   break;
            default:
			   if (funcion == DESPACHO_MENSAJE_CABECERA)
			   {
				   funcion = DESPACHO_MENSAJE_CABECERA;
			   }
               (*DespachadorExterno)(funcion,parametro,puntero);  
			   if (funcion == DESPACHO_MULTIPLE || funcion == DESPACHO_MENUITEM || funcion == DESPACHO_INICIO_TRAZA || funcion == DESPACHO_FIN_TRAZA || funcion == DESPACHO_PUNTO_TRAZA || funcion == DESPACHO_ENLACEWORD || funcion == DESPACHO_BROWSE || funcion == DESPACHO_SUPERUSUARIO)
			   {
				  if (puntero)
				  {
			   ...
			   {
				   if (/*funcion == DESPACHO_ESTADOCONTROL || */ funcion == DESPACHO_FOCOCONTROL || funcion == DESPACHO_REFRESCACONTROL /*|| funcion == DESPACHO_DESTRUYECONTROL */)
				   {
#ifdef FGVWIN
			/* si se activa ojo con el signal_ping */
					   /*agi_cola_win();*/
#endif
...
*/
      ...
      case 15:
         {
#ifdef FGVJAVA
			 if (strstr(pdata,"Excel.Sheet ")) {
				if (DespachadorExterno)
					(*DespachadorExterno)((int)DESPACHO_MULTIPLE,665,pdata);		   
			 }
			 else
				 system_proceso(pdata);
#else
      ...
      default:
         break;
            ...
            case 100:
               ...
               establece_desconexion(0);
               aviso(0,"LA VERSION DEL CLIENTE ES INFERIOR A LO REQUERIDO");
               return(2);
            case 103:
               aviso(0,"El SERVIDOR SE ESTA ACTUALIZANDO, ACCEDA PASADO UN TIEMPO PRUDENCIAL");
            case 101:
...
{   
   ...
   flush_mysock();
   if (leecontrol_en_server(NULL,0,42) < 0)
	   return;
   modo_terminal();
}


int examina_mensaje_terminal()
{
...
int envia_debug() {
    xferbuf.hdr.signature = RWNIDENTIFICACION;                 
    xferbuf.hdr.command = DS_DEBUG_BREAK;
    xferbuf.hdr.length = HDRSIZE;
    senddatamessage(MySock.sock,&xferbuf);
    flush_mysock();
}
...
int modo_terminal()
{
   ...
   while(-1)
   {
         ...
         do {
            ...
            if ((err = select(MySock.sock+1,&setread,NULL,&seterror,&timeout)) == SOCKET_ERROR || (err && FD_ISSET(MySock.sock,&seterror)))
            {
               ...
               {				   
                  ...
                  {
                     aviso(0,"LA CONEXION CON EL SERVIDOR SE HA ROTO (EXCEPCION)!! REINTENTO EN 60 Segundos");
                  }
                  else
                  {
                     aviso(0,"LA CONEXION CON EL SERVIDOR SE HA ROTO!! REINTENTO EN 60 Segundos");
                  }				  
                  establece_desconexion(0);
				  /*
                  sleep(60);		  
      ...
      if (rwp_rcv_buffer || FD_ISSET(MySock.sock,&setread))
      {
         ...
         {
            if (xfer->hdr.command == CONTROL_DE_SERVIDOR || xfer->hdr.command == FILE_TRANSFER_CLOSE || xfer->hdr.command == FILE_TRANSFER_ACK || xfer->hdr.command == FILE_TRANSFER_NACK)
            {
...
#ifdef RWSERVER
extern int es_dssql;
int DSSQL_atexit(int portc,char *mensaje) {
	struct sockaddr_in addr;
	int len;
	SOCKET cc;
#ifdef FGVDOS
...
#endif
	len = sizeof(addr);
	cc = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	addr.sin_family = AF_INET;
	addr.sin_port   = htons((unsigned short)portc);
	addr.sin_addr.s_addr = inet_addr("127.0.0.1");
	if (connect(cc,&addr,len) != SOCKET_ERROR) {
		rwi_senddata(cc,mensaje,-1);
#ifdef FGVDOS
	    Sleep(100);
#else
	    usleep(100000);
...
int DSTCP_VInterface(int port,int param,int portc)
{
		...
		{	
			...
			}
			{
			 ...
			 errno = 0;
					...
					{
						if (param == 9999) {
							if (itcp_varios(client,3000,linea+5) <= 0)
								break;
						}
						else {
