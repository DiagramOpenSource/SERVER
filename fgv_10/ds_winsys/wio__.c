...
static unsigned int salida_caracter = 0;
static int inix = 0;
static int iniy = 0;
...
int parada()
{
   int c;
   recurre_raton(0);
   pprint(2460,"Pulse Tecla o Boton");
   c = que_qjes(leecar(),0);
   pprint(2460,"                   ");
   recurre_raton(1);
   return(c);
...
/* que se desactivan pgup y pgdn */
{
   ...
   {
      if (!menusg_botones || menusg_botones == (int *)0x00000001L)
									aviso(0,"uso incorrecto de menusg_...");
      else
      {
									ojo_al_dato = 2;
                           for (i=0;i<numero;i++)
...
BOOL CALLBACK AGIMPRESORA(HWND hWnd, UINT message,WPARAM wParam, LPARAM lParam)
{
 		 char tmp[81];
         char opc[81];
         FILE *file;
...
LPSTR FileName;
{
   ...
   if (hpalCurrent == NULL) {
      aviso(0,"Fallo al crear la paleta del DIB");
      return FALSE;
			}
...
void init_bmp_base(BMP_BASE_PAR *pPar)
{
         ...
         if (!pRorigen || pRorigen->top || pRorigen->bottom || pRorigen->left || pRorigen->right)
...
int old_ini_w_printer(m)
{
   ...
   {
      sprintf(tmp,"No se puede comenzar el proceso de impresion [%d]",GetLastError());
...
int id;
{
   ...
   if (id >= 823 && id <= 842)
      ...
      if (p)
      {
         sprintf(tmp,"Función%d Programada con %s",id-822,p);
      }
      else
         if (id < 833)
            sprintf(tmp,"Función %d (F%d)",id-822,id-822);
         else
            sprintf(tmp,"Función %d (Ctrl-F%d)",id-822,id-832);
			}
   else
      if (id == 801)
         strcpy(tmp,"VALIDAR");
               ...
               if (id == 807)
                  ...
                  if (id == 821)
                     ...
                     if (id == 820)
                        ...
                        if (id == 815)
                           strcpy(tmp,"AVANCE PAGINA");
                        else
                           if (id == 814)
                              strcpy(tmp,"RETROCEDER PAGINA");
                           else
                              if (id == 822)
                                 strcpy(tmp,"AYUDA");
                              else
                                 if (id == 1100)
                                    strcpy(tmp,"PROGRAMACION DE FUNCION");
                                 else
                                    if (id >= 1000 && id < 1100)
                                       strcpy(tmp,"Pulse el botón izquierdo para seleccionar la opcion");
                                    else
                                       tmp[0] = 0;
}
...
DS_WINSYS_API void WINAPI mensaje_mousemove(UINT x1,UINT y1,UINT Flags)
{
   ...
   if (abajo == 1 && !bTrack && (Flags & MK_LBUTTON) /*&& ((Flags & MK_CONTROL) || (Flags & MK_SHIFT))*/)
   {
      bTrack = TRUE;         
      ClearSelection(principal_w, &Rect, (Flags & MK_SHIFT) ? SL_EXTEND | Shape : Shape);
      SetRectEmpty(&Rect);
   ...
   if (ti)
   {
      ...
      mensaje_especial("Pulse el Boton Izquierdo del Raton Para Acceder a la Opcion");
      return; // nada mas
   }
   ...
   else
   {
      ...
      if (esta_en_el_raton(x,&estado) != -1)
      {
         ...
         else
         {
            ...
            mensaje_especial("Pulse el Boton Izquierdo del Raton Para Seleccionar la Opcion");
         }
      }
      else
      {
...
*/   
     ...
     default:
        ...
        {            
           ...
           if (__wParam == BUSCA_PTECS || __wParam == PIDO_TRASMISION || __wParam == PIDO_USUARIOS)
           {
               if (HaySeleccion == TRUE && wParam != MID_XCLIPCOPY)
               {
                  ClearSelection(principal_w, &Rect, (wParam & MK_SHIFT) ? SL_EXTEND | Shape : Shape);
           ...
           if (wParam <= -MID_XPRIMERA && wParam > -MID_XACERCA)
           {
               if (HaySeleccion == TRUE && wParam != MID_XCLIPCOPY)
               {
                  ClearSelection(principal_w, &Rect, (wParam & MK_SHIFT) ? SL_EXTEND | Shape : Shape);
              ...
              if (wParam >= 800 && wParam < MID_XACERCA)
              {
                   if (HaySeleccion == TRUE && wParam != MID_XCLIPCOPY)
                   {
                      ClearSelection(principal_w, &Rect, (wParam & MK_SHIFT) ? SL_EXTEND | Shape : Shape);
...
static int pon_pulsado(pulsacion)
int pulsacion;
{
   int i;
   if (pulsacion != -1)
...
int m;
{
   ...
   posible_char = 0;
   while (SinColaMensajes || GetMessage((LPMSG)&msg,NULL,0,0))
   {
      ...
      {          
	     ...
	     if (msg.hwnd && (msg.hwnd != principal_w || !agi_acelerador || !TranslateAccelerator(msg.hwnd, agi_acelerador, (LPMSG)&msg)))
                {
      ...
      if (salida_caracter == MID_DISENO)
      {
                           ...
                           w = w_ini(401,480);
                           w_salva(w);
                           mensaje(0,"Pulse la Tecla de Funcion a Programar");
                           MessageBeep(0);		  
                           salida_caracter = -1;
                           continue;
      }
      ...
      {
									aviso(0,"Solo se Programa desde la Funcion 11 hasta la 20");
      }
      if (en_programa == 1 && salida_caracter > 832 && salida_caracter < MID_MENUSG)
      {
									en_programa = 2;
                           mensaje(0,"Seleccione en el Menu la Opción a asociar con la Función");
      ...
      if (en_menu && en_programa != 2)
      {
									if (w)
                           {
                              aviso(0,"Anulada la Programación de la Función");
                              w_pinta(w);
                              w_fin(w);
                              w = NULL;
                           }
                           ...
                           default:
                              if (salida_caracter >= MID_XPRIMERA && salida_caracter < MID_XOPCION1)
                              {
                                 ...
                                 if (en_programa == 2)
                                 {
                                    ...
                                    if (!strlen(tmp2))
                                    {
                                       aviso(0,"Solo se pueden programar opciones con acceso directo");
                                       break;
                                    }
                                    quitabp(tmp2);
                                    ...
                                    sprintf(tmp2,"Entre Descripcion Para la Funcion%d",que_programa+1);
                                    ...
                                    MessageBeep(0);
                                    MessageBeep(0);
                                    sprintf(tmp2,"Funcion %d Programada con: %s",que_programa+1,desc_funs[que_programa]);
                                    aviso(0,tmp2);
                                    en_programa = 0;
                                    
                                    if (graba_fgv(1))
                                    {
                                       aviso(0,"La Funcion no se ha grabado permanentemente");
                                    }
                                    if (DespachadorExterno)
                                    {
                                 ...
                                 else
                                 {
                                    if (wwmenus)
                                    {
                                       if (!en_lee_teclado)
                                       {
                                          aviso(0,"ESPERE A QUE EL PROCESO EN CURSO TERMINE O PIDA UN ENTRADA DE DATOS");
                                       }
                                       else
                                       {
