...
extern INST std_inst[] = {
		  /* SIMBOLO          PARAMETROS        INSTRUCCION CASOS */
				 ...
				 "T_RECIBE_FICHERO"   ,"95959595"		 ,215,NULL,
				 ...
				 "T_ENVIA_FICHERO"   ,"95959595"	 ,224,NULL,
				 "FLEE_NUMERO",  "9598"              ,225,NULL,
				 "FGRABA_NUMERO","9598"              ,226,NULL,
				 "IP_REMOTO",    "95"				 ,227,NULL,
				 "NOMBRE_IP",    "9595"              ,228,NULL,			 
				 "ENLAZA_PROCESO", "9E7098"		     ,229,NULL,
				 "DESENLAZA_PROCESO","9E7098"	 	 ,230,NULL,
				 ...
				 "PRINT_PA"         ,"2198"          ,247,NULL,
				 "WORD_ASIGNA_TEXTO","989599"        ,248,NULL,
				 "ESPECIFICA"     , "959F"           ,249,NULL,
                 ...
                 "FOCOTECLADO"    , "98"             ,254,42,
                 "SUB_PINPA"      , "2198"           ,254,43,
                 "BORRA_HIJOS_PAN", "21"             ,254,44,
                 "DESTRUYECONTROL", "98"             ,254,45,
                 "REFRESCACONTROL", "98"             ,254,46,
                 "CONTROL_SIMPLE",  "9895989870"     ,254,47,
                 "BOTON_SALIDA"  ,  "9898989598"     ,254,48,
                 "OCX_REGISTRA"  ,  "95"             ,254,49,
                 "OCX_ACTIVA"    ,  "9598"           ,254,50,
                 "OCX_LIBERA"    ,  "98"             ,254,51,
                 "OCX_INVOCA"    ,  "989598989898"   ,254,52,
                 "MAINCOMMAND"    , "989595959595"   ,254,53,
                 "BLANCODLG"      , "9898"           ,254,54,
				 "MENUITEM"		  , "9895"           ,254,55,
				 "VALOR_INI"	  , "9595"           ,254,56,
				 "INICIOCOM"	  , "959895989898"   ,254,57,
				 "LEECOM"         , "959895"         ,254,58,
				 "GRABACOM"       , "9595"           ,254,59,
				 "FINCOM"         , "95"             ,254,60,
				 "OEM_ANSI"       , "9090"           ,254,61,
				 "ANSI_OEM"       , "9090"           ,254,62,
				 "VENTANA"        , "9898989895"     ,254,63,
				 "FINVENTANA"     , "98"             ,254,64,
				 "PCONTEXTO"      , "98989898"       ,254,65,
				 /* definicion de def ? */
				 NULL,NULL,255,NULL,
			};
...
int *f,*c;
{
	...
	if (nom[0] == '#') {
		...
		if (nom[final+1] == '#') 
        {
			...
			if (nom[final+fin_1+2] == '#') 
            { /* tercera parrilla = indireccion */
	...
	if (*f != F_INVALIDO && *f >= 0) {
		...
		if (fin_1 != -1 && flag >= 0) 
        {
			if (localiza_ipfi(*f) < nfi && *c >= vg_mpfi((*f))->f_ncampos) 
            {
				un_error("campo ilegal",nom);
...
int *j;
{
	...
	if (f1 > -2 && localiza_ipfi(f1) < nfi && c1 >= 0) {
		if (f1 == -1) {
		...
		if (tipo == CHARTIPO) {
			if (f2 > -2  && localiza_ipfi(f2) < nfi && c2 >= 0) {
				if (f2 == -1) {
...
int f,c,m;
{
	...
	if (c < 0) {
		...
		if (f == -1) {
            ...
            if (c < 0 || c >= vg_mpfi(f)->f_ncampos) return 1;
...
char *p;
{
	...
	if (_nivel == 1) {
        ...
        */
        ...
        {
            char tmpy[100];
            sprintf(tmpy,"errores(%d)",c_nerror);
            pprint(0,tmpy);
        }        
	}
...
for (i = 1;i < argc;i++)
{
   if (argv[i][0] == '-')
   {
      ...
      if (argv[i][1] == "N")
	     {         
         numero_generacion = atoi(argv[i]+2);
		 }
      else
	     {
		 bien = 0;
...
if (!bien) 
   ...
   pprint(101,"dscomp [-5 o -6 -Ddirbase -Mmodificadorpath -Nnumero_generacion] procesos\n");
   exit(2);
   }
...
fichs = (FF **)malloc(sizeof(char *) * MAXDEF*2);
...
for (j = 1;j < argc && !abortar;j++)
{
    ...
    *quita_path(tmp) = 0;
    ...
    for (j = 0;lista && lista[j] && !abortar;j++)
    {
        ...
        if (lista_inc)
        {
            for (nfi = 0;lista_inc[nfi];nfi++)
            {
                if (!strcmp(lista_inc[nfi],lista[j]))
                    break;
            }
            if (lista_inc[nfi])
            {
