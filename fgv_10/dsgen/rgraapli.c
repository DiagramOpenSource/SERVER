...
for (i = 0;i < totalfichs;i++) 
{
   ...
   if (fichs[i]->fichero[0] != ':')
   {
        c = strlen( quita_path(fichs[i]->fichero) ) + 1;
   }
   else
   {
       c = strlen( fichs[i]->fichero ) + 1;
   }
   ...
   {
       tamano += (unsigned int)rw_write_(file1,fichs[i]->fichero,c);
   }
}
...
graba_short(file1,(char *)&i3,sizeof(short));
    ...
    for (m = 0;m < 103;m++)
    {
        pppps[m] = (vg_mpfi(0)->tipo_de_def ^ nume_code());
    }
    mm = 1;
    for (m = 1;m < nfi;m++)
    {
		...
		ii = vg_mpfi(m)->tipo_de_def ^ nume_code();
        for (i = 0;i < mm;i++)
        {
            if (pppps[i] == ii)
            {
	...
	for (m = 0;m < /*nfi*/103;m++)
	{
        graba_short(file1,(char *)&(pppps[m]),sizeof(short));
	}
/*
for (m = 0;m < nfi;m++)
   {
