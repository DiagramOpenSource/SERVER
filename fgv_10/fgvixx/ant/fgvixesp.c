...
static int lock_control[IXXMAXFILES*2+5] = {0,};
...
static int lee_modo_diu(int fd)
{
	...
	ixx_lee(lfd,&(modo_diu[fd]),sizeof(long));	
    ...
    if (r >= 0 && ixx_haydiu && fd_diu && fd_diu[fd] >= 0)
    {
        ...
        if (m_diu)
        {
			...
			if (m_diu > 1)
			{
				if (rw_modulo)
				{
					ixx_graba(fd_diu[fd],rw_modulo,8);
				}
				else
				{
					ixx_graba(fd_diu[fd],"xxxxxxxx",8);
				}
			}
        }
        update_diu(1);
