...
char *pathdef;
{
            ...
            else
            {
               if (ixxfichs[ixx_fd]->pseudo_cierra)
               {
                  if (ixxfichs[ixx_fd]->nombredat && !strcmp(nombre,ixxfichs[ixx_fd]->nombredat) && ixxfichs[ixx_fd]->nombreixx)
                  {
                     ...
                     ixxfichs[ixx_fd]->fd_dat = (*ixxdat_abre)(ixxfichs[ixx_fd]->nombredat,NULL,NULL,NULL,NULL,NULL,&(ixxfichs[ixx_fd]->dtabla),ixxfichs[ixx_fd]->pathdef);
                     ixx_ojo_recursion = 0;
                     _e_log_open++;
#ifdef CONTROLNLM
                     pprint(2501,"        ");
		 ...
		 */
         ...
         ixxfich->fd_dat = (*ixxdat_abre)(nombre,descamp,defectos,&(ixxfich->tamreg),ixxfich,&ojo_norec,&(ixxfich->dtabla),pathdef);
         /* printf("punto 6\n");getchar(); */
#ifdef CONTROLNLM
         pprint(2501,"        ");
         sleep(1);
...
int ixx_fd,compactar;
{
...
#ifndef DSDBODBC
   void *respaldo_externo_ixx_init(int fd);
   ...
   if (ixxfich->fd_dat < 0)
			{
      ixxfich->fd_dat = (*ixxdat_abre)(ixxfich->nombredat,NULL,NULL,NULL,NULL,NULL,&(ixxfich->dtabla),ixxfich->pathdef);
			}
...
#ifndef DSDBODBC
   conrespaldo = respaldo_externo_ixx_init(ixx_fd);
#else
   conrespaldo = 0;
#endif
...
int ixx_directo_abre_datos(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla,char *pathdef)
{
	return (*ixxdat_abre)(nom,descamp,defectos,longi,ixxfic,ojo,dtabla,pathdef);
}
