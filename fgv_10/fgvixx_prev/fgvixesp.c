...
struct dbf_cabecera /* DBF manejo independiente del indice ... */
{
    unsigned char id;
    unsigned char any_update;
    unsigned char mes_update;
    unsigned char dia_update;
    unsigned long records;
    unsigned short  len_header;
    unsigned short  len_record;
...
struct dbf_campo
{
    ...
    unsigned char tipo;
    unsigned long data_address; /* desplazamiento del dato */
    unsigned char len;
...
struct dbf_desc
{
    struct dbf_cabecera cabeza;
    int fd;
    int tocado;
    int ncampos;    
    struct dbf_campo *campos; 
    FgvStdTabla *dbf_tabla;  
};
...
short ixxdat_graba_ddx(char **descamp,char *nombre,char *nom)  /* funciones para el fichero de datos .dat compatible C-Isam */
{
    FILE *file;
    int i;
    char c;
...
int ixx_dat_pasa_a_campo(Uchar *p2,Uchar *p1,int ntipos,int nlens,int tipos,int lens,int flag)
{
    ...
    if (tipos == IXXCALFA)
        ...
        default:
            if (ixxmen_aviso)
            {
                (*ixxmen_aviso)("Control en dat_pasa_a_campo [B]");
            }
            break;
        }
    }
...
static int dbf_crea_cabecera(char *nom,char *nombre,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla)
{
    int fd,xfd;
    char tmp[512];
    struct dbf_desc dbf;
        ...
        {		  
            ...
            else
            {
                ...
                {          
                    ixx_dat_cierra(fd);
                    no_crear_otro = 1;
                    fd = dat_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla);
                    no_crear_otro = 0;
                    if (fd >= 0)
                    {			              
                        dlen = dtabla->reclen;
                        ...
                        {     
                            ...
                            no_crear_cabecera  = 1;
                            xfd = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla);                
                            no_crear_cabecera  = 0;
                            if (xfd < 0)
                            {
                                free(tmp_buf);
                            ...
                            {
                                if (dbf_graba(xfd,reg,tmp_buf+1,dlen) < 0)
                                {
                                    dbf_cierra(xfd);
                                    dat_cierra(fd);
                                    unlink(nombre);
...
int dbf_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla)
{
    int fd,xfd;
    ...
    {
        if (no_crear_cabecera || dbf_crea_cabecera(nom,nombre,descamp,defectos,longi,ixxfic,ojo,dtabla) < 0)
        {
            return(-1);
        }
        fd = ixx_abre_dat(nombre);  
    ...
    if (fd >= 0)
    {
        ...
        if (ixx_lee(fd,&dbf,sizeof(dbf)) != sizeof(dbf) || (dbf.id != IXXDBFID && dbf.id != 0x3 && dbf.id != 0x30 /* && dbf.id != 0x83 */))
        {
            ...
            if (rename(nombre,tmp))
            {
                return(-1);
            }
            return(dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla));
        }
        ...
        if (difiere || faltan || sobran)
        {
            ...
            {
                if (dbf_crea_cabecera(nom,nombre,descamp,defectos,longi,ixxfic,ojo,dtabla) >= 0)
                {
                    recursion = 1;
                    fd_tmx = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla);
                    recursion = 0;
                    if (fd_tmx < 0)
                    {
                        free_dbf(i);
                ...
                while ((r = dbf_lee(i,reg,tmp_buf1,0 /* forzar lectura directa */)) >= 0)
                {
                    ...
                    if (dbf_graba(fd_tmx,reg,tmp_buf2,0) < 0)
                    {
                        malas++;
                    }
                    if (r)
...
int dbf_graba(int fd,long reg,Uchar *buf,int len)
{
    long pos;
    struct dbf_desc *dbf;
    char c = ' ';
...
int dat_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla)
{
        ...
        if (fd < 0)
        {
            ...
            if (fdbtr == -1 && !no_crear_otro)
            {
                ...
                if (fd >= 0)
                {
                    ...
                    /* ojo hay un DBF */
                    no_crear_otro = 1;
                    fd = dbf_abre(nom,descamp,defectos,longi,ixxfic,ojo,dtabla);			  
                    no_crear_otro = 0;
                }
                            }		  
                    ...
                    if (fd >= 0 && descamp)
                    {
                        ...
                        if (!file)
                        {
                            ntamreg = ixxdat_graba_ddx(descamp,nombre,nom);
                            if (ntamreg && longi)
                                *longi = ntamreg;
                            if (fdbtr > -1)
                            {
                        ...
                        if (file)
                        {
                            ...
                            nombre[0] = 'N';
                            fgets(nombre,250,file);
                            if (nombre[0] != 'N')
                                convierte = 1;
                            tamreg = 0;
                            ntamreg = 0;
                            nc = 0;
                            while(fgets(nombre,250,file) && nc < 256)
                            {
                                if (nc >= ncamp)
                                {
                                    discrepa = 2;
                                ...
                                if (discrepa < 2)
                                {
                                    if (tipos[nc] != ntipos[nc])
                                    {
                                        if (tipos[nc] != IXXCSHORT && tipos[nc] != IXXCDOUBLE)
                                        {
                                            ...
                                            if (ntipos[nc] != IXXCSHORT && ntipos[nc] != IXXCDOUBLE)
                                            {
                                                discrepa = 2;
                                            }
                                            else
                                                if (nlens[nc] != sizeof(short) && nlens[nc] != sizeof(double))
                                                {
                                                    discrepa = 2;
                                                }
                                                else
                                                    if (lens[nc] != sizeof(short) && lens[nc] != sizeof(double))
                                                    {
                                                        discrepa = 2;
                                                    }
                                                    else
                                    ...
                                    if (nfd >= 0)
                                    {
                                        ...
                                        if (nfd >= 0)
                                        {
                                            ...
                                            if (fdbtr > -1)
                                            {
                                                ...
                                                {
                                                    sprintf(nombre,"Aviso de Control en pase BTRIEVE[%d][%d]",*(short *)(p_block+128),tamreg);
                                                    if (ixxmen_aviso)
                                                    {
                                                        (*ixxmen_aviso)(nombre);
                                                    }
                                            ...
                                            while(-1)
                                            {
                                                ...
                                                if (ok)
                                                {
                                                    ...
                                                    if (fd > -1)
                                                        ixx_dat_cierra(fd);
                                                    if (fdbtr > -1 || unlink(nombre+128) >= 0)
                                                    {
                                                        if (rename(nombre,nombre+128) < 0)
                                                        {
                                                            ixx_lck_lseek(ixxfic->fd,LCKREZONAINI,SEEK_SET);
                                                            ixx_lock(ixxfic->fd,LK_UNLCK,1L);
                                                            ret(-1);
                                                        ...
                                                        sprintf(nombre,"%s.ddx",nom);
                                                        if (ixxdat_graba_ddx(descamp,nombre,nom) != ntamreg)
                                                        {
                                                            sprintf(nombre,"Aviso de Control de Averia en recuperacion de datos [%d]",ntamreg);
                                                            if (ixxmen_aviso)
                                                            {
                                                                (*ixxmen_aviso)(nombre);
                                                            }
        ...
        
{
    ...
    {   
        ...
        else
        {
            ...
            no_report_error = 1;
            no_crear_cabecera = 1;
            xfd = dbf_abre(otro_nom,descamp,defectos,longi,ixxfic,&ojete,dtabla);
            no_crear_cabecera = 0;
            if (xfd < 0)
            {
                strcpy(nombre,nom);
...
int dat_graba(int fd,long reg,Uchar *buf,int len)
{
    long pos = (reg-1)*(len+1);
    char c = 10;
    if (reg <= 0L || reg > ixxfich->cabecera.nrecords) 
...
{   
    ...
    if (ixx_control_diu) /* en este caso no queda mas remedio que bloquear */
    {
        ...
        while(-1) /* se espera (de momento asi) indefinidamente */
        {
            ...
            if (!ixx_lock(fd_ciu,LK_WAIT,(long)sizeof(long))) break;
        ...
        ixx_lseek(fd_ciu,(long)sizeof(long),SEEK_SET); 
        ixx_lock(fd_ciu,LK_UNLCK,(long)sizeof(long));
    }
    else
    {
 ...
 */
    ...
    do /* redundante pero para aseguarse del momento */
    {
        ixx_lseek(fd_ciu,(long)sizeof(long),SEEK_SET);
        if (ixx_lee(fd_ciu,(char *)&control_diu,sizeof(long)) != sizeof(long))
        {
    ...
    for (i = 0;i < total;i++)
    {
       ...
       {
           ixx_lseek(fd_ciu,(long)-((sizeof(long)*2)+52),SEEK_CUR);
           if (el_def)
           {
                memset(tmp_d,0,52);
                strcpy(tmp_d,el_def);
...
int diu_abre(char *nom,char **descamp,char *defectos,short *longi,IXXFILE *ixxfic,int *ojo,FgvStdTabla *dtabla)
{
    int i;
    int fd = (*ixxdiu_abre)(nom,descamp,defectos,longi,ixxfic,ojo,dtabla);
    ...
    if (fd >= 0 && ixx_haydiu)
    {
        ...
        if (ok)
        {
            ...
            {
                /* error o que? */
                ixx_cierra(fd_diu[fd]);
            }
            sprintf(tmp,"%s.diu",nom);

            update_diu(0);
            fd_diu[fd] = ixx_abre_dat(tmp);

            if (fd_diu[fd] >= 0)
            {
                if (!ixx_lseek(fd_diu[fd],0L,SEEK_END))
                {
                   ixx_cierra(fd_diu[fd]);
                   unlink(tmp);
                   fd_diu[fd] = -1;
...
int diu_graba(int fd,long reg,Uchar *buf,int len)
{
    int r = (*ixxdiu_graba)(fd,reg,buf,len);
    if (r >= 0 && ixx_haydiu && fd_diu && fd_diu[fd] >= 0)
    {
        ...
        ixx_graba(fd_diu[fd],&c,1);
        ixx_graba(fd_diu[fd],&reg,sizeof(reg));
        if (ixx_control_diu)
        {
            long hora = time(NULL);
            char usuario[20];
            ixx_graba(fd_diu[fd],buf,len);
            ixx_graba(fd_diu[fd],&hora,sizeof(long));
            memset(usuario,0,20);
            if (rw_usuario)
               strcpy(usuario,rw_usuario);
            ixx_graba(fd_diu[fd],usuario,10);
            ixx_graba(fd_diu[fd],&contador_global_diu,sizeof(long));
        }
        update_diu(1);
        errno = _ant_errno;
    }
...
int diu_borra(int fd,long reg,int len)
{
    ...
    char *buf = NULL;
    if (ixx_haydiu && fd_diu && fd_diu[fd] >= 0 && ixx_control_diu)
    {
        buf = malloc(len+1);
        memset(buf,0,len+1);
...
int diu_cierra(int fd)
{
    ...
    if (ixx_haydiu && fd_diu && fd_diu[fd] >= 0)
    {
        int _ant_errno = errno;
        ixx_cierra(fd_diu[fd]);
        fd_diu[fd] = -1;
